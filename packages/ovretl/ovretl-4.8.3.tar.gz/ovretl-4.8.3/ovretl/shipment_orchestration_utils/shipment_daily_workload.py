...
def shipments_remaining_days(active_shipments_data_df: pd.DataFrame, today: datetime.date) -> pd.DataFrame:
    ...
    shipments_with_start_dates_df.loc[:, "remaining_days"] = shipments_with_start_dates_df["total_transit_time"] - (
        today - shipments_with_start_dates_df["workflow_start_date"]
    ).apply(lambda delta: delta.days) - 1

...
class AFM:
    ...
    def baseline(
        ...
        for curve_number in range(0, self.z_position_raw.shape[1]):
            ...
            piezo_position = np.concatenate((_z, _z), axis = 0)
            ...
            if approach_dev < max_approach_noise and retract_dev < max_retract_noise:
                ...
                self.z_position[:,i] = piezo_position
    ...
    def adjust_tip_position(
        ...
        for i in range(0, self.z_position.shape[1]):
            ...
            tip_approach = _z[0:self.samps_line] + self.spr_const * _approach
            tip_retract = _z[(self.samps_line + 1):(self.samps_line * 2)] + self.spr_const * _retract

            tip_position = np.concatenate((tip_approach, tip_retract), axis = 0)

            self.z_position[:,i] = tip_position
    ...
    def plot_adjusted(
        ...
        '''
        fig = go.Figure()
        fig.add_trace(go.Scatter(y = self.approach[:,curve_number], x = self.z_position[:,curve_number][0:self.samps_line], mode='lines', name = 'Approach curve'))
        fig.add_trace(go.Scatter(y = self.retract[:,curve_number], x = self.z_position[:,curve_number][(self.samps_line + 1):(self.samps_line * 2)], mode='lines', name = 'Retract curve'))
        fig.update_layout(
            yaxis_title_text='Force (nN)',
            xaxis_title_text='Tip separation (nm)',
        )
    ...
    def plot_curves(self):
        ...
        for step in range(0, self.z_position.shape[1]):
            ...
            trace_1.append(go.Scatter(
                        ...
                        x = self.z_position[:, step][0:self.samps_line]
                    ))
            ...
            trace_2.append(go.Scatter(
                        ...
                        y = self.retract[:, step],
                        x = self.z_position[:, step][(self.samps_line + 1):(self.samps_line * 2)]
                    ))

...
class AFM:
    ...
    def nanoscope_read(
        ...
        self.z_position_raw = np.zeros([self.samps_line * 2 , len(self.filelist)])
        ...
        for filename in self.filelist:
            ...
            tip_position = hscale * np.arange(1, self.samps_line + 1)

            if adjusted_tip_position is True:

                _z_approach = tip_position + self.spr_const * _approach_curve
                _z_retract = tip_position + self.spr_const * _retract_curve

                _z_position = np.concatenate((_z_approach, _z_retract), axis=0)

            else:
                _z_position = np.concatenate((tip_position, tip_position), axis=0)
    ...
    def plot_raw(
        ...
        '''
        fig = go.Figure()
        fig.add_trace(go.Scatter(y = self.approach_raw[:,curve_number], x = self.z_position_raw[:,curve_number][0:int(self.samps_line/2)], mode='lines', name = 'Approach curve (raw)'))
        fig.add_trace(go.Scatter(y = self.retract_raw[:,curve_number], x = self.z_position_raw[:,curve_number][int(self.samps_line/2):self.samps_line], mode='lines', name = 'Retract curve (raw)'))
        fig.update_layout(
    ...
    def baseline(
        ...
        for curve_number in range(0, self.z_position_raw.shape[1]):
            ...
            approach_baseline_region = np.arange(baseline_start,baseline_end)
            retract_baseline_region = np.arange(baseline_start + self.samps_line, baseline_end + self.samps_line)

            _approach_region = _approach[approach_baseline_region]
            _retract_region = _retract[retract_baseline_region]
            _z_region = _z[approach_baseline_region]
    ...
    def plot_adjusted(
        ...
        '''
        fig = go.Figure()
        fig.add_trace(go.Scatter(y = self.approach[:,curve_number], x = self.z_position[:,curve_number][0:int(self.samps_line/2)], mode='lines', name = 'Approach curve'))
        fig.add_trace(go.Scatter(y = self.retract[:,curve_number], x = self.z_position[:,curve_number][int(self.samps_line/2):self.samps_line], mode='lines', name = 'Retract curve'))
        fig.update_layout(
            yaxis_title_text='Force (nN)',
            xaxis_title_text='Tip separation (nm)',
        )
    ...
    def plot_curves(self):
        ...
        for step in range(0, self.z_position.shape[1]):
            ...
            trace_1.append(go.Scatter(
                        ...
                        x = self.z_position[:, step][0:int(self.samps_line/2)]
                    ))
            ...
            trace_2.append(go.Scatter(
                        ...
                        y = self.retract[:, step],
                        x = self.z_position[:, step][int(self.samps_line/2):self.samps_line]
                    ))

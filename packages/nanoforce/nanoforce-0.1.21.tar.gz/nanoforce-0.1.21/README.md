...
```
By default, the length position used will be the piezo position, rather than the actual tip position. If a cantilever is likley to experience large deflection when in contact with the surface, the tip position can be estimated using the cantilever spring constant and measured force by setting the 'adjust_tip_position' input to True.
```
expt_1.baseline(adjust_tip_position = True)
```
## Contact point

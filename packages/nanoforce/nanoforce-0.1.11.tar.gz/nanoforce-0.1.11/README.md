...
```
By default, the length position used will be the piezo position, rather than the actual tip position. If a cantilever is likley to experience large deflection when in contact with the surface, the tip position can be estimated using the cantilever spring constant and measured force by setting the 'adjusted_tip_position' input to True.
```
expt_1.nanoscope_read(adjusted_tip_position = True)
```
## Plot raw data
...
```
expt_1.calc_modulus(poisson_ratio = 0.5, indenter_radius = 10 * 1e-9, soft_indenter = False, indenter_poisson_ratio = 0.5, indenter_modulus = 300)
```

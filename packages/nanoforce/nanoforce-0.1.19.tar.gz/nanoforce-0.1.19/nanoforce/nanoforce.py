...
class AFM:
    ...
    def baseline(
        ...
        for curve_number in range(0, self.z_position_raw.shape[1]):
            ...
            if adjust_tip_position is True:
                
                tip_approach = _z + self.spr_const * adjusted_approach
                tip_retract = _z + self.spr_const * adjusted_retract

                tip_position = np.concatenate((tip_approach, tip_retract), axis = 0)

            else:
                tip_position = np.concatenate((_z, _z), axis = 0)

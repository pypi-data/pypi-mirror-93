...
class AFM:
    ...
    def adjust_tip_position(
        ...
        for i in range(0, self.z_position.shape[1]):
            ...
            tip_approach = _z[:self.samps_line] + _approach / self.spr_const
            tip_retract = _z[self.samps_line:] + _retract / self.spr_const
    ...
    def plot_adjusted(
        ...
        '''
        fig = go.Figure()
        fig.add_trace(go.Scatter(y = self.approach[:,curve_number], x = self.z_position[:,curve_number][:self.samps_line], mode='lines', name = 'Approach curve'))
        fig.add_trace(go.Scatter(y = self.retract[:,curve_number], x = self.z_position[:,curve_number][self.samps_line:], mode='lines', name = 'Retract curve'))
        fig.update_layout(
            yaxis_title_text='Force (nN)',
            xaxis_title_text='Tip separation (nm)',
        )
    ...
    def plot_curves(self):
        ...
        for step in range(0, self.z_position.shape[1]):
            ...
            trace_1.append(go.Scatter(
                        ...
                        x = self.z_position[:, step][:self.samps_line]
                    ))

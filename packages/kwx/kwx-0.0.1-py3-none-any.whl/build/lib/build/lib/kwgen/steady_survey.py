...
"""
Text corpus keyword cleaning, modeling, analysis, and generation
...
def _prepare_corpus_path(
    ...
    if type(text_corpus) == str:
        try:
            ...
            text_corpus, clean_texts = prepare_survey_data(
                data=text_corpus,
                input_language=input_language,
...
def graph_topic_num_evals(
    ...
    for m in method:
        ...
        for t_n in topic_nums_to_compare:
            ...
            topics_dict[t_n] = _order_and_subset_by_coherence(
                model=tm, num_topics=t_n, num_keywords=num_keywords
            )[0]

            coherence_dict[t_n] = get_coherence(
                model=tm,
                text_corpus=text_corpus,
                num_topics=t_n,
                num_keywords=num_keywords,
...
def gen_word_cloud(
    ...
    """
    text_corpus = _prepare_corpus_path(
        text_corpus=text_corpus,
        clean_texts=None,
        input_language=input_language,
        incl_mc_questions=incl_mc_questions,
    ...
    display_string = _combine_tokens_to_str(
...
def _order_by_pos(outputs, output_language):
    ...
    if (
        output_language in lem_lang_abbr_dict.values()
...
def gen_survey_keywords(
    ...
    if method == "frequency" or method == "tfidf":
        if method == "frequency":
            ...
            if type(corpuses_to_compare) == str:
                try:
                    ...
                    corpuses_to_compare = prepare_survey_data(
            ...
            frequent_words = gen_survey_keywords(
                method="frequency",
                text_corpus=text_corpus,
                input_language=input_language,
        ...
        if return_topics:
            ...
            if output_language != input_language:
                ordered_topic_words = translate_output(
    ...
    if output_language != input_language:
        translated_keywords = translate_output(
            outputs=keywords,
            input_language=input_language,
            output_language=output_language,
        )
...
def gen_analysis_files(
    ...
    best_method, model_ideal_topic_num, ideal_lda_num_topics = graph_topic_num_evals(
        method=method,
        text_corpus=text_corpus,
        clean_texts=clean_texts,
        input_language=input_language,
    ...
    most_fred_kw = gen_survey_keywords(
        method="frequency",
        text_corpus=text_corpus,
        clean_texts=clean_texts,
        input_language=input_language,
    ...
    while more_words_to_ignore != False:
        ...
        new_words_to_ignore, words_added = prompt_for_ignore_words(
            ignore_words=new_words_to_ignore
        )
    ...
    ordered_most_freq_kw = _order_by_pos(

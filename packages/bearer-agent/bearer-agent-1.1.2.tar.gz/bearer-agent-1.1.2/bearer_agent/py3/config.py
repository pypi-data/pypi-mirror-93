...
def setup_hooks(cfg):
    ...
    # httpcore HTTP/2 modules are imported the first time a HTTP2 connection is made
    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_httpcore_http2_sync"), "httpcore._sync.http2"
    )
    wrapt.register_post_import_hook(
        _import_module_hook("bearer_agent.py3.hooks.intercept_httpcore_http2_async"), "httpcore._async.http2"
    )

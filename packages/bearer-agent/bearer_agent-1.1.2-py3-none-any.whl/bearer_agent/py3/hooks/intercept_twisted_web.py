...
def new_client_request_construct_wrapper(wrapped, _instance, args, kwargs):
    ...
    def _parse(method, uri, _headers, _body_producer, persistent=False,
               parsedURI=None):
...
class ConsumerProxy(wrapt.ObjectProxy):
    ...
    def write(self, bytes):
        ...
        try:
            ...
            self.__report__.report.end_report(classname(exc), "%r" % exc)
            reraise(*sys.exc_info())
...
def new_client_request_write_headers_wrapper(wrapped, instance, args, kwargs):
    ...
    setattr(instance, "bodyProducer", ProducerProxy(instance.bodyProducer, report, REQUEST_STAGE))
...
class ReadProtocolProxy(wrapt.ObjectProxy):
    ...
    def connectionLost(self, reason):
        ...
        ResponseDone = getattr(importlib.import_module("twisted.web._newclient"), "ResponseDone")
...
def new_client_response_construct_wrapper(wrapped, instance, args, kwargs):
    ...
    report.update(
        {
            "statusCode": status,
            "responseHeaders": (_process_headers, headers)
        },
...
def intercept(module):
    wrapt.wrap_function_wrapper(
        module,
        "Request._construct",
        new_client_request_construct_wrapper,
    )

    wrapt.wrap_function_wrapper(
        module,
        "Request._writeHeaders",
        new_client_request_write_headers_wrapper,
    )

    wrapt.wrap_function_wrapper(
        module,
        "Response._construct",
        new_client_response_construct_wrapper,
    )

    wrapt.wrap_function_wrapper(
        module,
        "Response.deliverBody",
        new_client_response_deliver_body_wrapper,
    )

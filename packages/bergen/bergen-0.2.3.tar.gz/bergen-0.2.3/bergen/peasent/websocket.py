...
class WebsocketPeasent(BasePeasent):
    ...
    async def connect_websocket(self):
        ...
        hosted_templates = [str(templateid) for templateid, function in self.templateid_template_map.items()]
        hosted_templates_qstring = ",".join(hosted_templates)

        uri = f"{self.websocket_protocol}://{self.websocket_host}:{self.websocket_port}/peasent/{self.unique_name}/?token={self.token}&templates={hosted_templates_qstring}"
        try:

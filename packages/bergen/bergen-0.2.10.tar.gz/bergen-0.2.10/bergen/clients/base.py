...
class BaseBergen:
    ...
    def __init__(self, auth: BaseAuthBackend, host: str, port: int, protocol = "http", auto_negotiate=True, bind=True, log=logging.INFO, local=None, loop=None,  client_type: ClientType = None, **kwargs) -> None:
        ...
        if auto_negotiate == True and not self.loop.is_running(): # If we are running in a loop we shouldn't negotiate
            self.negotiate()
    ...
    async def negotiate_async(self, client_type=None):
        ...
        self.main_ward = SubscriptionGraphQLWard(host=self.host, port=self.port, protocol=self.protocol, token=self.token, loop=self.loop)

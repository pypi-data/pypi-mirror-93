...
class BasePeasent(ABC):
    ...
    async def setup_and_run(self):
        ...
        self.templateid_function_map = {value[0].id: value[1] for value in all_templates_function_set}
        self.templateid_template_map = {value[0].id: value[0] for value in all_templates_function_set}

        logger.info(f"Following functions have been allowed! {[value[1].__name__ for value in all_templates_function_set]}")

        for value in all_templates_function_set:
            logger.info(f"Requesting to host {value[1].__name__} as Template {value[0].id}" )

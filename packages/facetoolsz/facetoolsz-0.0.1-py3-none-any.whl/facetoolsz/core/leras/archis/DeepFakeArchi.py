...
class DeepFakeArchi(nn.ArchiBase):
    ...
    def __init__(self, resolution, mod=None, opts=None):
        ...
        if mod is None:
            ...
            ### 这里的lowest_dense_res 如果是DF-ud等带有d的结构则全连接时除以32，因此最后的上采样会多进行一次
            ### Encoder 已经进行4次下采样，意味着在Inter阶段的分辨率为res//16, 但是高分图像除以32相当于分辨率变为res//32,
            ### 因此在Inter进行了一次上采样之后，普通的Decoder进行三次上采样，带D的encoder进行了4次上采样。

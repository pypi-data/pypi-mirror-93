...
class EventBus(GLXCurses.EventBusClient):
    ...
    def emit(self, detailed_signal, args):
        ...
        :type args: dict
        """
        GLXCurses.MainLoop().event_list.add(detailed_signal, args)
    ...
    def events_dispatch(self, detailed_signal, args):
        ...
        if (
                GLXCurses.Application().menubar
                and GLXCurses.Application().menubar == GLXCurses.Application().has_focus
        ):
            GLXCurses.Application().menubar.events_dispatch(detailed_signal, args)
        else:
            GLXCurses.Application().active_window.widget.events_dispatch(
                detailed_signal, args
            )

        if not isinstance(
                GLXCurses.Application().active_window.widget, GLXCurses.Dialog
        ):
...
class Application(EventBus, GLXCurses.Area, Spot, Handlers, metaclass=Singleton):
    """
    :Description:
    ...
    # Galaxie EvLoop
    def evloop_input_event(self):
        return self.screen.lowlevel_getch()

    def evloop_cmd(self):
        self.refresh()

    def evloop_finalization(self):
        self.screen.close()

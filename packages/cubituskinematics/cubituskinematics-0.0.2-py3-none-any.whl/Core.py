...
class Core(object):
    ...
    -> PerformGeometricShape()\tfunction that physically performs specified geometric shape
    -> PerformCustomCurve()\tfunction that physically performs specified mathematic function described with formulas (one for each axis)
    NOTE: Robotic arm performs physical movement ONLY when it's possible (so both parameters of the arm and specified end position are valid)
    """
    ...
    def LoadXML(self, fileName, name):
        self.__xmlName = f"{os.path.dirname(os.path.abspath(fileName))}/{name}"
    ...
    def PerformCustomCurve(self, Xequation, Yequation, Zequation, sampling=50, repeat=1):
        if sampling not in range(10, 101): raise ValueError("Incorrect sampling! Number must be in range <10; 100>.")
...
#----------------------------------------------------------------------------------------#
    ...
    def PerformUICustomCurve(self, sampling, repeat):
        from numpy import sin, cos, tan, pi, log
        fk = ForwardKinematics()
        fk.SetDefaultParameters(self.__xmlName)
        ik = InverseKinematics(fk)
        ...
        while(True):
            try:
                ...
                print("Your custom input function was written incorrectly!\n")
                if input("Do you want to try again? (Y/N): ") not in ['Y', 'y']: return
                clear()

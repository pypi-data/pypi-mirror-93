...
class JavaScriptEvaluator(Evaluator):
    ...
    def _split_commands(self, script: Script) -> tp.List[Script]:
        ...
        limiters = r'(^[\s]*[{}]+[\s]*)|([\s]*[{}]+[\s]*$)'
        open_parenthesis = 'if', 'switch', 'for', 'while', 'function', 'catch'
        not_commands = 'else', 'finally', 'default'
        
        re_commands = self._re(pattern)
        re_limiters = re.compile(limiters)
        
        coded_script = self._hide_strings(script)
        coded_script = coded_script.replace('\n', ' ')
        commands = list()
        ...
        while match:
            ...
            command = re_limiters.sub('', command)
            
            script = script[end:]
            coded_script = coded_script[end:]
            
            if match.groups()[-1] is None:
                if any(command.startswith(word) for word in open_parenthesis):
                    after_word = f'{command[-1]}{coded_script}'
                    start, end = self._find_scope(after_word, '(', ')')
                    if end >= 0:
                        command = f'{command[:-1]}{after_word[:(end + 1)]}'
                        script = script[end:]
                        coded_script = coded_script[end:]
            
                if any(command.startswith(word) for word in not_commands):
                    match = re_commands.search(coded_script)
                    continue
    ...
    def count_chars(self, script: Script) -> Count:
        ...
        script = self._remove_excess_spaces(script)
        script = self._remove_new_lines(script)
    ...
    def count_commands(self, script: Script) -> Count:
        ...
        This count does not considers:
            ...
            * Comments
            * Static values
            * Clauses `else`, `finally` and `default`
...
class Sample(object):
    ...
    def __init__(
            ...
            max_functions: int = 1,
            max_commands: int = 20,
            span_lines_by_commands: tp.Tuple[float, float] = (1.33, 6.5)) -> None:
        super().__init__()
    ...
    def _check_analysis_values(self) -> AnalysisChecks:
        ...
        return (
            ...
            (lines_by_commands < self.span_lines_by_commands[0]),
            (lines_by_commands > self.span_lines_by_commands[1]),
        )
...
class LongScript(Analyser):
    ...
    def __init__(
            ...
            max_functions: int = 1,
            max_commands: int = 20,
            span_lines_by_commands: tp.Tuple[float, float] = (1.33, 6.5),

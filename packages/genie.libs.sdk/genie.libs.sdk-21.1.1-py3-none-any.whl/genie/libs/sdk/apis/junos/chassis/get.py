...
def get_chassis_fpc_cpu_util(device, expected_slot, all_slots=False):
    """Returns chassis cpu utilization
    Args:
        device (obj): Device object
    ...
    if all_slots:
        return(out.q.get_values('cpu-total'))

    slot_ = out.q.get_values('cpu-total', expected_slot)
    return int(slot_) if slot_ else None


def get_chassis_fpc_cpu_util(device, cpu_total = 'cpu-total'):
    ...
    fpc_cpu_util = out.q.get_values(cpu_total)

    if fpc_cpu_util:
        return fpc_cpu_util[0]

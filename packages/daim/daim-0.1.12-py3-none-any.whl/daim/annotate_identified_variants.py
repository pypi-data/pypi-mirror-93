...
def annotate_mutations(path):
    # Open variant file with pandas
    identified_variants = pd.read_csv(path, sep='\t')
    
    doc_name = 'daim_report.pdf'
    ...
    p = Paragraph('Variant Call File Name: ' + '\t' + '\t' + '\t' + str(path) + '\n', style_['BodyText'])
    content.append(p)

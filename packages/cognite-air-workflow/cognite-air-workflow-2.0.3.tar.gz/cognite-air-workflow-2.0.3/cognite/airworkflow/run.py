...
def handle_call_schedulemanager():
    ...
    for project in project_to_config:
        ...
        function_asset = client.functions.retrieve(external_id=const.SCHEDULEMANAGER_EXTERNAL_ID)
        air_functions_asset = client.functions.retrieve(external_id=const.AIR_FUNCTIONS_SCHEDULEMANAGER_EXTERNAL_ID)
        if function_asset is not None:
            client.functions.call(external_id=const.SCHEDULEMANAGER_EXTERNAL_ID, wait=False)
        elif air_functions_asset is not None:
            client.functions.call(external_id=const.SCHEDULEMANAGER_EXTERNAL_ID, wait=False)
        else:

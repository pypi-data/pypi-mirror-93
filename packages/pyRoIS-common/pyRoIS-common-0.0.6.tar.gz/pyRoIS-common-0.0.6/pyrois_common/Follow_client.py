...
class Command(RoIS_Common.Command):
    ...
    def set_parameter(self, target_object_ref, distance, time_limit):
        s = self._proxy.set_parameter(target_object_ref, distance, time_limit)
...
class Query(RoIS_Common.Query):
    ...
    def get_parameter(self):
        (status, target_object_ref, distance, time_limit) = self._proxy.get_parameter()
        return (RoIS_HRI.ReturnCode_t(status), target_object_ref, distance, time_limit)
...
class Follow_Client(Command, Query, Event):
    """Follow_Client
    """
    def __init__(self, uri):
        self._uri = uri

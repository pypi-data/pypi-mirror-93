# Person_Identification_Client.py
#
...
class Event(RoIS_Common.Event):
    ...
    def poll_event(self):
        ...
        if methodname == 'person_identified':
            self.person_identified(params[0], params[1])

    def person_identified(self, timestamp, person_ref):
        self.events.append((timestamp,person_ref))
        # print("person_identified",timestamp, person_ref)
        if self.q is not None:
            self.q.put(("person_identified",timestamp, person_ref))
        # self.logger.debug('received completed event'
        #                 + command_id
        #                 + RoIS_Service.Completed_Status(status).name)


class Person_Identification_Client(Command, Query, Event):
    """Person_Identification_Client
    """

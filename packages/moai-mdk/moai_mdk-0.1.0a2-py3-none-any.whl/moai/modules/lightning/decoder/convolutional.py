...
class Convolutional(torch.nn.Module):
    def __init__(self,
        ...
        in_features, out_features = configuration.bottleneck_features, configuration.out_features
        next_features = in_features // configuration.feature_block_divisor
        for block_size in configuration.blocks:
            module_list.append(miup.make_upsample(
                upscale_type=upscale.type,
                features=in_features,
                **upscale.params
            ))
            for b in range(0, block_size):
                module_list.append(miconv.make_conv_block(
                        block_type="conv2d",

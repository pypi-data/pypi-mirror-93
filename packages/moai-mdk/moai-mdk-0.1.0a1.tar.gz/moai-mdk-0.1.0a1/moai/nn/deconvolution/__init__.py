...
def _update_deconv_op(name: str, type: typing.Type):
    if name not in __DECONV_FACTORY__.keys():
        ...
        log.error(f"Trying to add an already existing key {name} in the deconvolution operation factory.")
...
def make_deconv_op(
        ...
        **kwargs
        ...
        if deconvolution_type in __DECONV_FACTORY__.keys():
            return miu.instantiate(__DECONV_FACTORY__[deconvolution_type], 
            **{
                **locals(),
                **kwargs
            })
...
make_deconv_4x4 = functools.partial(make_deconv_op, #NOTE: only pass deconv_type and inout features
    kernel_size=4,
    stride=2,
    dilation=1,
    padding=1,
...
def _update_deconv_block(name: str, type: typing.Type):
    if name not in __DECONV_BLOCK_FACTORY__.keys():
        ...
        log.error(f"Trying to add an already existing key {name} in the deconvolution block factory.")
...
def make_deconv_block(
    ...
    **kwargs
    ...
    if block_type in __DECONV_BLOCK_FACTORY__.keys():
        return miu.instantiate(__DECONV_BLOCK_FACTORY__[block_type],
            **{ 
                **locals(),

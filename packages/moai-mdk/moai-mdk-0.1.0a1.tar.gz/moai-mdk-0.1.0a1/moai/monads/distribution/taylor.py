...
class Taylor(torch.nn.Module):
    ...
    def forward(self, 
        ...
        heatmaps:       torch.Tensor,       # spatial probability tensor of K keypoints
            ...
            _heatmaps = heatmaps.clone().detach() #TODO: check if clone is needed or if detach suffices
            _coords = coords.clone().detach()
            heatmap_height = _heatmaps.shape[-2]
            heatmap_width = _heatmaps.shape[-1]
            if self.grid_type != 'coord':
                _coords[..., 0] = (_coords[..., 0] + 1.0) / 2.0 * heatmap_width if self.grid_type == 'ndc' else _coords[..., 0] * heatmap_width
                _coords[..., 1] = (_coords[..., 1] + 1.0) / 2.0 * heatmap_height if self.grid_type == 'ndc' else _coords[..., 1] * heatmap_height            

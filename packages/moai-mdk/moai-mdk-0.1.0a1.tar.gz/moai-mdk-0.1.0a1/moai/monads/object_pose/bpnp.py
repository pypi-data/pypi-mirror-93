...
class BPnPFunction_fast(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            coefs = _get_coefs(P_6d[i].reshape(1,6), pts3d_flat.reshape(n,3), K).detach()
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                r = pts2d_i*Si-proj_i[0:2,:]
                coefs = _get_coefs(P_6d_flat.reshape(1,6), pts3d_flat.reshape(n,3), K_flat.reshape(3,3))
                coef = coefs[:,:,j].transpose(0,1) # size: [2,n]
                fj = (coef*r).sum()
                fj.backward()
                J_fy[j,:] = P_6d_flat.grad.clone()
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                pts2d_i = pts2d_flat.reshape(n, 2).transpose(0, 1)
                pts3d_i = torch.cat((
                    pts3d_flat.reshape(n, 3), torch.ones(n, 1, device=device)
                ), dim=-1).t()
                proj_i = KP.mm(pts3d_i)
                Si = proj_i[2, :].reshape(1, n)
...
class BPnP(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.BPnP_func = BPnPFunction if not isBatch else BPnPFunction_fast \
            if useFast else BPnPFunction_m3d

...
__all__ = ["Stanford2D3D"]

class Stanford2D3D(torch.utils.data.Dataset):
    def __init__(self, 
        filename: str,
        ...
        self.entries = _load_paths(filename, "Stanford", placements, image_types)
        log.info(f"Loaded {len(self.entries)} items from Stanford2D3D.")

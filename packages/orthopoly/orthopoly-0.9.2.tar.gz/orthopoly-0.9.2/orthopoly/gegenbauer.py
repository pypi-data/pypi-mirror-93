...
def gegen_hat(xhat, n, m):
    ...
    #instability warning
    if m > 45:
        warnings.warn('WARNING: the three term recurrence relation used to evaluate the Gegenbauer polynomials is weakly unstable for high order (m) evaluations. Relative error may be ~1e-10 for m~=49.5')
    # m = 0 warning
    if m == 0:
        warnings.warn('WARNING: the recurrence relationship used to compute the Gegenbauer polynomials will always yield zero for m=0.')
    #low n cases
    if n == 0: return(1.0 + 0.0*xhat) #want to return 1 in a vectorized way

...
from titanfe.ujo_helper import convert_ujotypes, python_to_ujo
...
class Brick:
    ...
    def __init__(self, instance_definition: BrickInstanceDefinition, metric_emitter, logger):
        ...
        self.input_mapping_by_port = {
            port["name"]: PortMapping(
                port["mappingRules"], schema_to_type(port["schema"], port["typeName"])
            )
            for port in instance_definition.ports.input
        }
        self.output_mapping_by_port = {
            port["name"]: PortMapping(
                port["mappingRules"], schema_to_type(self.flow.schema, "messageType")
            )
            for port in instance_definition.ports.output
        }
    ...
    def enqueue_result_as_packet(self, result, port=None, parent_packet=None):
        """
        map the result into a packet according to the mapping rules
        and put it into the result queue
        """
        ...
        if not self.output_mapping_by_port:
            if parent_packet:
                parent_packet.payload = result
            else:
                parent_packet = Packet(port=port, payload=result)

        else:
            rules, target_type = self.output_mapping_by_port[port]
            if not parent_packet:
                parent_packet = Packet(port=port, payload=deepcopy(target_type))

            parent_packet.payload = convert_ujotypes(
                source=result, target=parent_packet.payload, rules=rules
            )
    ...
    async def execute_brick(self, packet):
        ...
        if self.input_mapping_by_port:
            rules, target = self.input_mapping_by_port[packet.port]
            payload = convert_ujotypes(source=packet.payload, target=target, rules=rules)

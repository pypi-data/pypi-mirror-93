...
class TriggerImportDataBaseResource(Resource):
    ...
    def get(self):
        try:
            task_url    = analytics_conf.BASE_URL + self.get_task_url()
            queue_name  = self.get_task_queue()
            payload     = self.get_data_payload()or {}
            
            current_app.logger.debug('task_url=%s', task_url)
            current_app.logger.debug('queue_name=%s', queue_name)
            ...
            current_app.logger.debug(get_tracelog())
        ...
        try:
            ...
            if total_count>0:
                try:
                    ...
                    if task_url:
                        task_url    = analytics_conf.BASE_URL + task_url
                        queue_name  = self.get_task_queue()
                        
                        current_app.logger.debug('InitImportDataBaseResource: task_url=%s', task_url)
                        current_app.logger.debug('InitImportDataBaseResource: queue_name=%s', queue_name)
            ...
            current_app.logger.error('Faield due to %s', get_tracelog()) 

...
class Engine:
    ...
    def add(self, sprite, mass: float = 1, body: int = DYNAMIC,
        ...
        if group in sprites or group in forces:
    ...
    def update(self):
        for sprite in self.sprites:
            ...
            if type(sprite) in forces:
                sprite.calculate(self)
...
class Gravity_Force(Circle):
    
    def __init__(self, x: float = 100, y: float = 100, strength: float = 10000,
                 pull: float = 1, size: float = 50,
                 color: arcade.Color = (0, 0, 0), outline: float = 0):
        
        super().__init__(x, y, size, color, outline)
    ...
    def _set_strength(self, value: float):
    ...
    def _set_pull(self, value: float):
        self._pull = value
    ...
    def calculate(self, engine):
        for sprite in engine.sprites:
            ...
            if sprite is not self:
                x, y = Direction(
                    self._strength / (sprite.distance(self) / self._pull),
                    sprite.angle(self))

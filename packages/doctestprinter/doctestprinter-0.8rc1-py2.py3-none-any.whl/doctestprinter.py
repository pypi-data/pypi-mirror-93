...
def print_pandas(
    ...
    **kwargs
    ...
    Examples:
        ...
        The main task of :func:`print_pandas` is the possibility to define the
...
def prepare_pandas(
    ...
    """
    Builds explicitly string represenations for *pandas.DataFrame* and
...
def _count_length(sequence) -> int:
    return len(sequence)
...
class PandasFormatSpecification(Mapping):
    ...
    def decode_specification(format_definition: str) -> dict:
        ...
        match = _DECODES_FORMAT_DEFINITION.match(format_definition)
...
def _format_values_of_series_to_representation(
    ...
    max_item_length = formatted_values.map(_count_length).max()
...
_DECODES_FORMAT_DEFINITION = re.compile(
    r"{:?(?P<align>[<>=^]?)?(?P<width>\d+)?(?P<precision>\.\d+)?(?P<repr_type>[bcdeEfFgGnosxX%]?)?}"
)
_SPLITS_FORMAT_DEFINITIONS = re.compile(
...
def _split_formats_to_index_value_groups(all_formats: str) -> Dict[str, str]:
    ...
    """
    matched_formats = _SPLITS_FORMAT_DEFINITIONS.match(all_formats)
...
def _prepare_title_definition(index_to_print: pandas.Index, values_column_count):
    """
...
def _generate_dataframe_representation(
    ...
    Args:
        ...
        formatter(str):
            Compressed all_formats for the index and values.
...
def _set_defaults_setup(all_formats, max_line_count, max_title_width):
    """

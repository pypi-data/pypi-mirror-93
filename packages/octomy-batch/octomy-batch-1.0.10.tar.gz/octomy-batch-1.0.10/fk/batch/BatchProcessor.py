...
class BatchProcessor:
    ...
    def get_item_types(self):
        module_filepath = self.config.get("batch-filter-root", "/tmp/inexistant")
        logger.info(f"Looking for types in {module_filepath}:")
        ret = []
        # for path in glob.glob(module_filepath):
        for path_object in Path(module_filepath).rglob("*.py"):
    ...
    def list_modules(self):
        ...
        if not module_root_dir:
            ...
            files_objects = Path(module_filepath).rglob("*.py")
            for file_object in files_objects:
                module_filepath = file_object.name

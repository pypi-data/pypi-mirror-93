    ...
    def _prepare_content(self, content: Dict) -> List:
        ...
        if 'result' in content and 'data' in content['result']:
            if 'columns' in content['result']:
                ...
                prepared_content = content['result'].get('data', [])

        return prepared_content

    async def get_simple_intraday_group(self, group_name: str, delay: Optional[int] = None) -> List:
        path = f"/simple/intraday/group/{group_name}"
        if delay is not None:
            path += f"/{delay}"
        return await self._get_data(path)

    async def get_simple_intraday_ticker(self, tickerid: int, delay: Optional[int] = None) -> List:
        path = f"/simple/intraday/ticker/{tickerid}"
        if delay is not None:
            path += f"/{delay}"
        return await self._get_data(path)

    async def get_ticker_meta(self, tickerid: int) -> List:
        return await self._get_data(f"/meta/ticker/{tickerid}")

    async def get_group_meta(self, group_name: str) -> List:
        return await self._get_data(f"/meta/group/{group_name}")

...
class FOFDataManager(metaclass=Singleton):
    ...
    def _gather_asset_info_of_fof(self, fund_list):
        fof_aa = FOFDataManager.get_fof_asset_allocation([FOFDataManager._FOF_ID])
        hedge_nav = FOFDataManager.get_hedge_fund_nav(list(fund_list))
        for fund_id in fund_list:
            try:
                ...
                # 这里不需要减去计提业绩报酬调减的份额
                avg_cost = aa_info.amount.sum() / aa_info.unit_total.sum()
                nav = fund_nav.net_asset_value.array[-1]
                mv = (aa_info.unit_total.sum() - aa_info.share.sum()) * nav
                # TODO 这里需要考虑正常赎回的情况
                total_ret = mv / aa_info.amount.sum() - 1

                # TODO 这里应该用复权净值
                fund_nav = fund_nav.set_index('datetime')['acc_unit_value']
                res_status = Calculator.get_stat_result(fund_nav.index, fund_nav.array)
                info_since_inception = {
                    'start_date': res_status.start_date.isoformat(),
                    'days': (fund_nav.index.array[-1] - res_status.start_date).days,
                    'total_ret': f'{round(((fund_nav.array[-1] / fund_nav.array[0]) - 1) * 100, 2)}%',
                ...
                fund_nav = fund_nav[fund_nav.index >= aa_info.sort_values(by='datetime').datetime.array[0]]
                res_status = Calculator.get_stat_result(fund_nav.index, fund_nav.array)
                info_since_hold = {
                    'start_date': res_status.start_date.isoformat(),
                    'days': (fund_nav.index.array[-1] - res_status.start_date).days,
                ...
                self._wechat_bot.send_hedge_fund_info(self._FOF_ID, fund_id, nav_df.at[fund_id, 'datetime'], avg_cost, nav, mv, total_ret, info_since_inception, info_since_hold)
    ...
    def _gather_fof_info(self):
        try:
            ...
            self._wechat_bot.send_fof_info(FOFDataManager._FOF_ID, date, nav, mv, total_ret, info_since_inception)
        except Exception as e:
            traceback.print_exc()
    ...
    def service_start(self):
        ...
        if nav_df is not None:
            self._gather_asset_info_of_fof(nav_df.fund_id.unique())
        try:
            self.calc_nav()
    ...
    def virtual_backtest(hedge_fund_ids: Dict[str, float], start_date: datetime.date, end_date: Optional[datetime.date] = None, benchmark_ids: Tuple[str] = ()) -> Optional[Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]]:
        ...
        try:
            ...
            # 计算虚拟净值
            FOFDataManager._adj_nav = adj_nav.copy()
            ...
            if benchmark_ids:
                ...
                fof_with_benchmark = []
            fof_with_benchmark.append(fof_nav)
            ...
            if len(fof_with_benchmark) > 1:
                ...
                fof_with_benchmarks = fof_with_benchmarks[fof_with_benchmarks.fof_nav.notna()]

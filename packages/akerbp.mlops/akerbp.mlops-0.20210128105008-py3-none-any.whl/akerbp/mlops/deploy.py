...
def run_tests(test_path):
    ...
    if os.sep not in test_path:
        command.append("--pyargs")
    command.append(test_path)
    logging.info("Run tests: {test_path}")
...
def deploy(model_settings):
    ...
    for setting in model_settings:
        ...
        # Important to run tests after downloading model artifacts, in case
        # they're used to test prediction service
        if model_test_file:
            logging.info(f"Run service tests")
            run_tests('akerbp.mlops.tests')
        else:
            logging.warning("Model test file is missing! " \
                            "Didn't run service tests")

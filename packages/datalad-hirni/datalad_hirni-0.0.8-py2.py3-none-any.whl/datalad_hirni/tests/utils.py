...
@cached_url(url="https://github.com/psychoinformatics-de/hirni-demo")
def install_demo_dataset(ds, path, source, recursive=False):

    toolbox_overwrite = ds.config.get("datalad.hirni.toolbox.url")

    if toolbox_overwrite:
        ...
        subds.repo.call_git(["config", "-f", ".gitmodules", "--replace-all",
                             ...
                             toolbox_overwrite])
        subds.save(".gitmodules", message="Rewrite toolbox URL for hirni tests")

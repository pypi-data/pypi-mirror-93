...
def train(session):
    ...
    print(dir_path)
    xs = np.loadtxt(dir_path+"/data/"+model)
    n_train = xs.shape[0]
    ...
    for sigmal in sigmals:
        ...
        torch.save(den_net.state_dict(), dir_path+"/models/%.3f"%sigmal)
    ...
    plt.savefig(dir_path+'/../static/images/ERD.png',bbox_inches="tight")
    plt.clf()
    # Neff histogram
    den_nets = []
    for j in range(len(sigmals)):
        sigmal = sigmals[j]
        den_net = den()
        den_net.load_state_dict(torch.load(dir_path+"/models/%.3f"%sigmal))
    ...
    plt.savefig(dir_path+'/../static/images/Neff.png',bbox_inches="tight")
    plt.clf()

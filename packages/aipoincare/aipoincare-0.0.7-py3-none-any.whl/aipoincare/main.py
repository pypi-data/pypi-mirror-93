...
def home():
	...
	return render_template(dir_path+'/templates/home.html', session=session)
...
def get_parameter():
	...
	if change_mode == "Save":
		...
		save = True
		dir_path = os.path.dirname(os.path.abspath(__file__))
		return render_template(dir_path+'/templates/home.html', session=session, save=save)
	else:
		initialize()
		save = False
		dir_path = os.path.dirname(os.path.abspath(__file__))
...
def training():
	...
	update = True
	dir_path = os.path.dirname(os.path.abspath(__file__))
	return render_template(dir_path+'/templates/home.html', session=session, update=update, neff=neff, remove_dim=remove_dim, confidence=confidence)

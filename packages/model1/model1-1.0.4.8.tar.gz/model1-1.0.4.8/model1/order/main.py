...
class OrderManager(BaseMain):
    ...
    def get(self, **params):
        ...
        role = db.session.query(Order).filter(Order.id == params.get("id")).one()
        if role:
    ...
    def delete(self, **params):
        ...
        _model = db.session.query(Order).filter(Order.id == params.get("id")).first()
        if _model:

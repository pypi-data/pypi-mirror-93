...
class ModuleNetwork():
    ...
    def discover(self, timeout=None, retries=None):
        ...
        timeout : Float
            ...
            giving up and closing the port.  Defaults to one second.
        retries : Int
            How many times the module will close and re-open the discovery
            port before giving up.  This is necessary if the module powers
            up before the controller, as the controller creates the discovery
            channel and the module attaches to it when it exists.

            Defaults to 0 if a timeout is specified (no retries), or
            ModuleNetwork.RETRY_FOREVER if no timeout is specified.
        """
        self.state = ModuleState.DISCOVERY
        if timeout is None:
            timeout = 1
            retries = retries or self.RETRY_FOREVER
        else:
        ...
        while self.state == ModuleState.DISCOVERY:
            ...
            if self._discovery_socket is None:
                LOG.warning(f"### CONNECTING DISCOVERY SOCKET")
            ...
            if (time.time() - start_time) > timeout:
                ...
                if retries == self.RETRY_FOREVER or retries > 0:
                    # We didn't connect, so maybe the controller isn't there;
                    # disconnect and try again in a bit.
                    LOG.warning(f"### STOP CONNECTION AND RESET DISCOVERY")
                    self.stop()
                    self.state = ModuleState.DISCOVERY
                    ...
                    LOG.warning(f"### TIMED OUT AND GIVING UP")
                    raise TimeoutError(f"Timed out waiting "
                                       f"for discovery.")
    ...
    def publish(self, topic, message):
        ...
        packet = asdict(cmd)
        # QOS 1 : At least once
        self._mqtt.publish(f"{self.root_topic}/{topic}",

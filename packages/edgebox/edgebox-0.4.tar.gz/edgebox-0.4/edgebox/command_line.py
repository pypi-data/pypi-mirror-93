...
def is_cloudlet_ready(mc):
    ...
    return info and info[0]["state"] == 5
...
def edgebox_github(args):
    ...
    # Template app config
    app_tmpl_file = os.path.join(os.path.dirname(__file__), "app.yml.tmpl")
    with open(app_tmpl_file) as f:
        app_tmpl = f.read()

    docker = "docker.mobiledgex.net" if mc.deploy_env == "main" \
        else "docker-{}.mobiledgex.net".format(mc.deploy_env)

    t = Template(app_tmpl)
    app_yml = t.substitute({
        "appname": app_name,
        "devorg": org,
        "devorg_lc": org.lower(),
        "docker": docker,
    ...
    app_yml_file = os.path.join(repo_conf_dir, "app.yml")
    if os.path.exists(app_yml_file):
        nfile = app_yml_file + ".new"
        print("File exists: {0}. Writing to {1}".format(
            app_yml_file, nfile))
        app_yml_file = nfile

    with open(app_yml_file, "w") as f:
        f.write(app_yml)

    # Template appinsts config
    appinsts_tmpl_file = os.path.join(os.path.dirname(__file__), "appinsts.yml.tmpl")
    with open(appinsts_tmpl_file) as f:
        appinsts_tmpl = f.read()

    t = Template(appinsts_tmpl)
    appinsts_yml = t.substitute({
        "cloudlet": mc.cloudlet,
        "cloudletorg": mc.cloudlet_org,
        "devorg": org,
    })

    appinsts_yml_file = os.path.join(repo_conf_dir, "appinsts.yml")
    if os.path.exists(appinsts_yml_file):
        nfile = appinsts_yml_file + ".new"
        print("File exists: {0}. Writing to {1}".format(
            appinsts_yml_file, nfile))
        appinsts_yml_file = nfile

    with open(appinsts_yml_file, "w") as f:
        f.write(appinsts_yml)

    # Template github workflow config
    workflow_tmpl_file = os.path.join(os.path.dirname(__file__), "workflow.tmpl")
    with open(workflow_tmpl_file) as f:
        workflow_tmpl = f.read()

    t = Template(workflow_tmpl)
    workflow = t.substitute({
        "appname": app_name,
    ...
    workflow_file = os.path.join(repo_github_dir, "main.yml")
    if os.path.exists(workflow_file):
        nfile = workflow_file + ".new"
        print("File exists: {0}. Writing to {1}".format(
            workflow_file, nfile))
        workflow_file = nfile
...
Add your console credentials as Github secrets:
- MOBILEDGEX_USERNAME
- MOBILEDGEX_PASSWORD

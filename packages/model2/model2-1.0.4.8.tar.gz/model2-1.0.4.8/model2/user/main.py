...
class UserManager(BaseMain):
    ...
    def get(self, **params):
        ...
        role = db.session.query(User).filter(User.id == params.get("id")).one()
        if role:
    ...
    def delete(self, **params):
        ...
        _model = db.session.query(User).filter(User.id == params.get("id")).first()
        if _model:

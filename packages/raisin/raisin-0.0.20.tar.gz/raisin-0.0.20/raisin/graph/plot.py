...
class Display:
    ...
    def _id2color(self, color_id):
        ...
        * Ne fait pas de verifications.
        ...
        if color_id == 5:
            return "#00ffff"
        return "#%06x" % (hash((color_id, )) % 4294967296)

    def _place(self):
        """
        S'assure que chaque noeud aient un couple de
    ...
    def _plot_segment(self, ax, segment, *, color=None, legend=True):
        ...
        * Ne fait pas de verifications.
        ...
        if type(segment) == mod_edge.Arrow:
            rot = np.matrix([[v[0, 0], -v[1, 0]],
                             [v[1, 0],  v[0, 0]]],
                             dtype=float)
            fleche1 = 0.01*np.matrix([[-np.sqrt(3)/2],
                                      [ 0.5         ]],
                                      dtype=float)
            fleche2 = 0.01*np.matrix([[-np.sqrt(3)/2],
                                      [-0.5         ]],
                                      dtype=float)
    ...
    def cross_density(self, mobile_vertex=None):
        ...
        * Seul les noeuds dons les coordonnees sont fixees sont pris en compte.
        ...
        segments = [s for s in self.get_edges() if 
        ...
        return sum(cost(e1, e2)
            for e1 in self.get_edges(mobile_vertex)
            for e2 in segments) / (len(segments)*self.degree(mobile_vertex))

...
class Properties:
    ...
    def is_complete(self):
        ...
        * Un graphe est dit complet si pour toutes paire de sommets (x, y),
        ...
        self.properties["is_complete"] = (
            ...
            + np.diag(np.ones(max(self.id2vertex) + 1))).all()
        return self.properties["is_complete"]
    ...
    def is_multigraph(self):
        ...
        * Un graphe est dit multigraph si un sommet est relie
        ...
        for node_id, (edges_id, _) in self.graph.items():
            nodes_id = [self.id2edgarr[edge_id].vd.n for edge_id in edges_id] # Tous les noeuds pointes.

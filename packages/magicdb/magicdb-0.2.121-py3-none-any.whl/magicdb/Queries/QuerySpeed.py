...
class QuerySpeed:
    ...
    def get_from_firestore(self, **kwargs) -> dict:
    ...
    def d_arr_from_redis(self, keys: List[str]) -> T.Optional[List[dict]]:
        if not self.r:
            return None
        print("keys", keys)
        ...
        for raw_str in raw_strs:
            element = None if raw_str is None else ujson.loads(raw_str)
            ds.append(element)
        return ds
    ...
    def model_mapping_from_redis(
        ...
        if not self.r:
            return None
        with safe_span(self.make_safe_span_label(f"*mget-redis*{len(keys)=}")):
            raw_strs = self.r.mget(keys)
        mapping = {}
    ...
    def d_arr_to_redis(self, arr: List[dict]) -> None:
        if not self.r:
            return None
        mapping = {}
        for d in arr:
            ...
            mapping[model.key] = model.json(exclude_unset=True)
        self.r.mset(mapping)
    ...
    def model_mapping_to_redis(self, mapping: T.Dict[str, dict]) -> None:
        ...
        with safe_span(self.make_safe_span_label("mset-redis-from-miss")):
            self.r.mset(json_mapping)
        if to_delete:
            with safe_span(self.make_safe_span_label("delete-redis-from-miss")):
    ...
    def d_to_redis(self, d: dict) -> None:
        ...
        with safe_span(self.make_safe_span_label("set-redis-from-miss")):
            self.r.set(self.key, j)

    def get_mapping_from_db(self, ids: List[str], **kwargs) -> T.Dict[str, dict]:
        ...
        # d_arr = self.d_arr_from_redis(keys=keys)
        if None in model_mapping.values():
    ...
    def get_d_from_db(self, **kwargs) -> T.Optional[dict]:
        d = self.d_from_redis()
        if not d:
    ...
    def get_internal(self, create=False, validate_db_data=True, **kwargs):
        d = self.get_d_from_db(**kwargs)
    ...
    def get(self, id=None, create=False, validate_db_data=True, **kwargs):
        ...
        return self.get_internal(
            create=create, validate_db_data=validate_db_data, **kwargs
        )

    @magic_async
    def get_all(self, ids: List[str], validate_db_data=True, **kwargs) -> list:
        mapping = self.get_mapping_from_db(ids=ids, **kwargs)

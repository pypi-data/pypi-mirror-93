...
class HourCounter(IHourCounter):
    ...
    def history(self):
        if not self.__keep_history:
            return None
        return [((key - self.__initial_time) / 3600, self.__history[key]) for key in sorted(self.__history.keys(), key=lambda x: x)]
    ...
    def observe_count(self, count, clock_time=None):
        if clock_time is None:
            ...
            if clock_time != self.__sandbox.clock_time:
                raise Exception("Clocktime is not consistent with the Sandbox.")
    ...
    def __sort_hours_for_count(self):
        self.hours_for_count = {key: self.hours_for_count[key] for key in sorted(self.hours_for_count.keys(), key=lambda x: x)}

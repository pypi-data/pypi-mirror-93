...
def write_version_py(filename='quantum_volume/version.py'):
    cnt = """\
# THIS FILE IS GENERATED FROM quantum_volume SETUP.PY
...
sys.path.insert(0, os.path.join(local_path, 'quantum_volume'))  # to retrive _version

# always rewrite _version
if os.path.exists('quantum_volume/version.py'):
    os.remove('quantum_volume/version.py')
...
class PylintCommand(setuptools.Command):
    """Run Pylint on all quantum_volume Python source files."""
    description = 'Run Pylint on quantum_volume Python source files'
    user_options = [
    ...
    def run(self):
        ...
        command.append(os.getcwd()+"/quantum_volume")
...
class StyleCommand(setuptools.Command):
    ...
    def run(self):
        """Run command."""
        command = 'pycodestyle --max-line-length=100 quantum_volume'

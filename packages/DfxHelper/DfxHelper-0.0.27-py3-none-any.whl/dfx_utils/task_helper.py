...
class TaskSetting:
    def __init__(self, setting: Dict, handlers: Dict):
        """
...
class WorkerQueue(asyncio.Queue):
    ...
    async def __deal_job(self, name):
        ...
        handle_name = f'handle_{name}'
        handler = self.setting.get(handle_name)
    ...
    async def __deal_queue(self):
        ...
        while self.running:
            ...
            if setting_dict.get(f'handle_{job.name}') and self.queues.get(job.name):
...
if __name__ == '__main__':
    ...
    handlers = {'handle_test': test, 'handle_test1': test}
    task_setting = TaskSetting({}, handlers)
    worker = WorkerQueue(['test', 'test1'], task_setting)

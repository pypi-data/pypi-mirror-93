...
class BaseAuthTestCase(BaseZMQTestCase):
    ...
    def create_certs(self):
        ...
        server_public_file, server_secret_file = appdynamics_bindeps.zmq.auth.create_certificates(keys_dir, "server")
        client_public_file, client_secret_file = appdynamics_bindeps.zmq.auth.create_certificates(keys_dir, "client")
    ...
    def load_certs(self, secret_keys_dir):
        ...
        server_public, server_secret = appdynamics_bindeps.zmq.auth.load_certificate(server_secret_file)
        client_public, client_secret = appdynamics_bindeps.zmq.auth.load_certificate(client_secret_file)
...
class TestThreadAuthentication(BaseAuthTestCase):
    ...
    def test_curve(self):
        ...
        self.auth.configure_curve(domain='*', location=appdynamics_bindeps.zmq.auth.CURVE_ALLOW_ANY)
        server = self.socket(zmq.PUSH)
        server.curve_publickey = server_public
        server.curve_secretkey = server_secret
        server.curve_server = True

...
from appdynamics_bindeps.zmq.backend import Socket as SocketBase
from .poll import Poller
from .attrsettr import AttributeSetter
from .version import zmq_version, zmq_version_info
from appdynamics_bindeps.zmq.error import ZMQError, ZMQBindError
from appdynamics_bindeps.zmq.utils import jsonapi
from appdynamics_bindeps.zmq.utils.strtypes import bytes, unicode, basestring
from appdynamics_bindeps.zmq.utils.interop import cast_int_addr

try:
    from .constants import (SNDHWM, RCVHWM)
except ImportError:
    pass

try:
    from .constants import EVENT_ALL
except ImportError:
    pass

from .constants import (
    SNDMORE, RCVMORE, ENOTSUP, POLLIN, EADDRINUSE,
    HWM, PAIR,
    int64_sockopt_names,
    int_sockopt_names,
    bytes_sockopt_names,
...
class Socket(SocketBase, AttributeSetter):
    ...
    def set_hwm(self, value):
        ...
        if major >= 3:
            ...
            return self.setsockopt(HWM, value)
    ...
    def get_monitor_socket(self, events=None, addr=None):
        ...
        if zmq_version_info() < (4,):
            raise NotImplementedError("get_monitor_socket requires libzmq >= 4, have %s" % zmq_version())
        if addr is None:

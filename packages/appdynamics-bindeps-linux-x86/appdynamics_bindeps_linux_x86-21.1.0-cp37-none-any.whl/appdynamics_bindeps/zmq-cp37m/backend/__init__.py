...
from appdynamics_bindeps.zmq.utils.sixcerpt import reraise
...
if 'PYZMQ_BACKEND' in os.environ:
    ...
    if backend in ('cython', 'cffi'):
        backend = 'appdynamics_bindeps.zmq.backend.%s' % backend
    _ns = select_backend(backend)
else:
    # default to cython, fallback to cffi
    # (reverse on PyPy)
    if platform.python_implementation() == 'PyPy':
        first, second = ('appdynamics_bindeps.zmq.backend.cffi', 'appdynamics_bindeps.zmq.backend.cython')
    else:
        first, second = ('appdynamics_bindeps.zmq.backend.cython', 'appdynamics_bindeps.zmq.backend.cffi')

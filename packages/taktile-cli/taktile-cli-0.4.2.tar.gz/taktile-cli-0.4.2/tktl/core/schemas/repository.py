...
from tktl.core.t import AccessKind, ComputeTypeT, InstanceSizeT
...
class RepositoryDeployment(BaseModel, TablePrintableBaseModelMixin):
    ...
    def table_repr(self, subset=None):
        ...
        as_dict[
            "REST INSTANCE"
        ] = f"{as_dict.pop('rest_instance_type').value}.{as_dict.pop('rest_instance_size').value}"
        as_dict[
            "ARROW INSTANCE"
        ] = f"{as_dict.pop('arrow_instance_type').value}.{as_dict.pop('arrow_instance_size').value}"

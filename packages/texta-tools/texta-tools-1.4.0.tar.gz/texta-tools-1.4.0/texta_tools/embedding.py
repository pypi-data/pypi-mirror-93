...
class FastTextEmbedding:
    ...
    def get_similar(self, positives_used, negatives_used=list(), positives_unused=list(), negatives_unused=list(), n=20):
        ...
        positives = [positive.replace(' ', '_') for positive in positives_used]
        negatives = [negative.replace(' ', '_') for negative in negatives_used]
        ...
        not_a_suggestion = [not_suggestion.replace(' ', '_') for not_suggestion in negatives_used + positives_unused + negatives_unused if not_suggestion.replace(' ', '_') in self.model.wv.vocab]
        ...
        if positives:
            ...
            for s in similar_items:
                if s[0] not in not_a_suggestion:
                    similar_items_topn += [{'phrase': s[0].replace('_', ' '), 'score': s[1], 'model': self.name}]
                    i += 1
                if i == n:
                    break

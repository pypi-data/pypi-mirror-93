...
class EventBus(object):
    ...
    def __init__(self, mainloop=None, debug=None):
    ...
    def mainloop(self, value):
        if value is None:
            value = MainLoop()
        if not isinstance(value, MainLoop):
            raise TypeError("'mainloop' property value must be a Mainloop Instance or None")
        if value != self.mainloop:
            self.__mainloop = value

    def connect(self, detailed_signal: object, handler: object, *args: object) -> object:
        """
    ...
    def emit(self, detailed_signal, args):
        ...
        :type args: dict
        """
        self.mainloop.event_list.add(detailed_signal, args)

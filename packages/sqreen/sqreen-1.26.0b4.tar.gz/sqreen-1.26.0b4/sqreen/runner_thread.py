...
def is_serverless():
    ...
    if CONFIG["NO_SERVERLESS_DETECTION"]:
        return False
    try:
        return os.getenv("LAMBDA_TASK_ROOT") is not None and os.path.exists("/opt/extensions/sqreen-aws-extension")
    except Exception:
        return False
...
def start():
    ...
    if RUNNER_THREAD is None:
        try:
            ...
            timeout = min(CONFIG["INSTRUMENTATION_TIMEOUT"], CONFIG["SERVERLESS_INSTRUMENTATION_TIMEOUT"])

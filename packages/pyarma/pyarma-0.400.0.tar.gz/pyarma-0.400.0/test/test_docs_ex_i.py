...
# Operations
A = imat(5, 10, fill_randu)
B = imat(5, 10, fill_randu)
C = imat(10, 5, fill_randu)
...
# broadcasting
X = imat(6, 5, fill_ones)
v = imat(6, 1, fill_randu)
...
# submatrix
A = imat(5, 10, fill_zeros)

A[ 0:2, 1:3 ]             = imat(3,3, fill_randu)
# A[ span(0,2), span(1,3) ] = imat(3,3, fill_randu)
# A[ 0,1, size(3,3) ]       = imat(3,3, fill_randu)

B = A[ 0:2, 1:3 ].eval()
# C = A[ span(0,2), span(1,3) ].eval()
# D = A[ 0,1, size(3,3) ].eval()

A[:,1]          = imat(5,1,fill_randu)
# A[span_all, 1]  = imat(5,1,fill_randu)

X = imat(5, 5, fill_randu)
...
X[diag] = imat(5,1,fill_randu)
X[diag] += 6
X[diag].ones()

# set_real, set_imag
A = imat(4, 5, fill_randu)
B = imat(4, 5, fill_randu)
...
# swap
A = imat(4, 5, fill_zeros)
B = imat(6, 7, fill_ones )

A.swap(B)

# iterators
X = imat(5, 6, fill_randu)
...
X = imat(100, 200, fill_randu)

for val in X[40:60, 50:100]:
  print(val)

# as_col
X = imat(4, 5, fill_randu)
v = X.as_col()
...
#is_vec
A = imat(1, 5, fill_randu)
B = imat(5, 1, fill_randu)
C = imat(5, 5, fill_randu)
...
#as_scalar
r = imat(1, 5, fill_randu)
q = imat(5, 1, fill_randu)
...
#cross
a = imat(3, 1, fill_randu)
b = imat(3, 1, fill_randu)
...
#join
A = imat(4, 5, fill_randu)
B = imat(4, 6, fill_randu)
C = imat(6, 5, fill_randu)
...
X = imat(5, 6, fill_randu)
Y = imat(5, 6, fill_randu)
Z = max(X,Y)

#nonzeros
B = imat(100, 100, fill_eye)
b = nonzeros(B)
...
B = imat(size(A), fill_zeros)

C = imat()

C.randu(size(A))

D = imat(10,20, fill_ones)
...
#sigproc
#conv
A = imat(256, 1, fill_randu)

B = imat(16, 1, fill_randu)
...
#conv2
A = imat(256, 256, fill_randu)

B = imat(16, 16, fill_randu)
...
#fft
X = imat(100, 1, fill_randu)
   
Y = fft(X, 128)

#fft2
A = imat(100, 100, fill_randu)
...
# #hist
# v = imat(1000, 1, fill_randn) # Gaussian distribution
...
# #normpdf
# X = imat(10, 1, fill_randu)
# M = imat(10, 1, fill_randu)
# S = imat(10, 1, fill_randu)

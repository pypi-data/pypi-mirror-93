...
# cx_fmat class
A = cx_fmat(5, 5, fill_randu)
x = A[1,2]

M = umat(5, 5, fill_ones) # initialise 5x5 matrix with ones
...
# Operations
A = cx_fmat(5, 10, fill_randu)
B = cx_fmat(5, 10, fill_randu)
C = cx_fmat(10, 5, fill_randu)
...
# broadcasting
X = cx_fmat(6, 5, fill_ones)
v = cx_fmat(6, 1, fill_randu)
...
# submatrix
A = cx_fmat(5, 10, fill_zeros)

A[ 0:2, 1:3 ]             = cx_fmat(3,3, fill_randu)
# A[ span(0,2), span(1,3) ] = cx_fmat(3,3, fill_randu)
# A[ 0,1, size(3,3) ]       = cx_fmat(3,3, fill_randu)

B = A[ 0:2, 1:3 ].eval()
# C = A[ span(0,2), span(1,3) ].eval()
# D = A[ 0,1, size(3,3) ].eval()

A[:,1]          = cx_fmat(5,1,fill_randu)
# A[span_all, 1]  = cx_fmat(5,1,fill_randu)

X = cx_fmat(5, 5, fill_randu)
...
X[indices] = cx_fmat(4,1,fill_ones)
...
X[diag] = cx_fmat(5,1,fill_randu)
# TODO: FIX THIS
# X[diag] += 6
X[diag].ones()

# set_real, set_imag
A = fmat(4, 5, fill_randu)
B = fmat(4, 5, fill_randu)
...
# fast cx_mat
A = cx_fmat(4, 5, fill_randu)
B = cx_fmat(4, 5, fill_randu)

# C = cx_mat(A,B)

# ins_row/cols
A = cx_fmat(5, 10, fill_randu)
B = cx_fmat(5,  2, fill_ones )
...
# swap
A = cx_fmat(4, 5, fill_zeros)
B = cx_fmat(6, 7, fill_ones )

A.swap(B)

# iterators
X = cx_fmat(5, 6, fill_randu)
...
X = cx_fmat(100, 200, fill_randu)

for val in X[40:60, 50:100]:
  print(val)

# as_col
X = cx_fmat(4, 5, fill_randu)
v = X.as_col()
...
#is_vec
A = cx_fmat(1, 5, fill_randu)
B = cx_fmat(5, 1, fill_randu)
C = cx_fmat(5, 5, fill_randu)
...
#arg
A = cx_mat(5, 5, fill_randu)
B = arg(A)

#as_scalar
r = cx_fmat(1, 5, fill_randu)
q = cx_fmat(5, 1, fill_randu)
...
#cross
a = cx_fmat(3, 1, fill_randu)
b = cx_fmat(3, 1, fill_randu)
...
#imag/real
C = cx_mat(5, 5, fill_randu)

A = imag(C)
B = real(C)

#ind2sub
M = cx_fmat(4, 5, fill_randu)
...
#join
A = cx_fmat(4, 5, fill_randu)
B = cx_fmat(4, 6, fill_randu)
C = cx_fmat(6, 5, fill_randu)
...
X = cx_fmat(5, 6, fill_randu)
Y = cx_fmat(5, 6, fill_randu)
Z = max(X,Y)

#nonzeros
B = cx_fmat(100, 100, fill_eye)
b = nonzeros(B)
...
B = cx_fmat(size(A), fill_zeros)

C = cx_fmat()

C.randu(size(A))

D = cx_fmat(10,20, fill_ones)
...
#sigproc
#conv
A = cx_fmat(256, 1, fill_randu)

B = cx_fmat(16, 1, fill_randu)
...
#conv2
A = cx_fmat(256, 256, fill_randu)

B = cx_fmat(16, 16, fill_randu)
...
#fft
X = cx_fmat(100, 1, fill_randu)
   
Y = fft(X, 128)

#fft2
A = cx_fmat(100, 100, fill_randu)
...
# #hist
# v = cx_fmat(1000, 1, fill_randn) # Gaussian distribution
...
# #normpdf
# X = cx_fmat(10, 1, fill_randu)
# M = cx_fmat(10, 1, fill_randu)
# S = cx_fmat(10, 1, fill_randu)

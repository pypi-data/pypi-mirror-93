...
def test_expr_elem_1():
    ...
    assert math.isclose(pa.accu(pa.abs(A - A[pa.span_all, pa.span_all])), 0.0) == True

    assert math.isclose(pa.accu(pa.abs((A[pa.span_all,pa.span_all]+2) - A_plus_2)), 0.0, abs_tol=0.0001) == True
    assert math.isclose(pa.accu(pa.abs((2+A[pa.span_all,pa.span_all]) - A_plus_2)), 0.0, abs_tol=0.0001) == True

    # AttributeError: module 'pyarma' has no attribute 'zeros'
    # assert math.isclose(pa.accu(pa.abs((A+A+A) / (3*A)) - pa.ones(pa.size(A))), 0.0) == True

    # AttributeError: module 'pyarma' has no attribute 'randu'
    # assert math.isclose(pa.accu(pa.abs(A + pa.randu(pa.mat(A.n_rows+1, A.n_cols)))), 0.0) == True
    # assert math.isclose(pa.accu(pa.abs(A + pa.randu(pa.mat(A.n_rows, A.n_cols+1)))), 0.0) == True
...
def tsetting(debug = False, realtimeOutput = False, decimalSpaces = 2, colorfulOutput = True):
    global tRealtimeOutput, tDebug, tDecimalSpaces, tColourfulOutput
    f = inspect.currentframe()
    i = inspect.getframeinfo(f.f_back)
    log = '\n'

    if isinstance(debug, bool) == False:
        log += ('''ValueError: Failed to call function "tsetting" on line ''' + str(i.lineno) + ''': "debug" must be bool\n''')
    if isinstance(realtimeOutput, bool) == False:
        log += ('''ValueError: Failed to call function "tsetting" on line ''' + str(i.lineno) + ''': "realtimeOutput" must be bool\n''')
    if isinstance(decimalSpaces, int) == False:
        log += ('''ValueError: Failed to call function "tsetting" on line ''' + str(i.lineno) + ''': "decimalSpaces" must be integer\n''')
    if isinstance(tColourfulOutput, bool) == False:
        log += ('''ValueError: Failed to call function "tsetting" on line ''' + str(i.lineno) + ''': "colourfulOutput" must be bool\n''')
...
def __sPoint__(key, f):
    ...
    if tRealtimeOutput == False:
        ...
        if dictionaryEnd.get(key) == None:
            ...
            if tDebug == False:
                print(('''Time required for key " ''' + color + '''%s\033[0;0m" is:   '''%str(key) + color + str(rTime)) + ' \033[0;0mseconds')
            else:
                print(('''Time required for key "''' + color + '''%s\033[0;0m" is:   '''%str(key) + color + str(rTime)) + ' \033[0;0mseconds      Start line: ' + str(dictionaryCallStart.get(str(key))) + '      End line: ' +  str(dictionaryCallEnd.get(str(key))))
...
def __check__(debug, decimalSpaces, realtimeOutput):
    ...
    if keysStart != keysEnd:
        ...
        for i in keysStart:
            ...
            if debug == False:
                log += ('''Time required for key "''' + color + '''%s\033[0;0m" is:'''%str(i) + ' ' * (maximalKeyLength - len(i) + 3) + color + str(rTime)) + ' \033[0;0mseconds \n'
            else:
                log += ('''Time required for key "''' + color + '''%s\033[0;0m" is:   '''%str(i) + color + str(rTime)) + ' \033[0;0mseconds      Start line: ' + str(dictionaryCallStart.get(str(i))) + '      End line: ' +  str(dictionaryCallStart.get(str(i))) + '\n'
        print(log)

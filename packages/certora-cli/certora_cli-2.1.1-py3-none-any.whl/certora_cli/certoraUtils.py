...
verificationErrorMsgPrefix = BASH_RED_COLOR + "Prover found violations:" + BASH_END_COLOR
...
# completion messages
noVerificationErrors = BASH_GREEN_COLOR + "No errors found by Prover!" + BASH_END_COLOR
...
def check_results(actual: Dict[str, Any]) -> bool:
    ...
    if based_on_expected:  # compare actual results with expected
        ...
        if error_str:
            print(verificationErrorMsgPrefix, error_str)
        if has_violations():

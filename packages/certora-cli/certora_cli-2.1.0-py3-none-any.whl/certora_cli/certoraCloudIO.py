...
from certora_cli.certoraUtils import BASH_COLOR_ORANGE, BASH_END_COLOR, BASH_RED_COLOR, BASH_GREEN_COLOR
...
# error messages
TimeoutMsgPrefix = BASH_RED_COLOR + "Request timed out." + BASH_END_COLOR
ErrorStatusPrefix = BASH_RED_COLOR + "Error Status:" + BASH_END_COLOR
ConnectionErrorPrefix = BASH_RED_COLOR + "Connection error:" + BASH_END_COLOR
VaasErrorPrefix = BASH_RED_COLOR + "Server reported an error:" + BASH_END_COLOR
ServerErrorPrefix = BASH_RED_COLOR + "Server Error" + BASH_END_COLOR
WarningMsgPrefix = BASH_COLOR_ORANGE + "Warning:" + BASH_END_COLOR
ErrorMsgPrefix = BASH_RED_COLOR + "An error occurred:" + BASH_END_COLOR


# completion messages
completeCompress = BASH_GREEN_COLOR + "Finished compressing" + BASH_END_COLOR
completeSubmit = BASH_GREEN_COLOR + "Job submitted to server" + BASH_END_COLOR
    ...
    try:
        ...
        if "." in latest and "." in version:
            ...
            if int(main) > int(current_main):  # raises ValueError: invalid literal for int() with base 10
                ...
                print_version_warning(version, latest)
    except (requests.exceptions.RequestException, ValueError) as e:
        debug_print_(str(e))
    return True


def print_version_warning(current: str, latest: str) -> None:
    print(WarningMsgPrefix, f"You are using certora-cli {current}; however, version {latest} is available.")
    ...
    for file_name in file_names:
        if os.path.isdir(file_name):
            ...
            if total_dir_files == 0:
                print(ErrorMsgPrefix, f"Provided directory - '{file_name}' is empty.")
                return False
            ...
            print(ErrorMsgPrefix, f"Provided file - '{file_name}' does not exist.")
            return False
    if total_files < 1:
        if len(file_names) == 0:
            print(ErrorMsgPrefix, "No file was provided. Please, contact Certora on https://www.certora.com")
        else:
            print(ErrorMsgPrefix, f"Provided file(s) - {', '.join(file_names)} do(es) not exist.")
        return False
    ...
    for file_name in file_names:
        if os.path.isdir(file_name):
            try:
                ...
                print(ErrorMsgPrefix, "Could not compress a directory -", file_name)
                return False
        else:  # zip file
            try:
                ...
                print(ErrorMsgPrefix, "Could not compress", file_name)
                return False
...
def output_error_response(response: Response) -> None:
    ...
    try:
        ...
        if "errorString" in error_response:
            print(VaasErrorPrefix, error_response["errorString"])
        elif "message" in error_response:
            print(VaasErrorPrefix, error_response["message"])
...
def is_success_response(json_response: Dict[str, Any], status_url: str = "") -> bool:
    ...
    if "success" not in json_response:
        print(ErrorMsgPrefix, "The server returned an unexpected response:")
        print(json_response)
        print("Please contact Certora on https://www.certora.com")
        return False
    if not json_response["success"]:
        if "errorString" in json_response:
            print_error_message(json_response["errorString"], status_url)
        else:
            print(ErrorMsgPrefix, "The server returned an error with no message:")
            print(json_response)
            print("Please contact Certora on https://www.certora.com")
        return False
    return True
...
def print_conn_error() -> None:
    print(ConnectionErrorPrefix, "Server is currently unavailable. Please try again later.")
    print("For further information, please contact Certora on https://www.certora.com", flush=True)


# TODO: update error message - email notification (?)
def print_error_message(err_msg: str, status_url: str) -> None:
    print(ErrorMsgPrefix, err_msg)
    if status_url:
...
class CloudVerification:
    ...
    def cli_verify(self, args: argparse.Namespace, arg_str: str, debug: bool = False) -> bool:
        ...
        @param compare_to_expected: If true, compares the expected output with the actual results.
        ...
        try:
            ...
            print(ErrorMsgPrefix, "Unexpected response", e)
            return False
        ...
        # create new zip file
        if not compress_files(self.ZipFileName, CERTORA_BUILD_FILE, CERTORA_VERIFY_FILE, CERTORA_CONFIG_DIR):
            return False
            ...
            try:
                ...
                if r.status_code != requests.codes.ok:
                    ...
                    if not json_response:  # Error parsing json
                        print(ErrorMsgPrefix, "Failed to parse response. For more information visit", status_url)
                        break
                    ...
                    try:
                        ...
                        error_msg = "No status"
                        print_error_message(error_msg, status_url)
                        break
                    try:
                    ...
                    try:
                        ...
                        error_msg = "No output is available."
                        print_error_message(error_msg, status_url)
                        break
                ...
                if all_output:
                    ...
                    if curr_miss_out - first_miss_out > self.queue_wait_minutes * 60:  # more than N min
                        error_msg = "There was no output for {} minutes.".format(self.queue_wait_minutes)
                        print_error_message(error_msg, status_url)
                        raise TimeError()

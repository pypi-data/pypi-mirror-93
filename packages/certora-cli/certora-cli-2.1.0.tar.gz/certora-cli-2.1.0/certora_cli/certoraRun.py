...
from certora_cli.certoraUtils import get_version
...
def run_cmd(cmd: str, override_exit_code: bool, custom_error_message: Optional[str] = None) -> None:
    ...
    try:
        ...
        if exitcode:
            ...
            default_msg = f"Failed to run {' '.join(args)}, exitcode {exitcode}"
...
def run_local_type_check() -> None:
    ...
    # args to typechecker
    build_file = CERTORA_BUILD_FILE
    verify_file = CERTORA_VERIFY_FILE
...
def main_with_args(args: List[str], is_library: bool = False) -> None:
    ...
    try:
        ...
        if parsed_args.mode != Mode.TAC:
            ...
            main_with_args(parsed_args, is_library)

        if parsed_args.build_only:
            exit_if_not_library(0)
        else:
            certora_root_dir = sanitize_path(get_certora_root_directory())
            default_path = f"{certora_root_dir}/emv.jar"

            if parsed_args.jar is not None or \
                    (os.path.isfile(default_path) and parsed_args.staging is None and parsed_args.cloud is None):
                ...
                if parsed_args.disableLocalTypeChecking:
                    ...
                    print("local type checking finished successfully")

                cv = CloudVerification(parsed_args)
                result = cv.cli_verify(parsed_args, args_to_str(parsed_args), debug=DEBUG)
                if result:
                    exit_if_not_library(0)
                else:
                    exit_if_not_library(1)
        ...
        print("Encountered an error running Certora Prover:", e,
...
def type_tool_output_path(filename: str) -> str:
    ...
    if os.path.isfile(filename):
        warning_print(f"--toolOutPutpath {filename} file already exists")
...
def type_input_file(file: str) -> str:
    ...
    if '.sol' in file:
        if not re.search(r'^.+\.sol(:[^.:]+)?$', file):
            raise argparse.ArgumentTypeError(f"Bad input file format of {file}. Expected <contract_path>:<alias>")

        if ':' in file:
            if is_windows():  # We might have a path with : like C:\Users...
                if file.count(':') == 2:  # : for both drive and an alias
                    file_path, alias = file.rsplit(':', 1)
                elif re.search(r'^[A-Z]:\\', file):  # : just for drive
                    file_path = file
                    alias = os.path.basename(file).replace('.sol', '')
                else:  # A single : for an alias
            ...
            if not re.search(r'^\w+$', alias):
                raise argparse.ArgumentTypeError(
                    f"file alias {alias} can contain only alphanumeric characters or underscores")
        else:
            file_path = file
...
def type_alias(contract: str) -> str:
    if not re.match(r'^\w+$', contract):
        raise argparse.ArgumentTypeError(
            f"contract alias {contract} can include only alphanumeric characters or underscores")
    return contract
...
def _get_natural_alias(contract: str) -> str:
    ...
    contract = os.path.basename(contract)
...
def warn_verify_file_args(files: List[str]) -> Tuple[Set[str], Set[str], Dict[str, str], Dict[str, str]]:
    ...
    @return: (contracts, files, contract_to_file, file_to_contract)
        ...
        file_to_contract - a mapping from a file path -> name of a contract within it we verify
    """
    ...
    if len(files) == 1 and (files[0].endswith(".conf") or files[0].endswith(".tac")):
        return set(), set(), dict(), dict()  # No legal file aliases
    ...
    for f in files:
        if ':' in f:
            # Can be either for alias declaration or because we have a drive in windows path e.g. C:\Users...

            warn_alias = True  # Only warn if the : marked an alias and not a drive in path

            if is_windows():  # colon might be for an alias or for a drive in path or both
                num_colons = f.count(':')
                assert num_colons <= 2
                if num_colons == 2:  # We have two colons: one for an alias, one for a drive in path
                    filepath, contract_name = f.rsplit(':', 1)
                else:
                    if re.search(r'^[A-Z]:\\', f):  # colon is for a drive in path. No alias
                        filepath = f
                        contract_name = _get_natural_alias(filepath)
                        warn_alias = False
                    else:  # colon was for an alias. No drive in path
        ...
        if filepath in file_to_contract:
            if contract_name != file_to_contract[filepath]:
                raise argparse.ArgumentTypeError(f"file {filepath} was given two different aliases: "
...
def check_contract_name_arg_inputs(args: argparse.Namespace) -> None:
    ...
    """
    contract_aliases, file_paths, contract_to_file, file_to_contract = warn_verify_file_args(args.files)
    args.contracts = contract_aliases
...
def check_packages_arguments(args: argparse.Namespace) -> None:
    ...
    if args.packages is not None and len(args.packages) > 0:
        ...
        if not os.path.exists(PACKAGE_FILE):
            warning_print(f"Default package file {PACKAGE_FILE} not found")
        elif not os.access(PACKAGE_FILE, os.R_OK):
            warning_print(f"No read permissions for default package file {PACKAGE_FILE} not found")
        else:
            try:
                ...
                warning_print(f"Failed in processing {PACKAGE_FILE}: {ex_type}, {ex_value}")
...
def check_args_post_argparse(args: argparse.Namespace) -> None:
    ...
    # URI - consider changing this in certoraBuild BuildInput()
    if args.java_args is not None:
...
def args_to_str(args: argparse.Namespace) -> str:
    """
    Gets an argparse objects and outputs a string that would have generated it.
    This is used as a necessary evil as we must pass the resulting string in the HTTP request to the cloud.
    This is the opposite action to argparse.parse_args().
    Why do we need it and not pass argv?
    1. We have some argument defaults in args and not in argv
    2. This approach would not work with a TAC file
    ...
    for arg, val in args.__dict__.items():
        if val is not None and val is not False:
            if arg == "struct_link":
                build_arg_list.append("--structLink")
            elif arg == "tool_output":
                build_arg_list.append("--toolOutput")
            elif arg == "java_args":
                build_arg_list.append("--javaArgs")
            else:
                build_arg_list.append("--" + arg)

            if arg in ["files", 'verify', 'assert_contracts']:
                for f in val:
                    build_arg_list.append(f)
            elif arg == 'address':
                for contract, address in val.items():
                    build_arg_list.append(f"{contract}:{address}")
            else:
                build_arg_list.append(str(val))

    return ' '.join(build_arg_list)

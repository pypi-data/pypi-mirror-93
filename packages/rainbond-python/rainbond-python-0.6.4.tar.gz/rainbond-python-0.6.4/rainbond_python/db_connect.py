...
from .tools import handle_date, handle_db_to_list, handle_db_dict, handle_db_id, handle_db_remove
...
class DBConnect:
    ...
    def delete_docu(self, find_docu: dict, many: bool = False, false_delete: bool = False) -> dict:
        try:
            ...
            str_data = 'MongoDB(组件)出现删除错误: {0}'.format(err)
            logging.warning(str_data)
            abort(Response(str_data, 500, {}))
    ...
    def find_docu(self, find_dict: dict, many: bool = True) -> list:
        try:
            ...
            str_data = 'MongoDB(组件)出现查询错误: {0}'.format(err)
            logging.warning(str_data)
            abort(Response(str_data, 500, {}))
    ...
    def find_docu_by_id(self, id: str, raise_err=True) -> dict:
        ...
        if not isinstance(id, str):
            logging.warning("类型错误，预期 %s ,却得到 %s" % (str, type(id)))
            abort(Response("类型错误，预期 %s ,却得到 %s" % (str, type(id)), 400, {}))
        entity = self.find_docu({'id': id}, many=False)
        if entity is None or len(entity) == 0:
            if raise_err:
                logging.warning('找不到 id= %s 的记录' % (id,))
    ...
    def find_docu_by_id_list(self, id_list: list) -> list:
        ...
        if not isinstance(id_list, list):
            ...
            abort(Response("类型错误，预期 %s ,却得到 %s" %
                           (list, type(id_list)), 400, {}))
        id_search = []
        for id in id_list:

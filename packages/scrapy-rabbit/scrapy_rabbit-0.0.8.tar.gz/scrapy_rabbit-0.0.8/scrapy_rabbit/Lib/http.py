...
from fuclib import proxyUser
from fuclib import proxyPass
    ...
    async def run(self, session, request):
        ...
        while retry_times < self.max_times:
            try:
                ...
                error(f"第{retry_times}次请求超时，url：{request['url']}")
                retry_times += 1
            except Exception as e:
                error(f"第 {retry_times} 次请求报错({type(e).__name__}: {e})，url：{request['url']}")
                retry_times += 1
    ...
    async def quest(session, request):
        if request['method'].upper() == 'GET':
            async with session.get(url=URL(request['url'],
                                   ...
                                   proxy_auth=BasicAuth(proxyUser, proxyPass)) as res:
                text = await res.read()

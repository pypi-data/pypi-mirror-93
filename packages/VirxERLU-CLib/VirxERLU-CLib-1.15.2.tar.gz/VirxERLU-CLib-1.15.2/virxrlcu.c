...
static const double simulation_dt = 1. / 80.;
...
_Bool can_reach_target_forwards(double max_time, double boost_accel, double *distance_remaining, double *car_speed, unsigned char car_boost)
{
    ...
    const double quad_dt = simulation_dt * 4;

    double T = max_time + simulation_dt;
    const double r = d / T;
    ...
    while (T > 0 && !(r > ((b < bc_dt && v <= max_speed_no_boost) ? max_speed_no_boost : max_speed) || (b < bc_dt && v > max_speed_no_boost && r > v)))
    {
        v += (v < 0) ? bk_dt : throttle_acceleration(v) * simulation_dt;

        if (b > bc_dt && v < max_speed)
        {
            v += ba_dt;
        ...
        if (d <= fabs(v) * quad_dt || v - r > -100)
            return 1;
    }

    return d <= fabs(v) * quad_dt || v - r > -100;
}

_Bool can_reach_target_backwards(double max_time, double *distance_remaining, double *car_speed)
{
    double d = *distance_remaining;
    double v = *car_speed;
    ...
    if (r > max_speed_no_boost)
        return 0;

    if (v - r > -100)
        return 1;

    while (T > 0 && r <= max_speed_no_boost)
    {
        v += (v < 0) ? bk_dt : throttle_acceleration(v) * simulation_dt;
...
_Bool legacy_generic_is_viable(double *T, double jump_time, double *boost_accel, Vector *car_forward, double *car_speed, unsigned char *car_boost, Vector direction, double *distance_remaining, double *total_jump_accel)
{
    ...
    double forward_distance = *distance_remaining + forward_angle * turn_rad;
    double backward_distance = *distance_remaining + backward_angle * turn_rad;

    _Bool forward = *T > 0 && can_reach_target_forwards(*T, *boost_accel, &forward_distance, car_speed, *car_boost);
    _Bool backward = *T > 0 && forward_angle >= 1.6 && *car_speed <= max_speed_no_boost && can_reach_target_backwards(*T, &backward_distance, car_speed);
...
_Bool generic_is_viable(double *T, double jump_time, double *boost_accel, Car *me, Vector direction, double *distance_remaining, double *total_jump_accel)
{
    ...
    {
        forward_time -= 2 * (forward_angle / PI);
        backward_time -= 2 * (backward_angle / PI);
    }

    _Bool forward = forward_time > 0 && can_reach_target_forwards(forward_time, *boost_accel, distance_remaining, &true_car_speed, me->boost);
    true_car_speed *= -1;
    _Bool backward = backward_time > 0 && forward_angle >= 1.6 && true_car_speed < 1000 && can_reach_target_backwards(backward_time, distance_remaining, &true_car_speed);
...
_Bool legacy_double_jump_shot_is_viable(double *T, double *boost_accel, double *distance_remaining, Vector *direction, Vector *car_forward, unsigned char *car_boost, double *car_speed, double slope)
{
    ...
    return forward_time > 0 && can_reach_target_forwards(forward_time, *boost_accel, distance_remaining, car_speed, *car_boost);
}
...
Shot parse_slice_for_shot_with_target(_Bool do_ground_shot, _Bool do_jump_shot, _Bool do_double_jump_shot, _Bool do_aerial_shot, double *time_remaining, double *ball_radius, double *boost_accel, Vector *gravity, Ball *ball_slice, Car *me, Target *targets)
{
	...
	if(post_info.fits)
	{
		...
		{
            Vector shot_vector = get_shot_vector_2d(&direction, ball_slice, targets);
            Vector ball_offset = get_ball_offset(ball_slice, &shot_vector, ball_radius);
			double slope = find_slope(flatten(shot_vector), flatten(car_to_ball));

            if (in_2d_field(&ball_offset, &me->hitbox))
            {
                double distance_remaining = flat_dist(ball_offset, me->location) - (me->hitbox.length * 0.95);
		...
		{
            Vector shot_vector = get_shot_vector_3d(&direction, ball_slice, targets);
            Vector ball_offset = get_ball_offset(ball_slice, &shot_vector, ball_radius);

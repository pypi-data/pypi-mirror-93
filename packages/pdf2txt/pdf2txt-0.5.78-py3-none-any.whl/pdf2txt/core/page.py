    ...
    def detect_semantic_structure(self):
        ...
        for region_number, region in enumerate(self.regions):
            ...
            if self.page_number==0 and region_number <=1:
                if region_number==1 and region.height<self.bbox.height/8:
                ...
                while i < len(p)-1:
                    ...
                    if len(l)>=2 or (len(l)==1 and utils.get_type(l[0].Text.strip('%'))=="Numeric"):
                        lines_for_table.append(l)
                            ...
                            table = Table(region, textlines=lines_for_table, rect=utils.get_BoundingBox_from_objects(tables)).extract()
...
#                    pp=[p for p in pp if p not in table.textlines]
    ...
    def is_title2(self, textline, font_stat, page_font_stat, title_font={}, font_body={}):
        ...
        if title_font and textline[0].font+str(textline[0].Text.isupper() or textline[0].is_bold)==title_font['name']:
            ...
            return False
        if (len(font_stat["name_by_frequency"]) > 1 or len(font_stat["size_by_frequency"]) > 1) and textline[0].font_size >= font_stat['second_largest']:
...
class Region(Page):
    ...
    def __init__(self, parent_page, bbox):
        ...
        self.bbox=BoundingBox(top=int(bbox.top*parent_page.pdf_height_scaler), left=int(bbox.left*parent_page.pdf_width_scaler), right=int(bbox.right*parent_page.pdf_width_scaler), bottom=int(bbox.bottom*parent_page.pdf_height_scaler))

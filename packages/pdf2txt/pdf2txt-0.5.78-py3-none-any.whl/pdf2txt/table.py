...
class Table(object):
    ...
    def extract_from_lines(self):
        ...
        self.textlines = [self.post_process_token(line) for line in self.page.text_lines if line[0].top>= bbox.top and line[0].top<= bbox.bottom]
        if self.textlines == []:
            return self
    ...
    def _fix_header_rows(self, header, tolerance=5):
        ...
        for i in reversed(range(1, len(header))):
            ...
            for token in header[i - 1]:
                if token.left - tolerance > min(t.left for t in header[i]) or len(header[i - 1]) == len(header2[-1]) or len(header[i - 1]) == len(header[i]):  # not left justified or as many header as the last row
                    for t in header[i - 1]:

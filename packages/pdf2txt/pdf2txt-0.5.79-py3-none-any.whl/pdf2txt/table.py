...
class Table(object):
    ...
    def extract_from_textlines(self, rows):

        cols = set()
        for row in rows:
            for token in row:
                col = self.vertically_in_line_with(
                    token, rows, tolerance=5
                )
                # col_exists = False
                # for c in cols:
                #     if bool(c & col):
    ...
    def extract_from_lines(self):
        ...
        self.textlines = [self.post_process_token(line) for line in self.page.text_lines if
                            line[0].top >= bbox.top and line[0].top <= bbox.bottom]
        if self.textlines == []:
            return self
...
#                    row = [item for sublist in self.horizontally_within_lines(b, t) for item in sublist]
                ...
                if row and row not in rows:
                    rows.extend(row)
        if sum([len(r) for r in rows[self._nb_headers:]]) < sum([len(l) for l in self.textlines[len(header):]]):
            return self.extract_from_textlines( )

#            last_row = [item for sublist in self.horizontally_within_lines(b, None) for item in sublist]
        last_row = self.horizontally_within_lines(b, None, ncols)
        if last_row and len(last_row) >= ncols - 1:
            rows.extend(last_row)
        elif last_row:
            self.textlines = self.textlines[:-1]

        return self.extract_from_textlines(rows)
    ...
    def _fix_header_rows(self, header, ncols, tolerance=5):
        ...
        for i in reversed(range(1, len(header))):
            ...
            for token in header[i - 1][start_index:]:
                if token.left - tolerance > min(t.left for t in header[i][start_index:]) and(len(header[i - 1]) == len(header2[-1]) or len(header[i - 1]) == len(header[i])):  # not left justified or as many header as the last row
                    for t in header[i - 1]:
...
class TableFinder(object):
    ...
    def _guess_tables_from_lines(self, lines, threshold=3):
        ...
        min_row_height = 40
        left_right = utils.cluster_objects_multi(lines, "left", "right", 2)
    ...
    def guess_table_lines(self, token_threshold_v=3, token_threshold_h=2, pruning_cut_off=2):
        from pdf2txt.core.page import Region
        h_edges = self.words_to_edges_h(word_threshold=token_threshold_h)
        h_edges=utils.cluster_objects_multi(h_edges, "left", "right", 2)
    ...
    def guess_table_lines2(self, token_threshold_v=3, token_threshold_h=2, pruning_cut_off=2):
        from pdf2txt.core.page import Region
        h_edges = self.words_to_edges_h(word_threshold=token_threshold_h)
        v_edges = self.words_to_edges_v(word_threshold=token_threshold_v)

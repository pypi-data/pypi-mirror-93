...
class ML_models():
    ...
    def normalization(self):
        ...
        normalized_train = np.empty(shape=self.train_inputs.shape)
        normalized_test = np.empty(shape=self.test_inputs.shape)

        for i in range(self.train_inputs.shape[1]):
            max_train.append(max([_[i] for _ in self.train_inputs]))
            min_train.append(min([_[i] for _ in self.train_inputs]))

            max_test.append(max([_[i] for _ in self.test_inputs]))
            min_test.append(min([_[i] for _ in self.test_inputs]))

        for index, (arr_train, arr_test) in enumerate(zip(self.train_inputs, self.test_inputs)):
            train = [(e - mi)/(ma - mi) for e,mi,ma in list(zip(arr_train,min_train,max_train))]
            normalized_train[index] = train

            test = [(e - mi)/(ma - mi) for e,mi,ma in list(zip(arr_test,min_test,max_test))]
            normalized_test[index] = test
    ...
    def standardization(self):
        ...
        standardized_train = np.empty(shape=self.train_inputs.shape)
        standardized_test = np.empty(shape=self.test_inputs.shape)

        for i in range(self.train_inputs.shape[1]):
            mean_train.append(np.mean([_[i] for _ in self.train_inputs]))
            std_train.append(np.std([_[i] for _ in self.train_inputs]))

            mean_test.append(np.mean([_[i] for _ in self.test_inputs]))
            std_test.append(np.std([_[i] for _ in self.test_inputs]))

        for index, (arr_train, arr_test) in enumerate(zip(self.train_inputs, self.test_inputs)):
            train = [(e - mean)/std for e,mean,std in list(zip(arr_train,mean_train,std_train))]
            standardized_train[index] = train

            test = [(e - mean)/std for e,mean,std in list(zip(arr_test,mean_test,std_test))]
            standardized_test[index] = test

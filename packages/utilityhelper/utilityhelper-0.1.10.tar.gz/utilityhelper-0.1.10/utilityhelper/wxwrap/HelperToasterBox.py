...
class ToasterBox(TB.ToasterBox):
    @staticmethod
    def MakeText(win, info='', tbstyle=TB.TB_SIMPLE, windowstyle=TB.TB_DEFAULT_STYLE,
                 closingstyle=TB.TB_ONTIME, scrollType=TB.TB_SCR_TYPE_FADE):
        
        tbox = ToasterBox(win, tbstyle, windowstyle, closingstyle, scrollType)
        
        if info:
            tbox.SetPopupText(info)
            tbox.SetPopupSize(wx.Size(200, 80))
        else:
            tbox.SetPopupSize(wx.Size(100, 40))

        #tb.SetPopupPosition(win.TopLevelParent.GetPos())
        tbox.CenterOnParent(wx.BOTH)    
        #tbox.SetPopupBackgroundColour("black")
        tbox.SetPopupPauseTime(2000)
        tbox.SetPopupScrollSpeed(8)
        return tbox

...
class PortManager(object):
    ...
    def check_modem_lines(self, force_notification=False):
        modemstate = (
            (self.serial27.getCTS() and MODEMSTATE_MASK_CTS) |
            (self.serial27.getDSR() and MODEMSTATE_MASK_DSR) |
            (self.serial27.getRI() and MODEMSTATE_MASK_RI) |
            (self.serial27.getCD() and MODEMSTATE_MASK_CD)
        )
    ...
    def _telnetProcessSubnegotiation(self, suboption):
        ...
        if suboption[0:1] == COM_PORT_OPTION:
            ...
            if suboption[1:2] == SET_BAUDRATE:
                backup = self.serial27.baudrate
                try:
                    (baudrate,) = struct.unpack("!I", suboption[2:6])
                    if baudrate != 0:
                        self.serial27.baudrate = baudrate
                except ValueError as e:
                    if self.logger:
                        self.logger.error("failed to set baud rate: %s" % (e,))
                    self.serial27.baudrate = backup
                else:
                    if self.logger:
                        self.logger.info("%s baud rate: %s" % (baudrate and 'set' or 'get', self.serial27.baudrate))
                self.rfc2217SendSubnegotiation(SERVER_SET_BAUDRATE, struct.pack("!I", self.serial27.baudrate))
            elif suboption[1:2] == SET_DATASIZE:
                backup = self.serial27.bytesize
                try:
                    (datasize,) = struct.unpack("!B", suboption[2:3])
                    if datasize != 0:
                        self.serial27.bytesize = datasize
                except ValueError as e:
                    if self.logger:
                        self.logger.error("failed to set data size: %s" % (e,))
                    self.serial27.bytesize = backup
                else:
                    if self.logger:
                        self.logger.info("%s data size: %s" % (datasize and 'set' or 'get', self.serial27.bytesize))
                self.rfc2217SendSubnegotiation(SERVER_SET_DATASIZE, struct.pack("!B", self.serial27.bytesize))
            elif suboption[1:2] == SET_PARITY:
                backup = self.serial27.parity
                try:
                    parity = struct.unpack("!B", suboption[2:3])[0]
                    if parity != 0:
                            self.serial27.parity = RFC2217_REVERSE_PARITY_MAP[parity]
                except ValueError as e:
                    if self.logger:
                        self.logger.error("failed to set parity: %s" % (e,))
                    self.serial27.parity = backup
                else:
                    if self.logger:
                        self.logger.info("%s parity: %s" % (parity and 'set' or 'get', self.serial27.parity))
                self.rfc2217SendSubnegotiation(
                    SERVER_SET_PARITY,
                    struct.pack("!B", RFC2217_PARITY_MAP[self.serial27.parity])
                    )
            elif suboption[1:2] == SET_STOPSIZE:
                backup = self.serial27.stopbits
                try:
                    stopbits = struct.unpack("!B", suboption[2:3])[0]
                    if stopbits != 0:
                        self.serial27.stopbits = RFC2217_REVERSE_STOPBIT_MAP[stopbits]
                except ValueError as e:
                    if self.logger:
                        self.logger.error("failed to set stop bits: %s" % (e,))
                    self.serial27.stopbits = backup
                else:
                    if self.logger:
                        self.logger.info("%s stop bits: %s" % (stopbits and 'set' or 'get', self.serial27.stopbits))
                self.rfc2217SendSubnegotiation(
                    SERVER_SET_STOPSIZE,
                    struct.pack("!B", RFC2217_STOPBIT_MAP[self.serial27.stopbits])
                    )
            elif suboption[1:2] == SET_CONTROL:
                if suboption[2:3] == SET_CONTROL_REQ_FLOW_SETTING:
                    if self.serial27.xonxoff:
                        self.rfc2217SendSubnegotiation(SERVER_SET_CONTROL, SET_CONTROL_USE_SW_FLOW_CONTROL)
                    elif self.serial27.rtscts:
                    ...
                    self.serial27.setRTS(True)
                    if self.logger:
                        self.logger.info("changed RTS to active")
                    self.rfc2217SendSubnegotiation(SERVER_SET_CONTROL, SET_CONTROL_RTS_ON)
                elif suboption[2:3] == SET_CONTROL_RTS_OFF:
                    self.serial27.setRTS(False)
                    if self.logger:

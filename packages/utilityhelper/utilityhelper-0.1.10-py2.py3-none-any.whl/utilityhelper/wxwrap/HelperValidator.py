...
import string
import wx
from utilityhelper.wxwrap.HelperToasterBox import ToasterBox
...
class MyValidator(wx.Validator):
    ...
    def Validate_TC_Content(self, win):
        ...
        if len(text) == 0:
            tbox = ToasterBox.MakeText(win, "请输入数据")
            tbox.Show()
    ...
    def OnChar(self, event):
        ...
        if self.flag == ALPHA_ONLY and six.int2byte(key) in string.ascii_letters.encode('utf-8'):
            event.Skip()
            return

        if self.flag == DIGIT_ONLY and six.int2byte(key) in string.digits.encode('utf-8'):
            event.Skip()
            return

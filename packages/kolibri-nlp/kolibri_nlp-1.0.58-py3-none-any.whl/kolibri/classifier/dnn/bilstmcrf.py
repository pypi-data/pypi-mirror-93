...
from tensorflow.keras.layers import Dense, LSTM, Bidirectional, Embedding, Input, Dropout, TimeDistributed, ZeroPadding1D, Conv1D
...
class BiLstmCrf(object):
    ...
    def build(self):
        ...
        if self._use_crf:
            crf = CRF(self._num_labels, sparse_target=False, learn_mode='marginal')
            loss = crf.loss
            pred = crf(z)
        else:
...
class BiLstrmCnnCRF(object):
    ...
    def build(self):
        ...
        # bilstm
        bilstm = Bidirectional(LSTM(units=self._word_lstm_size, return_sequences=True))(word_embeddings)
        bilstm_d = Dropout(self._dropout)(bilstm)

        # cnn
        half_window_size = 2
        padding_layer = ZeroPadding1D(padding=half_window_size)(word_embeddings)
        conv = Conv1D(nb_filter=50, filter_length=2 * half_window_size + 1, padding='valid')(padding_layer)
        conv_d = Dropout(0.1)(conv)
        dense_conv = TimeDistributed(Dense(self._fc_dim, activation='relu'))(conv_d)

        # merge
        rnn_cnn_merge = concatenate([bilstm_d, dense_conv])
        dense = TimeDistributed(Dense(self._num_labels, activation='relu'))(rnn_cnn_merge)
        ...
        if self._use_crf:
            ...
            loss = 'categorical_crossentropy'
            pred = Dense(self._num_labels, activation='softmax')(dense)

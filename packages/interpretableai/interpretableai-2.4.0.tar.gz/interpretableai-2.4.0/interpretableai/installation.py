...
def iai_run_julia_setup(**kwargs):
    ...
    if os.path.isfile(replace_sysimg_file):  # pragma: no cover
    ...
    if 'runtime' in kwargs and os.name == 'nt':  # pragma: no cover
...
def julia_tgz_url(version, build):  # pragma: no cover
    arch = 'x86_64'
    if islinux():
        os_code = 'linux-gnu'
    elif isapple():
...
def iai_download_url(julia_version, iai_version):  # pragma: no cover
    if isapple():
        os_code = 'macos'
    elif iswindows():
        os_code = 'win64'
...
def sysimage_save_replace_command(image_path, target_path):  # pragma: no cover

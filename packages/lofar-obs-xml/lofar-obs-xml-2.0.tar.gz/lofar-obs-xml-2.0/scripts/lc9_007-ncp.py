...
for mid_day_string in sys.argv[1:]:

    mid_day    = momxml.ephem.Date(mid_day_string)
...
main_folder = momxml.Folder(children=date_folders, name='EoR-NCP', description='EoR NCP observations')

with open('eor-ncp-%d%02d%02d.xml' % (start_dates[0].tuple()[0:3]), 'w') as output:
    output.write(momxml.xml([main_folder], 'LC9_007', description='LOFAR Epoch of Reionization observations on the 7 NCP fields: On A Fast Track to the EoR/Cosmic Dawn '))

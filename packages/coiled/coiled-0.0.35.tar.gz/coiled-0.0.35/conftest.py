...
def container_registry_fixture():
    """
    Not using as a pytest fixture because this is in a OneToOne relationship.
    Therefore returning a fresh one every time to avoid UNIQUE constraints
    """
    if settings.TESTING_DEFAULT_REGISTRY == "in_process":
        cr = ContainerRegistry(type=ContainerRegistryType.IN_PROCESS)
        cr.save()
        return cr
    return None

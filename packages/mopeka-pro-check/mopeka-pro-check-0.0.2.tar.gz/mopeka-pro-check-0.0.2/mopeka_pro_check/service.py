...
Code was written refrencing that project but it is so signficantly different that I am not
...
class ReadStats(object):
  ...
  def __str__(self):
    return f"ReadStats ( Ignored Ad Count: {self._ignored_ad_count}, Processed Ad Count: {self._processed_ad_count}, Error Ad Count: {self._error_ad_count})"
...
class MopekaService(object):
  ...
  def RemoveSensorToMonitor(self, sensor: MopekaSensor) -> None:
    ...
    if sensor._bdaddress in self.SensorMonitoredList:
      if self._scanning_mode == ServiceScanningMode.FILTERED_MODE:
        self._stop()
      self.SensorMonitoredList.remove(self.SensorMonitoredList[sensor._bdaddress])

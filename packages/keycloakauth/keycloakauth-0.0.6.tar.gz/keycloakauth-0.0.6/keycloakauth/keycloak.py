...
class KeycloakWorker(django_keycloak_auth.KeycloakConnect):
    ...
    @token_decorator
    def remove_role(self, role_uuid):
        try:
            resp = self.session.get(f'{self.server_url}{self.roles_endpoint}/{role_uuid}',
                                    headers=self.headers, verify=self.verify)
            resp.raise_for_status()
        except Exception as error:
            raise KeycloakException(f'Request for {role_uuid} role failed. Error: {error}')

    @token_decorator
    def remove_group(self, group_uuid):
        try:
            resp = self.session.delete(f'{self.server_url}{self.groups_endpoint}/{group_uuid}',
                                       headers=self.headers, verify=self.verify)
            resp.raise_for_status()
        except Exception as error:
            raise KeycloakException(f'Request for {group_uuid} group failed. Error: {error}')

...
class KeycloakWorker(django_keycloak_auth.KeycloakConnect):
    ...
    def remove_role(self, role_name):
        ...
        endpoint = self.roles_endpoint % {'client_uuid': client_uuid}
        try:
            resp = self.session.delete(f'{self.server_url}{endpoint}/{role_name}', headers=self.headers,
                                       verify=self.verify)
            resp.raise_for_status()
        except Exception as error:
            raise KeycloakException(f'Request for {role_name} role failed. Error: {error}')

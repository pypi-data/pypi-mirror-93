...
class ActionsImpl(Actions):
    ...
    def get(self, uri, params={}):
        ...
        r = None
        try:
            url = build_url(config_info['account'], uri, params)
            r = requests.get(url, params, headers=config_info['api_key'])
    ...
    def create(self, uri, payload):
        ...
        r = None
        try:
            url = build_url(config_info['account'], uri, {})
            r = requests.post(url, data=payload, headers=config_info['api_key'])
        except requests.exceptions.RequestException as e:
            custom_print(UNABLE_TO_CONNECT_OR_TIMED_OUT, err=True)
            exit()
        return r

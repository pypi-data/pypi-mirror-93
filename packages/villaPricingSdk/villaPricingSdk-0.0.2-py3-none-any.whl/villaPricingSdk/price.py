# AUTOGENERATED! DO NOT EDIT! File to edit: priceSdk.ipynb (unless otherwise specified).

__all__ = ['union', 'Endpoints', 'PricingSdk', 'uploadDf', 'ingestData', 'branchQuery', 'queryAll2',
           'querySingleProduct2']
...
class Endpoints:
  ...
  inputS3 = lambda self: f'input-price-bucket-{self.branchName}'
  querySingleProduct = lambda self: f'single-product-query-inventory-{self.branchName}'
  queryBranch = lambda self: f'query-branch-price-{self.branchName}'
  triggerIngestion = lambda self: f'trigger-ingestion-price-{self.branchName}'
  queryAll2 = lambda self: f'query-all-price2-{self.branchName}'


class PricingSdk:
  ''' interact with villa pricing database '''
...
# Cell
@add_method(PricingSdk)
def uploadDf(self, df:pd.DataFrame, key:str ='1000', invApi:str = '7gha4v75z5')->bin:
...
# Cell
@add_method(PricingSdk)
def ingestData(self, functionName= 'trigger-ingestion-price-dev-manual', key='1000', dtype='feather'):

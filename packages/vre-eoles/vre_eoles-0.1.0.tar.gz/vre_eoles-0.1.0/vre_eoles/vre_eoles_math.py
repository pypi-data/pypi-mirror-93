...
def gaussian_smoothing(mu, sig, x, y):
    ...
    y_convolve = y_convolve[min(np.argwhere(x_filter>=0))[0]:min(np.argwhere(x_filter>=35))[0]]
    x_filter = x_filter[min(np.argwhere(x_filter>=0))[0]:min(np.argwhere(x_filter>=35))[0]]
...
def conv_vwf(ws, alpha, beta, power):
    ...
    for w in ws:
        ...
        kernel_support = np.arange(-int(n_sigma_square * int(sigma_square+1)),
                                    int(n_sigma_square * int(sigma_square+1)) + delta_w,delta_w) # m/s
        ...
        kernel = 1./np.sqrt(2.* np.pi * sigma_square) * np.exp(-np.power(kernel_support, 2.)/
                                                               (2 * sigma_square))

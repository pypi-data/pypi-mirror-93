...
def request_vwf(args):
    ...
    token = "cc64041f6877fe93164de04f3a1ac365d3654283"
...
def vwf_api(df, year, root_outputs, root_inputs_vwf, median_hub_height=80, EOLES=False):
    """
    ...
    def turbine_name(county,):
        ...
        turbine_name = vre.wp2wvf[turbine_type]
    ...
    for county in df.index:
        ...
        if (not test) & (not (county + ".xlsx" in files)):
            ...
            idx += 1
            data, metadata, status = vre.request_vwf(args)
            if (
                status

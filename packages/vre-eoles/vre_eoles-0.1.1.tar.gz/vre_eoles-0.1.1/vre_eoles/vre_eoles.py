...
def vre_aggregation(year, root_outputs, root, new_turbine=False):
    ...
    This routine aggregates all the excel files in three files:
        - the first contains the loading rate with a sample time of one hour
    for each county.
        - the second contains the wind speed with a sample time of one hour
    for each county.
        - the third contains, for each county, the parameters provided to the ninja api.
    """

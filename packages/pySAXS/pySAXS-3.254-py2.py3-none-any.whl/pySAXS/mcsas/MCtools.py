...
def BeamConvolution(ToConvolve,q,dq):
    sigma=dq/2.3548#0.003/2.3548#par[6]/2.3548
    #ToConvolve=IMonodisperse
    Tc=zeros(len(q))
    for j in range(len(q)):
        B=gs(q,1,q[j],sigma)
        Tc[j]=Tc[j]+sum(B*ToConvolve)/sum(B)
    return Tc
...
def Analyze_1D(q,I,E,Bounds=[],Nsph=[],Maxiter=[],Rpfactor=1.5/3,Nreps=100,qlims=[0,inf],\
               Histbins=50,Histscale='lin',drhosqr=1,Convcrit=1.,StartFromMin=False,Maxntry=5,\
               silent=True,ConvValueDisplay=None):
    '''
    This function runs the monte-carlo fit MCFit_sph() several times, and returns 
    bin centres, means, standard deviations and observability limits for the bins. 
    Eventually, this may also plot. The drhosqr value can be set to the contrast 
    ...
    for nr in arange(0,Nreps):
        ...
        Rrep[:,nr],Irep[:,nr],ConVal,Details = MCFit_sph(q,I,E,Bounds=Bounds,Nsph=Nsph,Maxiter=Maxiter,Rpower=Rpower,OutputI=True,\
                                                         ...
                                                         silent=silent,ConvValueDisplay=ConvValueDisplay)
        while ConVal>Convcrit:
            nt+=1
            Rrep[:,nr],Irep[:,nr],ConVal,Details=MCFit_sph(q,I,E,Bounds=Bounds,Nsph=Nsph,Maxiter=Maxiter,Rpower=Rpower,OutputI=True,\
                                                           Convcrit=Convcrit,StartFromMin=StartFromMin,OutputDetails=True,silent=silent,\
                                                           ConvValueDisplay=ConvValueDisplay)
            if nt>Maxntry:
                print("could not reach optimization criterion within {0} attempts, exiting...".format(Maxntry+2))
...
################################### Monte-carlo procedure #################################
###########################################################################################
def MCFit_sph(q,I,E,Nsph=200,Bounds=[],Convcrit=1,Rpower=2,Maxiter=1e5,Prior=[],Qlimits=numpy.array([]),MaskNegI=False,\
              OutputI=False,StartFromMin=False,OutputDetails=False,OutputIterations=False,\
              silent=True,ConvValueDisplay=None):
    '''
    ...
    while (Conval>Convcrit) &(Niter<Maxiter):
        ...
        if Convalt<Conval: # it's better
            ...
            if not silent:
                print("Improvement in iteration number %i, convergence value %f of %f" %(Niter,Conval,Convcrit))
            if ConvValueDisplay is not None:
                ConvValueDisplay(Conval)
            Nmoves+=1
            if OutputIterations:
...
################################### Monte-carlo procedure #################################
###########################################################################################
def MCFit_sphTEST(q,I,E,Nsph=200,Bounds=[],Convcrit=1,Rpower=2,Maxiter=1e5,Prior=[],Qlimits=numpy.array([]),MaskNegI=False,OutputI=False,StartFromMin=False,OutputDetails=False,OutputIterations=False,silent=False):
    '''
    Rewrite of the monte-carlo method previously implemented in Matlab. 
    Simpler form, but open source might mean slight improvements.
    '''
    ...
    #calculate the intensities
    Iset=FFset**2*(Vset[:,newaxis]+0*FFset)**2 #a set of intensities
    #Iset=BeamConvolution(Iset, q, 0.003)
    Vst=sum(Vset**2) # total volume squared
    It=sum(Iset,0) # the total intensity - eq. (1)
    # Optimize the intensities and calculate convergence criterium
    Sc,Conval1=Iopt_v1(I,It/Vst,E,[1,1]) # V1 is more robust w.r.t. a poor initial guess
    Sc,Conval=Iopt(I,BeamConvolution(It/Vst, q, 0.003),E,Sc) # reoptimize with V2, there might be a slight discrepancy in the residual definitions of V1 and V2 which would prevent optimization.
    #print "Initial conval V1",Conval1
    print("Initial conval V2",Conval)
    ...
    while (Conval>Convcrit) &(Niter<Maxiter):
        ...
        Vtt=(4.0/3*pi)*Rt**Rpower
        Itt=(Ft**2*Vtt**2)
        Itt=BeamConvolution(Itt, q, 0.003)
        # Calculate new total intensity
        Itest=(It-Iset[Ri,:]+Itt) # we do subtractions and additions, which give us another factor 2 improvement in speed over summation and is much more scalable
        Vstest = (sqrt(Vst)-Vset[Ri])**2+Vtt**2
        # optimize intensity and calculate convergence criterium

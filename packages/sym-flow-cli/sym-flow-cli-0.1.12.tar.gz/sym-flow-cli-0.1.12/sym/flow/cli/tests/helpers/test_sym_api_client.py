...
from sym.flow.cli.errors import NotAuthorizedError, SymAPIUnknownError, UnknownOrgError
...
class TestSymAPIClient:
    ...
    def test_get_organization_from_email(self, mock_api_get, sym_api_client, test_org):
        ...
        mock_api_get.assert_called_once_with(
            f"organizations/from-email/{quote(email)}", auth_required=False
        )
        assert org == test_org
    ...
    def test_get_organization_from_email_missing_data_errors(
        ...
        with patch(
            "sym.flow.cli.helpers.sym_api_client.SymAPIClient.get",
            return_value=(
                get_mock_response(200, data={"client_id": "123"}),
                "test-request-id",
            ),
        ...
        with patch(
            "sym.flow.cli.helpers.sym_api_client.SymAPIClient.get",
            return_value=(
                get_mock_response(200, data={"slug": "test"}),
                "test-request-id",
            ),
    ...
    @patch(
        "sym.flow.cli.helpers.sym_api_client.SymAPIClient.get",
        return_value=(
            get_mock_response(200, url="http://hello.sym.com"),
            "test-request-id",
        ),
    )

...
from mjango import db, exceptions
...
class MongoBase:
    ...
    def filter(self, query, skip, limit):
        return self.client.find(query, skip=skip, limit=limit)
...
class MongoDB(db.BaseDB):
    ...
    def filter(self, query, skip=0, limit=0):
        skip = skip or 0
        limit = limit or 0
        return self.db_base.filter(query, skip, limit)

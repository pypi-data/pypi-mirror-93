...
'''
from brails.modules.RoofTypeClassifier import RoofClassifier
...
class CityBuilder:
    ...
    def __init__(self, attributes=['story','occupancy','roofshape'], numBldg=10, random=True,bbox=[], place='',state='', footPrints='OSM', save=True, fileName='', workDir='tmp',GoogleMapAPIKey='', overwrite=False, reDownloadImgs=False):
        ...
        if stateName:# bbox, in the US
            ...
            if footPrints=='OSM':
                ...
                self.BIM, footprintFilename = getOSMFootprints(bbox=bbox,save=save,workDir=workDir,overwrite=True)
            elif footPrints=='Microsoft':
                # MS
                print(f"Trying to get MS footprints by the bbox {bbox}.")
                self.BIM, footprintFilename = getMSFootprintsByBbox(bbox=bbox,stateName=stateName,save=save,workDir=workDir,overwrite=overwrite,GoogleMapAPIKey=GoogleMapAPIKey)
        else: # out of US or by place
            if len(bbox)==4: # out of US: use OSM no matter what
                print(f"Trying to get OSM footprints by the bbox {bbox}.")
                self.BIM, footprintFilename = getOSMFootprints(bbox=bbox,save=save,workDir=workDir,overwrite=True)
            else: # by place
                if footPrints=='OSM':
                    ...
                    assert False, f"Trying to get Microsoft footprints by the place name {place}. This only works for the US!"
                    self.BIM, footprintFilename = getMSFootprintsByPlace(place=place,state=state,save=save,workDir=workDir,overwrite=overwrite)
        
        '''
        else:
            if footPrints=='OSM':
                self.BIM, footprintFilename = getOSMFootprints(place=place,save=save,workDir=workDir,overwrite=overwrite)
            else:
                # This will cause error if the interested region is out of the US
                assert False, "Microsoft footprint only covers the United States."
                self.BIM, footprintFilename = getMSFootprintsByPlace(place=place,state=state,save=save,workDir=workDir,overwrite=overwrite)
        '''

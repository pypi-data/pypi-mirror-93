...
@_available_to_user_util
def traverse(
    self, string='', level=100, split=True, onlyDict=False, onlyLeaf=False, skipDynaLoad=False, noSubmodules=False, traverse_classes=(dict,)
):
    """
...
class SortedDict(dict):
        ...
        if (
            ...
            try:
                if os.environ['USER'] == 'meneghini':
                    printw('%s dynaloading because %s attribute was requested' % (self.__class__.__name__, attr))
                    print_stack()
    ...
    @dynaLoad
    def diff(
        self, other, ignoreComments=False, ignoreContent=False, skipClasses=(), noloadClasses=(), precision=0.0, order=True, quiet=True
    ):
        """
    ...
    @dynaLoad
    def pretty_diff(self, other, ignoreComments=False, ignoreContent=False, skipClasses=(), noloadClasses=(), precision=0.0, quiet=True):
        """

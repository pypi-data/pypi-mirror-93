...
try:
    ...
    warnings.warn('Unable to import omfit_plot; plotting will not work: %s'%repr(_excp))

__all__ = ['OMFITtranspNamelist', 'OMFITtranspData', 'OMFITtranspMultigraph', 'OMFITplasmastate', 'OMFITtranspOutput',
           'check_TRANSP_run', 'wait_TRANSP_run', 'next_available_TRANSP_runid', 'OMFITfbm']

_OMFITtranspNamelist_class_attributes={'collect_arrays':{'__offset__':1},  #Collect arrays to access array data consistently within OMFIT
...
class OMFITtranspNamelist(OMFITnamelist):
    ...
    def __init__(self, filename, **kw):
        ...
        tmp.update(kw)
        OMFITnamelist.__init__(self, filename, **tmp) #the call to OMFITnamelist.__init__ with **kw takes care of setting up the class attributes
    ...
    def load(self):
        ...
        for sec in list(sections.keys()):
            sections[sec]='\n'.join(sections[sec])
            tmp=NamelistFile(None,input_string=sections[sec],
                             **{attr:getattr(self,attr) for attr in _OMFITtranspNamelist_class_attributes})
            if sec=='main':
                self.update(tmp)
            else:
                self[sec]=tmp
...
class OMFITtranspData(SortedDict):
    ...
    def __init__(self, transp_output, data):
        ...
        if isinstance(transp_output, OMFITnc):
            ...
            self['DIM_OF'] = [transp_output[transp_output[data]['__dimensions__'][k]]['data'] for k in reversed(list(range(len(self['DATA'].shape))))]
            self['UNITS_DIM_OF'] = [transp_output[transp_output[data]['__dimensions__'][k]]['units'].strip() for k in reversed(list(range(len(self['DATA'].shape))))]
            ...
            if len(self['DATA'].shape) == 2:
                ...
                if len(self['DIM_OF'][1].shape) == 1:
                    self['DIM_OF'] = [self['DIM_OF'][0],
                                      np.array([self['DIM_OF'][1]] * self['DIM_OF'][0].shape[1]).T]
        ...
        # MDS+
            ...
            raise ValueError('transp_output of class %s is not recognized. Must be either %s or %s' % (transp_output.__class__, OMFITmds, OMFITnc))
    ...
    def plot(self,axes=None,label='RPLABEL',slice_axis=None,slice_at=[],**kw):
        ...
        # 2D data
            ...
            if slice_axis==None:
                view = View2d(self['DATA'].T,coords=[x,t],dims=[x0label,x1label],name='$['+self['UNITS']+']$',
                              axes=axes,imag_options=im_opt,**kw)
            ...
            if hasattr(tab.fig,'view'):
                ...
                def update_lines(tab,event=None):
                    ...
                    for tid in note.tabs():
                        ...
                        axx.fill_between(v._hslice_xdata,v._hslice_mean-v._hslice_std,v._hslice_mean+v._hslice_std,
                                         color=l.get_color(),alpha=0.3)
                        l, = axy.plot(v._vslice_xdata,v._vslice_mean,label=k,**v.plot_options)
                        axy.fill_between(v._vslice_xdata,v._vslice_mean-v._vslice_std,v._vslice_mean+v._vslice_std,
                                         color=l.get_color(),alpha=0.3)
                    # refresh labels
...
def next_available_TRANSP_runid(runid, project, increment=1, skiplist=[], server=None):
    ...
    for k in lst:
        ...
        try:
            tmp = OMFITmdsValue(server=server, treename=tree, shot=runid,
                                TDI='\\' + tree.upper() + '::TOP.OUTPUTS.ONE_D.AIMP').data()
...
class OMFITplasmastate(OMFITnc):
    ...
    def to_omas(self, ods=None, time_index=0, update=['core_profiles', 'equilibrium', 'core_sources']):
        ...
        if 'core_profiles' in update:
            ...
            # define coordinate of input/output quantities
            coordsio = {'core_profiles.profiles_1d.%d.grid.rho_tor_norm' % time_index: self['rho']['data'][:-1] + np.diff(self['rho']['data']) / 2.}
        ...
        if 'core_sources' in update:
            ...
            def set_source(si, source_name,id_index, source_value, destination):
                ...
                coordsio = {}
                coordsio['core_profiles.profiles_1d.%d.grid.rho_tor_norm' % (time_index)] = self['rho']['data'][:-1] + np.diff(self['rho']['data']) / 2.
                coordsio['core_sources.source.%d.profiles_1d.%d.grid.rho_tor_norm' % (si, time_index)] = self['rho']['data'][:-1] + np.diff(self['rho']['data']) / 2.
                with omas_environment(ods, coordsio=coordsio, cocosio=cocosio):
            ...
            vol = interp1d(self['rho']['data'], self['vol']['data'])(self['rho']['data'][:-1] + np.diff(self['rho']['data']) / 2.)

            # assign data

            prof1d['grid.volume'] = vol

            volumetric_electron_heating_terms = {
                'pohme':7, 'pbe':2, 'peech':3, 'pmine':5 , 'pfuse':6}
            volumetric_ion_heating_terms = {
                'pbi': 2, 'picth':5, 'pmini':5, 'pfusi':6}
...
class OMFITfbm(OMFITnc):
    ...
    def plot(self, rf=None, zf=None, cartesian=True, fig=None):
        ...
        if fig is None:
            fig = pyplot.gcf()
        fig.suptitle('{}'.format(self['TOKAMAK']['data']) + ' {}'.format(self['TRANSP_RUNID']['data']) + ' t={0:0.2f}s'.format(float(self['TIME']['data'])))
    ...
    def plot_energy_space(self, rmin, rmax, zmin, zmax, emin=0 * 1E3, emax=1000 * 1E3, cartesian=True, ax=None):
        ...
        levels = [np.amin(total_f[:, emini:emaxi] / k),

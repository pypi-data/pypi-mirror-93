...
class _OMFITauxiliary(dict):
    def __new__(cls, *args, **kwargs):
        instance = super(_OMFITauxiliary, cls).__new__(cls, *args, **kwargs)
        instance.defaults = {}
        return instance
    ...
    def __getitem__(self, key):
        ...
        if key == 'lastBrowsedDirectory' and key in self:
            tmp = super(_OMFITauxiliary, self).__getitem__(key)
        ...
        return super(_OMFITauxiliary, self).__getitem__(key)
...
def sshOptions(sshName='OMFITssh'):
    ...
    if not os.path.exists(sshlink):
        ...
        try:
            ...
            pass
    return sshlink + ' -o StrictHostKeyChecking=no' \
                     ' -o CheckHostIP=no' \
                     ' -o Compression=yes' \
                     ' -o ForwardAgent=yes' \
...
def encrypt_credential(credential, password, otp, keys=None):
    ...
    '''
    if password == 'd3dpub':
        printd(f'Refusing to store credential {credential}, since password is {password}')
        return
...
def function_to_tree(funct, self_ref):
    ...
    return OMFITpythonTask(
        '{}.py'.format(name), fromString='# TEMPORARY FILE: EDITS WILL NOT BE SAVED!\n'
                                         '# This file was generated by omfit.omfit_classes.utils_base.function_to_tree()\n\n'
                                         'defaultVars('+args+'\n'+self_def+body)

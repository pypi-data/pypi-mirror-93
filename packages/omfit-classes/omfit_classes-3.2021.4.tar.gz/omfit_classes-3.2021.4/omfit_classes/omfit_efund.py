...
class OMFITmhdin(OMFITnamelist):
    ...
    def load(self, *args, **kw):
        ...
        super(OMFITnamelist, self).load(*args, **kw)
    ...
    def plot_flux_loops(self, display=None, colors=None, label=False, ax=None):
        ...
        if label:
            for k in range(len(x0)):
                ax.text(x0[k], y0[k], '\n ' + str(k), ha='left', va='top', size=8, color='w', zorder=1000, weight='bold', clip_on=True)
                ax.text(x0[k], y0[k], '\n ' + str(k), ha='left', va='top', size=8, color='b', zorder=1001, clip_on=True)
    ...
    def plot_magnetic_probes(self, display=None, colors=None, label=False, ax=None):
        ...
        for k in range(len(x0)):
            ...
            if colors is None:
                ...
                col = cm(c0[k])
            if l0[k] > 0:
                ax.plot(segx, segy, '-', lw=2, color=col, zorder=100, alpha=0.75)
                ax.plot(x0[k], y0[k], '.', color=col, zorder=100, alpha=0.75, mec='none')
            else:
                ax.plot(segx, segy, '.-', lw=2, color=col, zorder=100, alpha=0.75, mec='none')

        # labels
        if label:
            for k in range(len(x0)):
                ax.text(x0[k], y0[k], '\n ' + str(k), ha='left', va='top', size=8, color='w', zorder=1000, weight='bold', clip_on=True)
                ax.text(x0[k], y0[k], '\n ' + str(k), ha='left', va='top', size=8, color='r', zorder=1001, clip_on=True)
    ...
    def save(self, *args, **kw):
        ...
        try:
            ...
            super(OMFITnamelist, self).save(*args, **kw)
...
class OMFITdprobe(OMFITmhdin):
    @dynaLoad
    def load(self, *args, **kw):
        super(OMFITdprobe, self).load(*args, **kw)

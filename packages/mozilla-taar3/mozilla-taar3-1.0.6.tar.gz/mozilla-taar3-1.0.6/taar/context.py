...
chain.
"""
from taar.logs.interfaces import IMozLogging, ITAARCache
...
def _default_context(cache_cls=None, cache_singleton=False, logger_cls=None, log_level=None):
    ctx = Context()

    from taar.logs.stubs import LoggingStub
    from taar.recommenders.cache import TAARCache

    logger_cls = logger_cls or LoggingStub
    cache_cls = cache_cls or TAARCache
    ...
    ctx[ITAARCache] = cache_cls.get_instance(ctx) if cache_singleton else cache_cls(ctx)
    return ctx
...
def default_context(cache_cls=None, cache_singleton=False, logger_cls=None, log_level=None):
    ctx = _default_context(cache_cls, cache_singleton, logger_cls, log_level)

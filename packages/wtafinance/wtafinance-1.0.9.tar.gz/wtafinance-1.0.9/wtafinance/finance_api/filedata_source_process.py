...
class DataSource(BaseFunc):
    ...
    def init_body(self, body_data):
        ...
        for classUnit in self.file_head.class_subset:
            ...
            if i == self.file_head.class_count-1:
...
class MianClass():
    ...
    def get_data(self,code_list,fqt,date_time=None, system=None, custom=None):
        '''
        ...
        for param, paramVal in dp["param_data"].items():
            if paramVal is None:
                break
            is_sys = param == "system"
            dir = self.RootPath + ("/System" if is_sys else "/Custom")

            if(dp["code"] != "*"):
                for code in dp["code"]:

                    path = dir + ("/SH" if code[0] == "6" else "/SZ")
                    file_path = path + "/" + code + "." + fqt_data[str(fqt)]
                    p = {
                        "param_data": paramVal,
                        "is_sys": is_sys,
        ...
        if len(task_list)>10:
            ...
            for fut in as_completed(tasks):
                ...
                if is_sys:
                    ...
                    res_data["custom"].append(res_data_item)
        else:

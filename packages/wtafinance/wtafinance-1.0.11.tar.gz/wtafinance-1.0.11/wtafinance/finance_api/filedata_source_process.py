...
class ClassCodeConfig():
    config = [
        ...
        ,
        {
            "ClassCode": "1008",
            "ClassName": "五分钟数据",
            "TargetUnitCode": {'1008004': '最高价', '1008007': '成交额', '1008002': '开盘价', '1008005': '最低价', '1008008': '换手率'
                , '1008009': '振幅', '1008010': '涨跌幅', '1008003': '收盘价', '1008001': '时间', '1008006': '成交量'}
        }
    ]
...
class MianClass():
    ...
    def _get_data_path(self,data_type,ktype,fqt,code):
        ...
        :param ktype: [day,fzline,finance]
        :param fqt:
        :return:
        '''
        day_path ={
    ...
    def get_k_data(self,code,start,end='',fqt="0",ktype='day'):
        '''行情数据'''
    ...
    def get_Index_data(self, code,start,end='',fqt="0",ktype='day'):
        '''
        指数数据
        :param code:
        :param start:
    ...
    def get_stockIndex_data(self,code,base_code,start,end='',fqt="0",ktype='day',bFillNA=False):
        k_data = self.get_k_data(code,start,end,fqt,ktype)
        index_data = self.get_Index_data(base_code,start,end,fqt,ktype)
        base_stock_data = index_data.loc[:, ['日期', '收盘价']]
        base_stock_data.rename(columns={'收盘价': '指数收盘价'}, inplace=True)

        result = pd.concat([k_data.set_index('日期'), base_stock_data.set_index('日期')], axis=1).reset_index()

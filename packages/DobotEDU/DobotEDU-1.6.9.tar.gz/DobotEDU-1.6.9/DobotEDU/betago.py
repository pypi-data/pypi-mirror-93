...
class BetaGoApi(object):
    ...
    def set_stop_point_server(self, port_name, x, y):
        return self.__dobotlink.MagicianGO.SetStopPointServer(
            portName=port_name, PointX=x, PointY=y)

    @get_portname
    def set_stop_point_param(self, port_name, scope_err, stop_err):
        return self.__dobotlink.MagicianGO.SetStopPointParam(
            portName=port_name, scopeErr=scope_err, stopErr=stop_err)

    @get_portname
    def get_stop_point_state(self, port_name):
        return self.__dobotlink.MagicianGO.GetStopPointState(
            portName=port_name)
    ...
    def is_nearby(self, point, num=40, err=2):
        ''' 若在指定点附近则停车,默认周围大小为40cm,如果出现到点不动作得到情况调大area默认值,如果定位不准确减小err默认值'''
        self.set_stop_point_server(x=point[0], y=point[1])
        self.set_stop_point_param(scope_err=num, stop_err=err)
        time.sleep(0.01)
        while True:
    ...
    def _mul(self, A, b):
        return [self._dot(row, b) for row in A]

    def _imgh1_to_imgh2(self, Mx, My, need_tranxy, apriltag_h):
        '''物体平面与标定平面图像坐标转换'''
        if need_tranxy:
            p_x_camera = 223 + 48  # 摄像头中心所在的机器x坐标
            p_y_camera = 0  # 48是末端到摄像头中心的距离
        else:
            p_x_camera = 0
            p_y_camera = -223 - 48
        x1 = Mx - p_x_camera
        y1 = My - p_y_camera
        self.theta_h = apriltag_h - 5  # 5为标定平面高度，后续改为10(小方块标定高度)
        tran_Mx = x1 * (1 - self.theta_h / self.camera_h) + p_x_camera
        tran_My = y1 * (1 - self.theta_h / self.camera_h) + p_y_camera
        return tran_Mx, tran_My
    ...
    def imgxy_to_armxy(self,
        ...
        '''图像坐标转机器坐标'''
        img_point = [px, py, 1]
        q = self._mul(self.calibra_h_car, img_point)
        dobot_point = [qt / q[-1] for qt in q[:-1]]
        Mx = dobot_point[0]
        My = dobot_point[1]
        if need_tranxy:  # 在仓库置物盒上方，需要坐标转换
            temp = Mx
            Mx = -My
            My = temp
        if suck_apriltag:
            Mx, My = self._imgh1_to_imgh2(Mx, My, need_tranxy, apriltag_h)
        return Mx, My
    ...
    def grab_obj_cartofloor(self, object_class=0):
        ...
        global move_x0, move_y0, move_time
        global object_id
        detect_flag = True
        need_trans = False
        NoObj_time = 0
                ...
                for i in range(datalist['count']):  # 取第一个目标物体进行抓取
                    if datalist['dl_obj'][i]['id'] > index_1 and datalist[
                        ...
                        while datalist['count'] == 0 and NoObj_time < 5:
                            ...
                            NoObj_time += 1
                        if datalist[
                                'count'] == catch_num:  # 目前置物盒物体数量是否等于已抓取数量
                            thetaX += 60  # 设置下次放置点
                        else:  # 如果数量和不等于抓取数量（可能未成功抓取，或者漏检测物体），则机械臂往里面走10mm再次检测
                            move_x0 = 0
                            move_y0 = 10
                            self._set_ptpcmd(1, p_x0 + move_x0, p_y0 + move_y0,
                                             p_z0, p_R0)  # 小车置物盒上方零位点
                            time.sleep(0.5)
                            datalist = self.get_arm_camera_obj()
                            NoObj_time = 0
                            while datalist['count'] == 0 and NoObj_time < 5:

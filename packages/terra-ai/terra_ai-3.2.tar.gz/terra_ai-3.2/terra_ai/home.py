...
def параметризация_аудио(файл):
  ...
  spectral_centroids = librosa.feature.spectral_centroid(y_org, sr=sr)[0]
  #Вычисляем временя для визуализации
  ...
  plt.figure(figsize=(14, 5))#задаем размер
  librosa.display.waveplot(y_org, sr=sr, alpha=0.4) #Построим амплитуду сигнала
  ...
  print()
  print('Спектральный спад частоты:')
  # Вычисляем и отображаем спектральный спад частоты   
  plt.figure(figsize=(14, 5))#задаем размер
  spectral_rolloff = librosa.feature.spectral_rolloff(y_org, sr=sr, roll_percent=0.85)[0]
  librosa.display.waveplot(y_org, sr=sr, alpha=0.4) # Построим амплитуду сигнала

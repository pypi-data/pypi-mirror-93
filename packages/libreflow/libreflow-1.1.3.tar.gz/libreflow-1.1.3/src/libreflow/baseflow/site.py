...
class MinioFileUploader(PythonRunner):
    ...
    def argv(self):
        args = ["C:/Users/Baptiste/dev/LibreFlow/libreflow-git/src/libreflow/scripts/minio_file_uploader.py"]
...
class MinioFileDownloader(PythonRunner):
    ...
    def argv(self):
        args = ["C:/Users/Baptiste/dev/LibreFlow/libreflow-git/src/libreflow/scripts/minio_file_downloader.py"]
        args += self.extra_argv
        return args
...
class Request(flow.Action):

    _revision = flow.Parent()
    priority = flow.IntParam(50)
    site_name = flow.Param(None, SiteChoiceValue).ui(label="Site to query")

    def get_buttons(self):
        site_names = self.site_name.choices()
        # Actually, an existing revision implies there is at least one
        # site on which it is available, which is the source site
        self.site_name.set(site_names[0])
    ...
    def run(self, button):
        ...
        # Check if the version is not already present
        current_site = self.root().project().get_current_site()
        current_site_status = self._revision.sync[current_site.name()]

        # Get requested site
        sites = self.root().project().admin.sites
        requested_site = sites[self.site_name.get()]
        
        # Add a download job for the requesting site
        current_site.queue.submit_job(
            job_type="Download",
            init_status="WAITING",
            emitter_oid=self._revision.oid(),
            user=self.root().project().get_user(),
            studio=current_site_status.name(),
            priority=self.priority.get(),
        )
        self._revision.set_sync_status("Requested")
        ...
        for job in requested_site.queue.mapped_items():
            if job.emitter_oid.get() == self._revision.oid() and job.type.get() == "Upload":
                self.root().session.log_warning(

...
class TrackedFile(baseflow.file.TrackedFile):
    ...
    def get_playblast_folder(self):
        settings = get_contextual_dict(
            self._department, "settings", ["path"]
        )
        return os.path.join(self.root().project().get_root(), settings["path"], "preview")
    
    def get_render_path(self):
        settings = get_contextual_dict(
            self._department, "settings", ["path"]
        )
        return os.path.join(self.root().project().get_root(), settings["path"], "render")

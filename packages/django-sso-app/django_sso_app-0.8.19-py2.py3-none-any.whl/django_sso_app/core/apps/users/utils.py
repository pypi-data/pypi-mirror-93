...
def add_user_to_default_groups(user):
    setattr(user, '__dssoa__user_loggin_in', True)  # do not update rev

    user_groups = user.groups.values_list('name', flat=True)
    for gn in app_settings.DEFAULT_USER_GROUPS:
        if gn not in user_groups:
            group, _created = Group.objects.get_or_create(name=gn)

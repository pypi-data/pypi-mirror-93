...
def load_addons(plugin_name):
    if plugin_name.startswith("__"):
        ...
        path = Path(f"ultroid/addons/{plugin_name}.py")
        name = "ultroid.addons.{}".format(plugin_name)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
    else:
        ...
        spec.loader.exec_module(mod)
        sys.modules["ultroid.addons." + plugin_name] = mod
        try:
...
def load_pmbot(plugin_name):
    if plugin_name.startswith("__"):
        ...
        path = Path(f"ultroid/assistant/pmbot/{plugin_name}.py")
        name = "ultroid.assistant.pmbot.{}".format(plugin_name)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
    else:
        ...
        mod.asst_cmd = asst_cmd
        spec.loader.exec_module(mod)
        sys.modules["ultroid.assistant.pmbot" + plugin_name] = mod

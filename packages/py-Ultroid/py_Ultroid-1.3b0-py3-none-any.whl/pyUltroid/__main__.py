...
# for addons
if Var.ADDONS:
    os.system("git clone https://github.com/xditya/UltroidAddons.git ./ultroid/addons/")
    print("Installing packages for addons")
    os.system("pip install -r ./ultroid/addons/addons.txt")
    path = "ultroid/addons/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as a:
            patt = Path(a.name)
            ...
            if not plugin_name.startswith("__") or plugin_name.startswith("_"):
                print(f"Ultroid - Installed - Addon - {plugin_name}")
...
if Var.MSG_FRWD is True:
    path = "ultroid/assistant/pmbot/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as a:
            patt = Path(a.name)

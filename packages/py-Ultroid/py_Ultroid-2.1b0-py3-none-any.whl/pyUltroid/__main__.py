...
for name in files:
    with open(name) as a:
        ...
        try:
            ...
            print(f"Ultroid - Install - ERROR - {plugin_name}")

# for addons
if Var.ADDONS:
    os.system(
        "git clone https://github.com/xditya/UltroidAddons.git ./addons/")
    ...
    for name in files:
        with open(name) as a:
            ...
            try:
                ...
                if not plugin_name.startswith("__") or plugin_name.startswith("_"):
                    print(f"Ultroid - Installed - Addons - {plugin_name}")
            except BaseException:
                print(f"Ultroid - Addons - Install - ERROR - {plugin_name}")
...
for name in files:
    with open(name) as a:
        ...
        try:
            ...
            print(f"Ultroid - Assistant - Install - ERROR - {plugin_name}")
...
async def plug():
    if PLUG is not None:
        chat = int(PLUG)
        plugins = await ultroid_bot.get_messages(chat, None , filter=InputMessagesFilterDocument)
        total = int(plugins.total)
        totals = range(0, total)
        for ult in totals:
            uid = plugins[ult].id
            file = await ultroid_bot.download_media(await ultroid_bot.get_messages(chat, ids=uid), "./addons/")
            if "(" not in file:
                upath = Path(file)
                name = upath.stem
                try:
                    load_addons(name.replace(".py", ""))
                    print(f"Ultroid - Installed - {(os.path.basename(file))}")
                except BaseException:
                    print(f"Ultroid - ERROR - {(os.path.basename(file))}")
            else:

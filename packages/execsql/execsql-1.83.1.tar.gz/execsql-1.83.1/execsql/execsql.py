...
class WriteableZipfile(object):
	def __init__(self, zipfile_name):
		if sys.version_info.major < 3 or (sys.version_info_major >=3 and sys.version_info.minor < 3):
			self.zf = zipfile.ZipFile(zipfile_name, mode='a', compression=zipfile.ZIP_STORED, allowZip64=True)
		elif sys.version_info_major >=3 and sys.version_info.minor < 7:
			self.zf = zipfile.ZipFile(zipfile_name, mode='a', compression=zipfile.ZIP_BZIP2)
		else:
			self.zf = zipfile.ZipFile(zipfile_name, mode='a', compression=zipfile.ZIP_BZIP2, compresslevel=9)
	def close(self):
		self.zf.close()
...
def x_export(**kwargs):
	...
	if tee and zipfilename is not None:
		raise ErrInfo("error", other_msg="Cannot append to a file within a zipfile")
	if zipfilename is not None and length(outfile) > 1 and outfile[1] == ":":

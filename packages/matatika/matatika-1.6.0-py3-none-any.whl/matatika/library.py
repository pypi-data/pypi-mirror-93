...
class MatatikaClient():
    ...
    def fetch(self, dataset_id_or_alias: str, data_format: DataFormat = None) \
        ...
        data = client.fetch(dataset_id_or_alias, data_format=DataFormat.CHARTJS)
        ...
        if VALID_UUID4.fullmatch(dataset_id_or_alias):
            dataset = Dataset.from_dict(
                catalog.get_dataset(dataset_id_or_alias))
        else:
            if not self.workspace_id:
                raise VariableNotSetError('workspace_id')
            dataset = Dataset.from_dict(
                catalog.get_workspace_dataset(dataset_id_or_alias))

        data = catalog.get_data(dataset.dataset_id)

...
from tests.unittest_base import MOCK_DATASET_RESPONSE
...
class TestCLIGet(TestCLI):
    ...
    def test_get_dataset(self, mock_get_request):
        ...
        mock_get_request.return_value.json.return_value = MOCK_DATASET_RESPONSE

        result = self.runner.invoke(matatika, ["get",
                                               "dataset",
                                               str(uuid.uuid4())])

        self.assertIn(json.dumps(MOCK_DATASET_RESPONSE), result.output)

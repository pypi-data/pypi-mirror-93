...
from tqsdk.utils import _generate_uuid, _query_for_quote, _symbols_to_quotes
    ...
    **注意** ：如果未订阅 quote，模拟交易在下单时会自动为此合约订阅 quote ，根据回测时 quote 的更新规则，如果此合约没有订阅K线或K线周期大于分钟线 **则会自动订阅一个分钟线** 。
                ...
                if symbols_diff:
                    ...
                    for query_id, query_result in symbols_diff.items():
                        if query_id.startswith("PYSDK_quote") and query_result.get("error", None) is None:
                            quotes = _symbols_to_quotes(query_result, {"instrument_id", "price_tick"})
        ...
        if self._pending_peek:
            ...
            if not self._diffs:
                while self._requests:
                    ...
                    self._diffs.append(self._requests[min_request_key]["diff"])
        ...
        async with TqChan(self._api, last_only=True) as update_chan:
            ...
            try:
                async for _ in update_chan:
                    ...
                    if left_id == -1 or right_id == -1 or chart.get("more_data", True):
                    ...
                    while True:
                        ...
                        if dur == 0:
                            ...
                            rangeset = self._sended_to_api.setdefault((symbol_list[0], dur), [])
                            self._sended_to_api[(symbol_list[0], dur)] = _rangeset_range_union(rangeset, (
                            ...
                            for i, symbol in enumerate(symbol_list):
                                ...
                                if other_id >= 0:
                                    ...
                                    rangeset = self._sended_to_api.setdefault((symbol, dur), [])
                                    self._sended_to_api[(symbol, dur)] = _rangeset_range_union(rangeset,
                                                                                               (other_id, other_id + 1))
    ...
    def _gc_data(self):
        ...
        for ins, dur in self._requests:
            ...
            last_id = self._sended_to_api[(symbol_list[0], dur)][-1][-1] - 1

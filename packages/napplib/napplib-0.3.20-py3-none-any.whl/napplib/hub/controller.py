...
class HubController:
		...
		for product in products:
			...
			if r:
				...
				if hub_product_id != 0:
					...
					logging.info(f'Criando o produto... [{"sucesso ao registrar o produto (200)" if response.status_code == 200 else response.status_code}]')
	...
	def get_order_by_foreign_id(self, server_url='', token='', orderForeignId='', storeId='', purchasedAt=''):
		...
		url = f'{server_url}/orderByForeignId/?foreignId={orderForeignId}&storeId={storeId}&purchasedAt={purchasedAt}'
	...
	def create_order_invoice(self, server_url='', token='', invoice=''):
		...
		r = r.content.decode('utf-8')
		logging.info(f'Creating invoice {invoice.get("orderId")["Int64"]}...{r}')
	...
	def create_order_shipping(self, server_url='', token='', shipping=''):
		...
		r = r.content.decode('utf-8')
		logging.info(f'Creating shipping {shipping.get("orderId")["Int64"]}...{r}')
	...
	def update_order_shipping(self, server_url='', token='', shipping=''):
		...
		payload = json.dumps(shipping)
		r = requests.put(f'{server_url}/orders/', headers=headers, data=payload)
		logging.info(f'Updating order {shipping.get("orderId")["Int64"]}...{r.status_code}')
	...
	def create_order_payment(self, server_url='', token='', payment=''):
		...
		r = r.content.decode('utf-8')
		logging.info(f'Creating payment {payment.get("orderId")}...{r}')

...
%amp=input('Enter the amplitude of Carrier and Modulating signal');
...
fc1=input('Enter the freq of 1st Sine Wave carrier:'); %20HZ
fc2=input('Enter the freq of 2nd Sine Wave carrier:');%40HZ
...
tdmsig=reshape(sig,1,2*l1); % Display of TDM Signal 
figure(2)
stem(tdmsig);
title('TDM Signal');
ylabel('Amplitude--->');
xlabel('Time--->'); % Demultiplexing of TDM Signal
...
close all
samples=100;
nos=3;% number of users
mfreq=[30 40 50]; % modulating signal frequency in Hz
cfreq=[300 600 900];% carrier frequency allocated to the different users in
Hz
freqdev=100; % choose frequency deviation
% generate modulating signal
t=linspace(0,1000,samples); 
for i=1:nos
 m(i,:)=sin(2*pi*mfreq(1,i)*t);
end
...
figure
C = {'k','b','r','g','y',[.5 .6 .7],[.8 .2 .6],[.3 .2 .2]}; % Cell array of
colros.
for i=1:nos
 figure (1)
 hold on

...
from polyaxon.client.decorators import can_log_events, can_log_outputs, check_no_op
...
class Run(RunClient):
    ...
    def __init__(
        ...
        self._set_exit_handler(is_new_instance=False)
    ...
    def _set_exit_handler(self, is_new_instance: bool = False):
    ...
    def _start(self):
        if settings.CLIENT_CONFIG.is_offline:
            ...
            logger.info(f"A new offline run started at {self._artifacts_path}")
        self.start()
        ...
        def excepthook(exception, value, tb):
            self.log_failed(
                ...
                traceback="Type: {}, Value: {}".format(exception, value),
            )
            # Resume normal work
    ...
    def _end(self):
        ...
        if settings.CLIENT_CONFIG.is_offline:
            self._persist_offline_run(artifacts_path=self._artifacts_path)

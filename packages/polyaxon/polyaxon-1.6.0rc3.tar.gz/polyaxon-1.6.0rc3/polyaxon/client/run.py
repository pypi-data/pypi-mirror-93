...
class RunClient:
    ...
    def end(
        ...
        if self.status in LifeCycle.DONE_VALUES:
            return
        self.log_status(
            status=status, reason=reason, message=message, traceback=traceback
        )
        time.sleep(
            0.1
    ...
    @check_no_op
    def log_failed(
        self, reason: str = None, message: str = None, traceback: str = None
    ):
    ...
    @check_no_op
    def _persist_offline_run(self, artifacts_path: str):
        ...
        with open(run_path, "w") as config_file:
            config_file.write(ujson.dumps(self.run_data.to_dict()))
        ...
        with open(lineages_path, "w") as config_file:
            config_file.write(
                ujson.dumps([l.to_dict() for l in self._lineages.values()])
            )

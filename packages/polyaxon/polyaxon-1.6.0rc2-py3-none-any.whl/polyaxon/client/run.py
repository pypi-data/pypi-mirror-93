...
class RunClient:
    ...
    def __init__(
        ...
        if settings.CLIENT_CONFIG.is_offline and not self._run_uuid:
            self._run_uuid = self._run_uuid or uuid.uuid4()
    ...
    def _update(
        ...
        if settings.CLIENT_CONFIG.is_offline:
            self._offline_data.update(data)
    ...
    def sync_events_summaries(
        self, last_check: Optional[datetime], artifacts_path: str
    ):
        ...
        for events_kind in get_dirs_under_path(artifacts_path):
            _summaries, _last_values = self._sync_events_summaries(
                events_path=artifacts_path,

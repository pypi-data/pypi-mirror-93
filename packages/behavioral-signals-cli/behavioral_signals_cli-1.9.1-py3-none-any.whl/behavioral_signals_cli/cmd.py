...
async def submit_from_csv_async(csv_file, pid_file, tag=None, nchannels=1, max_async_sends=MAX_ASYNC_UPLOADS):
            ...
            async with aiofiles.open(pid_file, 'w') as pidsfile:
                with open(csv_file, 'r') as f:
                    ...
                    for row in reader:
                        ...
                        filepath, data = get_audio_metadata(row, tag, nchannels)
                        ...
                        if last_time_called - initial_time_called < REQUESTS_LIMIT_PERIOD_SECS and counter > MAX_REQUESTS_PER_SEC:
                            await asyncio.sleep(REQUESTS_LIMIT_PERIOD_SECS)
                            counter = 0
                        elif last_time_called - initial_time_called > 1:
                            initial_time_called = last_time_called
                            counter = 0
    ...
    with open(csv_file, 'r') as f:
        with open(pid_file, 'w') as p:
            ...
            for row in reader:
                ...
                try:
                    filename, data = get_audio_metadata(row, tag, nchannels)
...
def get_audio_metadata(row, tag, nchannels):
    DATA_FIELDS = [

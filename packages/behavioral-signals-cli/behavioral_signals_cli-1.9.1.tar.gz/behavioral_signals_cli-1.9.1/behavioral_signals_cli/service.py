...
class OliverAPI():
    ...
    def GetDiarizationResults(self, pid, output_dir=None, dest=None):
        ...
        try:
            results = self.api.get_process_results_diarization(self.opts.apiid, pid)
            results = results.to_dict()
        except ValueError as e:
    ...
    def getInfoAsync(self, pids, poll=False):
        ...
        while True:
            for pid in polling_pids:
                resp = self.get_process_info_async(pid)
    ...
    def GetResultsAsync(self, processes, output_dir=None):
        ...
        for pid in processes:
            response = self.api.get_process_results(self.opts.apiid, pid, async_req=True)
        ...
        for pid, response in responses.items():
            try:
                ...
                # if str(e) == "Invalid value for `basic`, must not be `None`":
                #     logging.warning("{}: No behavioural results have been calculated for process".format(pid))
                # else:
                #     logging.warning("{}: Process failed: {}".format(pid, e))
    ...
    def GetFrameResultsAsync(self, processes, output_dir=None):
        ...
        for pid in processes:
            response = self.api.get_process_results_frames(self.opts.apiid, pid, async_req=True)
            responses[pid] = response
    ...
    def GetASRResultsAsync(self, processes, output_dir=None):
        ...
        for pid in processes:
            response = self.api.get_process_results_asr(self.opts.apiid, pid, async_req=True)
            responses[pid] = response
    ...
    def GetDiarizationResultsAsync(self, processes, output_dir=None):
        ...
        for pid in processes:
            response = self.api.get_process_results_diarization(self.opts.apiid, pid, async_req=True)
            responses[pid] = response
        ...
        for pid, response in responses.items():
            try:
                ...
                # if str(e) == "Invalid value for `tokens`, must not be `None`":
                #     logging.warning("{}: No Diarization output".format(pid))
                # else:
                #     logging.warning("{}: Diarization failed: {}".format(pid, e))
                noresult_pids.append(pid)
                results = None
            except Exception as e:
                if e.status == 405 or e.status == 503:
    ...
    def GetFeaturesResultsAsync(self, processes, output_dir=None, dest=None):
        ...
        for pid in processes:
            response = self.api.get_process_results_features(self.opts.apiid, pid, async_req=True)
            responses[pid] = response
    ...
    @sleep_and_retry
    @limits(calls=MAX_REQUESTS_PER_SEC_PER_ENDPOINT, period=REQUESTS_LIMIT_PERIOD_SECS)
    def get_process_info_async(self, pid):
        return self.api.get_process_info(self.opts.apiid, pid, async_req=True)

...
class SQLAlchemyPostgresql(SQLAlchemy):
    ...
    def _create_engine(self, options: Dict) -> Tuple[str, object]:
        ...
        connection_uri = (
            f"postgresql+psycopg2://{options.get('username', None)}:{options.get('password', None)}@{options.get('host', None)}:{options.get('port', None)}}",
        )
        # To see commit statement: add echo=True, echo_pool=True
        engine = create_engine(connection_uri)
        # Create db if missing
        self._create_database(engine=engine, name=options.get("db_name", None))
        self._dispose(engine=engine)

        connection_uri = f"{connection_uri}/{options.get('db_name',None)}"
        engine = create_engine(connection_uri)
        return connection_uri, engine

    def _create_database(self, engine:object = None, db_name: str = "foobar", encoding="UTF8"):
        connection = None
        try:

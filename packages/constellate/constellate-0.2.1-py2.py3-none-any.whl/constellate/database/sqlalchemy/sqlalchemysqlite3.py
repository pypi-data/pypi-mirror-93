...
class SQLAlchemySqlite3(SQLAlchemy):
    ...
    def vacuum(self):
        ...
        with raw_sqlite3_connection(
            self._engine, issue_begin=True, issue_commit_or_rollback=True, close=True
        ) as connection:
            # for pragma in ["pragma wal_checkpoint(TRUNCATE);", "pragma vacuum;",
            # "pragma optimize;", "pragma integrity_check;", "pragma
            # foreign_key_check;"]:
    ...
    def backup(self, options: Dict = {}):
        ...
        def progress(status, remaining, total):
            self._logger.debug(f"{status}: Backing up {total - remaining} of {total} pages...")

...
class StandardFunction:
    ...
    def ASI(time_:np.ndarray, acc_x:np.ndarray,
        ...
        acceleration_x_average = np.convolve(acc_x.flatten(), np.ones(N) / N, mode='same').reshape(-1, 1)
        acceleration_y_average = np.convolve(acc_y.flatten(), np.ones(N) / N, mode='same').reshape(-1, 1)
        acceleration_z_average = np.convolve(acc_z.flatten(), np.ones(N) / N, mode='same').reshape(-1, 1)

        from scipy import signal
        sos = signal.butter(2, 13, 'lowpass', fs=1000, output='sos')
        acceleration_x_butter = signal.sosfilt(sos, acc_x.flatten()).reshape(-1, 1)
        acceleration_y_butter = signal.sosfilt(sos, acc_y.flatten()).reshape(-1, 1)
        acceleration_z_butter = signal.sosfilt(sos, acc_z.flatten()).reshape(-1, 1)
        #acceleration_y_butter = np.convolve(acc_y.flatten(), np.ones(N) / N, mode='same').reshape(-1, 1)
        #acceleration_z_butter = np.convolve(acc_z.flatten(), np.ones(N) / N, mode='same').reshape(-1, 1)

        plt.plot(acc_x)
        plt.plot(acceleration_x_average)
        plt.plot(acceleration_x_butter)
        plt.show()

        return np.sqrt((acceleration_x_butter/limit_acc_x)**2
                       + (acceleration_y_butter/limit_acc_y)**2
                        + (acceleration_z_butter/limit_acc_z)**2)

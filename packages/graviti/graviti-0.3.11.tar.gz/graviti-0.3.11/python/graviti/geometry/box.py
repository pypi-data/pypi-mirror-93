...
from ..utility import ReprBase, ReprType, UserSequence, common_loads
...
class Box2D(UserSequence[float]):
    ...
    def loads(cls: Type[_T], loads: Dict[str, float]) -> _T:
        """Load a Box2D from a list of dict containing coordinates of the 2D box.

        :param loads: A list of dict containing coordinates of a 2D box
        [
            {
                "x": ...
                "y": ...
    ...
    def _loads(self, loads: List[Dict[str, float]]) -> None:
        self._data = (loads[0]["x"], loads[0]["y"], loads[1]["x"], loads[1]["y"])
    ...
    def _dumps(self) -> Dict[str, float]:
...
class Box3D(ReprBase):
    ...
    def loads(cls: Type[_T], loads: Dict[str, float]) -> _T:
        ...
        :param loads: A dict containing coordinates of a 3D box
        {
            "size": {
                "x": ...
                "y": ...

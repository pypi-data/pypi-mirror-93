...
class DroneHandler:
    ...
    def get_latest_production_deploy(self):
        with Halo(text="Contacting drone...", spinner="dots", color="magenta"):
            REGEX = r"deploy-([a-z]+)-production"
            try:
                builds = requests.get(
                    f"{BASEURL}/api/repos/primait/{self._repo}/builds?per_page=100",
                    headers={"Authorization": f"Bearer {self._token}"},
                ...
                builds = [
                    {
                        "country": re.findall(REGEX, b["deploy_to"])[0],
                        "version": b["params"]["DRONE_TAG"],
                    }
                    for b in builds
                    if b["event"] == "promote" and re.search(REGEX, b["deploy_to"])
                ]
                return builds
            except Exception as e:

...
class Release:
    ...
    def __init__(self, project, deploy, config, tokens):
    ...
    def run(self):
        ...
        if self._deploy:
            self._start_deploy(new_version)
    ...
    def _start_deploy(self, version):
        current = self._drone.get_latest_production_deploy()
        if current:
        ...
        for country in countries:
            if not version:
                releases = self._get_latest_releases_since(
                    [rel["version"] for rel in current if rel["country"] == country]
                )
            ...
            if "number" not in promotion:
                logger.warning(
                    f"Unable to promote production build. Response: {promotion}"
                )
                return

            logger.info("Deploy started successfully!")
            logger.info(
                f"You can follow the deploy status at {self._drone.get_build_url(promotion['number'])}"
            )

            commits = self._github.get_commits_since_release(self._repo, version)
            self._manage_youtrack_card(commits, version, country)

    def _get_latest_releases_since(self, threshold):
    ...
    def _manage_youtrack_card(self, commits, new_version, country):
            ...
            for issue_id in issue_ids:
                try:
                    self._youtrack.comment(
                        issue_id,
                        f"Deploy in production of {self._project} in country {country} done with the release {new_version}",
                    )

...
class CPManager:
    ...
    def get_manifest(self, match_id: str, virtual_cam_id: int):
    ...
    def upload_manifest(self, manifest: dict, match_id: str, virtual_cam_id: int):
        for i in range(5):
            try:
                manifest_path = f"{match_id}/camera_positions/{virtual_cam_id}/cp.json"
                blob = self.bucket.blob(manifest_path)
    ...
    def upload_data(self, cam_data: bytes, match_id: str, virtual_cam_id: int):
        ...
        for time_id in time_idx_set:
            ...
            for i in range(5):
                try:
                    blob.upload_from_string(cam_data)
                    break
                except TooManyRequests:
                    pass

        new_max = int(max(time_idx_set))
        manifest = self.get_manifest(match_id, virtual_cam_id)
        if manifest:
        ...
        self.upload_manifest(manifest, match_id, virtual_cam_id)

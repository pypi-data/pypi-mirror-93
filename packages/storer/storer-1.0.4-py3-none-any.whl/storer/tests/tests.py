...
class TestsStorer(unittest.TestCase):
    ...
    def test_loop_loading(self):
        ...
        """
        s = Storer(path_dumps=PATH_DUMPS, dump_name=DUMP_NAME, verbose=True, compressed=False)
        for idx in range(10): 
        ...
        s2 = Storer(path_dumps=PATH_DUMPS, dump_name=DUMP_NAME, verbose=True, compressed=False)
        s2.load()
        for idx in range(10): 
            value = s.get(name=str(idx))
            assert value == idx, f"Got something different: {value}"

	...
	async def on_message(message):
		if not message.author.bot:
			...
			def remember(text):
				bot._nowcmd=text
				return True
			if any([message.content[:len(max(bot._cmds, key=len))].startswith(bot._codes["command"][cmd]["prefix"]+bot._codes["command"][cmd]["name"]) and remember(cmd) for cmd in bot._codes["command"]]):

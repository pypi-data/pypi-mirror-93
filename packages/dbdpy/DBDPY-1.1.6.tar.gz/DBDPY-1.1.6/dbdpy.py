...
def _start(client, bot):
	...
	async def replaces(line, message, guild, content, msg, vars, timerep, author):
		...
		while "$getSite"+bot._style[0] in line:
			if nowloop==line:
				break
			else:
				nowloop=line
				try:
					r=str(requests.get(line.split("$getSite"+bot._style[0])[-1].split(bot._style[1])[0].split(bot._separator)[0]).text)
					for i in range(len(r)//2000):
						r=r[i:i+1*(2000)]
					line=line.replace("$getSite"+bot._style[0]+line.split("$getSite"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],str(r))
				except Exception as e:
					line=line.replace("$getSite"+bot._style[0]+line.split("$getSite"+bot._style[0])[-1].split(bot._style[1])[0]+bot._style[1],"none")
	...
	async def on_message(message):
		if not message.author.bot:
			if str(message.content).startswith(bot._client.user.mention):
				if bot._help:
					await message.channel.send(embed=discord.Embed(title="Комманды("+str(len(bot._cmds)-1)+")",description=f"```py\n"+"\n".join(bot._cmds)+"\n```"))
			def remember(text):
				bot._nowcmd=text
				return True
			if any([message.content[:len(max(bot._cmds, key=len))].startswith(get_vars(bot._codes["command"][cmd]["prefix"].replace("$authorID",str(message.author.id)).replace("$serverID",str(message.guild.id)), message.guild, {})+get_vars(bot._codes["command"][cmd]["name"].replace("$authorID",str(message.author.id)).replace("$serverID",str(message.guild.id)), message.guild, {})) and remember(cmd) for cmd in bot._codes["command"]]):
				await use(message, bot._nowcmd, get_vars(bot._codes["command"][bot._nowcmd]["prefix"].replace("$authorID",str(message.author.id)).replace("$serverID",str(message.guild.id)), message.guild, {})+get_vars(bot._codes["command"][bot._nowcmd]["name"].replace("$authorID",str(message.author.id)).replace("$serverID",str(message.guild.id)), message.guild, {}), "command")
	if bot._self:

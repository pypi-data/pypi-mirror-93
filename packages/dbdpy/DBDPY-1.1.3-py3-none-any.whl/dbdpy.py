	...
	async def on_message(message):
		if not message.author.bot:
			...
			def remember(text):
				bot._nowcmd=text
				return True
			if any([message.content[:len(max(bot._cmds, key=len))].startswith(await replaces(bot._codes["command"][cmd]["prefix"], message, message.guild, message.content, message, {}, 0, message.author)+await replaces(bot._codes["command"][cmd]["name"], message, message.guild, message.content, message, {}, 0, message.author)) and remember(cmd) for cmd in bot._codes["command"]]):

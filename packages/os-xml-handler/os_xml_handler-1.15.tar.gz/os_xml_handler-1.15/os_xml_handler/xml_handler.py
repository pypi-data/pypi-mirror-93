...
def get_child_nodes(node_parent, node_tag, node_att_name=None, node_att_val=None, namespace_map=None):
    selector = node_tag
    return find_all_nodes(node_parent, node_tag, node_att_name, node_att_val, recursive=False, namespace_map=namespace_map)


# will search in all of the direct children of the root
def get_root_direct_child_nodes(xml_file, node_tag, node_att_name=None, node_att_val=None, namespace_map=None):
    return get_child_nodes(get_root_node(xml_file), node_tag, node_att_name, node_att_val, namespace_map)


# will return a list of nodes which doesn't have a specific attribute
def get_nodes_from_xml_without_att(xml_file, node_tag, node_att_name=None, namespace_map=None):
...
def create_and_add_new_node(parent_node, node_tag, att_val_dict=None, node_text=None, namespace_map=None):
    ...
    node = etree.SubElement(parent_node, node_tag, node_att_val_dict)
...
#             if len(xml1_direct_children) > 0:
#                 add_nodes(parent_node, xml1_direct_children)
#             else:
#                 set_node_text(parent_node, f'{get_text_from_node(xml1_child)}\n{get_text_from_node(parent_node)}')
#         else:
...
def find_all_nodes(parent_node, node_tag, node_att_name=None, node_att_val=None, recursive=True, namespace_map=None):
    ...
    if recursive:
        ...
        return parent_node.findall(selector, namespace_map)

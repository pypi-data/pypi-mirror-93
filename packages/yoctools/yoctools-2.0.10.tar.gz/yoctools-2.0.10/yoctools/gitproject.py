...
class GitRepo:
        ...
        try:
            ...
            if version not in self.repo.heads:
                ...
                if version in origin.refs:
                    ...
                    branch = self.repo.create_head(version, self.repo.tags[version])
                    branch.set_tracking_branch(origin.refs[version])
                    branch.checkout()
            else:
                origin.pull(version)
    ...
    def GetRemoteBranches(self, remote=''):
        br_arr = []
        try:
    ...
    def commit(self, message=''):
        ...
        if self.repo.is_dirty(untracked_files=True):
            try:
                ...
                remote.push()
                put_string("Commit [%s]         ok." % name)
            except Exception as ex:
                put_string("Commit [%s]         failed(%s)." % (name, str(ex)), level='warning')

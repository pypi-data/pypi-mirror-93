    ...
    def download(self, branch=''):
        if True:
            if self.check_git_integrity():
               put_string('Component %s is installed, skip to install it.' % self.name, level='warning')
               return
            if self.version:
                branch = self.version
            elif not branch:
            ...
            if len(branches) == 0:
                put_string('There is no branch found for component:%s in remote.' % self.name, level='warning')
                return

...
# 画图基础通用参数；
class Pyecharts_Var:
    def __init__(self, title, x_value, y_name, y_value, *args, subtitle='', is_symbol_show=False, line_width=1, is_show=False, color_list=('#00b088', '#f76b61', '#ffb55d', '#8470ff', '#00a2ff', '#ffe400', '#11d2c2', '#c263f9')):
...
class Line_Py(Pyecharts_Var):
    ...
    def line_render_qimai(self):
        ...
        # 插入其他y线条；
        for num, agr_data in enumerate(self.args):
            c.add_yaxis(
                series_name=agr_data[0],
                is_symbol_show=self.is_symbol_show,  # 线上不用点标记
                color=self.color_list[num+1],
                linestyle_opts=opts.LineStyleOpts(width=self.line_width),  # 线条加粗
                y_axis=agr_data[1],
                label_opts=opts.LabelOpts(is_show=self.is_show),
            )

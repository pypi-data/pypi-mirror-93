...
MongoCursosPy = pymongo.cursor.Cursor
MongoCursor = Union[MongoCursorMotor, MongoCursosPy]
...
class SemSpare:
    log: Any = None
    _sem_base: SemBase = None
    SemName = collections.namedtuple('SemName', 'atlas db io max nmap os ping socket ssh')
    sem_name: SemName = dataclasses.field(default=None, init=False)
    spare: str = sem_base_spare['name']
...
def get_global_path(env: bool = False, log: bool = False):
    p: Optional[Path] = globals().get('path', None)
    if env and p:
        return p.env
    if log and p:
        return p.log
...
class EnvHomeFile(Asdict):
    ...
    def __init__(self, file: str = None, prefix: str = str()):
        self.prefix = prefix
        self.file.read_env(User.home.joinpath(file if file else '.env').text, verbose=True, override=True)
...
class Kill:
    ...
    def __post_init__(self):
        p = get_global_path()
        self.log = self.log if self.log else p.log
    ...
    @staticmethod
    def get_pkill(command: str, parameter: str = str()) -> Kill:
        global pkill
        if pkill is None:
            pkill = Kill(command=command, log=path.log.child, parameter=parameter)
        return pkill
...
class MongoBase(SemSpare):
    ...
    host: str = '127.0.0.1'
    log: Any = None
    max_pool: int = SemBase.atlas + SemBase.atlas_new
    password: NameDefault = None
    port: int = None
    retry: bool = True
    server_timeout: int = 300000
    srv: bool = True
    username: NameDefault = None
    ...
    def __post_init__(self):
        ...
        if self.password or self.username:
            env = EnvHomeFile(f'.{self.env}')
            self.password = env.var(self.password) if self.password else str()
            self.username = env.var(self.username) if self.username else str()

    @property
    def client(self) -> MongoClientPy:
        self.log.debug('Start')
        while True:
            try:
                uri, kwargs = self.connection
                rv = MongoClientPy(uri, **kwargs)
                self.log.debug('End')
                return rv
    ...
    async def client_aio(self, spare: str = str()) -> MongoClientMotor:
        args = Frame.args()
        task = asyncio.current_task().get_name()
        await self.log.adebug('Start', f'{args=}', f'{task=}')
        sem_name = SemSpare.sem_name.atlas if self.srv else SemSpare.sem_name.db
        ...
        async with self.sem(sem_name, spare):
            ...
            while True:
                try:
                    uri, kwargs = self.connection
                    rv = MongoClientMotor(uri, **kwargs)
    ...
    async def db_aio(self, spare: str = str()) -> MongoDBMotor:
    ...
    def db_default(self) -> MongoDB:
        return self.client.get_default_database()

    async def db_default_aio(self, spare: str = str()) -> MongoDB:
...
class IpPort:
    ...
    @staticmethod
    async def which_open(ip: str = LOCALHOST, semaphore: SemBase = sem, log: Logr = None,
                         p: Path = None) -> UnionDictList:
        """
        Scan host or hosts.

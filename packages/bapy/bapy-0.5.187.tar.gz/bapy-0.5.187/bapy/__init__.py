...
Collection = Union[pymongo.database.Collection, motor.motor_asyncio.AsyncIOMotorCollection]
...
UnionDictListSetStrTuple = Union[dict, list, set, str, tuple]  # type: ignore
UnionDictListStrTuple = Union[dict, list, str, tuple]  # type: ignore
...
UnionListStrTuple = Union[list, str, tuple]  # type: ignore
...
sem_spare = dict(name='new', factor=20)


@dataclasses.dataclass
class Sem(AsdictLarge):
    atlas: int = 499
    atlas_new: int = round(atlas / sem_spare['factor'])
    db: int = 500
    io: int = 500
    max: int = 5000
    nmap: int = 500
    nmap_new: int = round(nmap / sem_spare['factor'])
    os: int = 300
    os_new: int = round(os / sem_spare['factor'])
    ping: int = 400
    ping_new: int = round(ping / sem_spare['factor'])
    socket: int = 500
    ssh: int = 300
    ssh_new: int = round(ssh / sem_spare['factor'])
    _factor: int = 1
...
def flat_list(l: Iterable, recurse: bool = False, unique: bool = False) -> Union[list, Iterable]:
    """
    ...
    flat = []
    _ = [flat.extend(flat_list(item) if recurse else item)
...
class Env(Asdict):
    ...
    debug: Union[Env, Field] = None
    debug_async: Union[Env, Field] = None
    ic: Union[Env, Field] = None
    ...
    verbose: Union[Env, Field] = None

    def __init__(self, prefix: str = str(), sem_factor: int = Sem._factor):
        self.prefix = prefix
        ...
        self.debug = self.debug_async = self.ic = self.verbose = self.Field(False, True, 'bool')
    ...
    def sem(self):
        s = [self.file.int(upper_prefix(k, prefix=self.prefix), v) for k, v in
             Sem(_factor=self.sem_factor.value).defaults().items() if k not in ['sem_factor', 'value']]
        return Sem(*s)
...
class Flow(Flows):
    ...
    def add_tasks(self, t: Union[list, FlowPython, FlowBash]) -> Any:
        """
        Add tasks to flow.

        Args:
            t: List of tasks/flows to run.
...
class Frame:
    ...
    @classmethod
    def func(cls, back: bool = False) -> str:
...
class GitHub:
    ...
    cli: dict[str, list] = dataclasses.field(init=False)  # type: ignore
...
def get_all(obj: UnionDictList, get: GetAll = GetAll.KEYS, gen: bool = False) -> Iterable:
    # noinspection PyUnresolvedReferences
    """
...
class Log(verboselogs.VerboseLogger):
    ...
    colors: tuple = ('white', 'cyan', 'green', 'yellow', 'purple', 'blue', 'red', 'fg_bold_red', )  # type: ignore
...
class MongoBase:
    ...
    log: MongoClient = dataclasses.field(default=None, init=False)

    def __post_init__(self):
        self.log = self.log if self.log else path.log

    def client(self, a: bool = False) -> Union[MotorClient, MongoClient]:
        while True:
            try:
                client = MotorClient if a else MongoClient
                return client(self.uri, connectTimeoutMS=200000, serverSelectionTimeoutMS=300000,
                              maxPoolSize=Sem.atlas + Sem.atlas_new)
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
                self.nap()
                continue

    def db(self, a: bool = False) -> Union[pymongo.database.Database, motor.motor_asyncio.AsyncIOMotorDatabase]:
        while True:
            try:
                return self.client(a).get_database(self.db_name())
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
                self.nap()
                continue
    ...
    async def col_names_async(self) -> list:
        return await self.db(True).list_collection_names(filter=self.filters_base.col_names)
    ...
    async def cols_list_async(self) -> list:
        return [self.db(True).get_collection(item) for item in await self.col_names_async]
    ...
    async def cols_unique_async(self, field: str = '_id') -> dict:
        return {item.name: item.distinct(field) for item in await self.cols_list_async}

    def db_default(self, a: bool = False) -> Union[pymongo.database.Database,
                                                   motor.motor_asyncio.AsyncIOMotorDatabase]:
        return self.client(a).get_default_database()

    def db_name(self, a: bool = False) -> str:
        return self._db_name if self._db_name else self.db_default(a).name

    @staticmethod
    def get_sleep(lock: bool = False, queue: bool = False, rand: bool = False) -> Union[int, float]:
        if lock:
            sleep = MongoBase.naps.queue
        elif queue:
            sleep = MongoBase.naps.queue
        elif rand:
            sleep = round(random.uniform(MongoBase.naps.os[0], MongoBase.naps.os[1]), 2)
        else:
            sleep = MongoBase.naps.mongo
        return sleep

    @staticmethod
    def nap(lock: bool = False, queue: bool = False, rand: bool = False) -> None:
        time.sleep(MongoBase.get_sleep(lock=lock, queue=queue, rand=rand))

    @staticmethod
    async def anap(lock: bool = False, queue: bool = False, rand: bool = False) -> None:
        await asyncio.sleep(MongoBase.get_sleep(lock=lock, queue=queue, rand=rand))
...
class Mongo(MongoBase):
    """Mongo Helper Class."""
    _col_name: str = None
    col_name_default: str = 'test'

    def __post_init__(self):
        super(Mongo, self).__post_init__()

    def col(self, a: bool = False, name: str = None) -> Collection:
        args = Frame.args()
        while True:
            try:
                return self.db(a).get_collection(name if name else self.col_name(a))
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {args} {repr(exception)}')
                self.nap()
                continue

    def col_default(self, a: bool = False) -> Union[pymongo.database.Collection,
                                                    motor.motor_asyncio.AsyncIOMotorCollection]:
        return self.db(a).get_collection(self.col_name_default)

    def col_name(self, a: bool = False) -> str:
        return self._col_name if self._col_name else self.col_default(a).name

    def col_find(self, find: dict = None) -> pymongo.cursor.Cursor:
        while True:
            try:
                return self.col().find(find if find else dict())
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
                self.nap()
                continue

    def count(self, find: dict = None) -> int:
        while True:
            try:
                return self.col().count_documents(find if find else dict())
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
                self.nap()
                continue
    ...
    @property
    async def count_async(self, find: dict = None) -> int:
        while True:
            try:
                return await self.col(True).count_documents(find if find else dict())
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {repr(exception)}')
                await self.anap()
                continue
    ...
    async def count_estimated_default_async(self) -> list:
        return await self.col_default(True).estimated_document_count()
    ...
    def delete(self, ip: str):
        ...
        while True:
            try:
                self.col(True).delete_one({'_id': ip})
    ...
    async def delete_async(self, ip: str):
        args = Frame.args()
        task = asyncio.current_task().get_name()
        await self.log.adebug('Start', args, f'{task=}')
        while True:
            try:
                await self.col(True).delete_one({'_id': ip})
                await self.log.adebug('End', args, f'{task=}')
                break
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {args}, {task=}, {repr(exception)}')
                await MongoBase.anap()
                continue

    @property
    async def dump_async(self) -> list:
        await self.log.adebug('Start')
        while True:
            try:
                rv = await self.col(True).find().to_list(None)
    ...
    def dump_dict(self) -> collections.OrderedDict:
        self.log.debug('Start')
        while True:
            try:
                rv = collections.OrderedDict({item.get('_id'): del_key(item, '_id') for item in self.col_find()})
                self.log.debug('End')
                return rv
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
    ...
    async def find_async(self, field: str, id_key: bool = True) -> Union[dict[str, dict], list[dict]]:
        args = Frame.args()
        await self.log.adebug('Start', args)
        while True:
            try:
                rv = await self.col(True).find({field: True}, {'_id': 1, field: 1}).to_list(None)
                if id_key:
                    col = dict()
                    for item in rv:
                ...
                await self.log.awarning(f'Waiting for connection: {args}, {repr(exception)}')
                await self.anap()
                continue

    def find_field(self, field: str, filtering: typing.Any = None, rv_dict: bool = False) -> Union[dict, list]:
        args = Frame.args()
        self.log.debug('Start', args)
        while True:
            try:
                rv = dict()
                for item in self.col(True).find({field: True}, {'_id': 1, field: 1}):
                    if (value := item.get(field, None)) is not None:
                        rv[item['_id']] = value
    ...
    async def find_field_async(self, field: str, filtering: typing.Any = None,
                               rv_dict: bool = True) -> Union[dict, list]:
        args = Frame.args()
        await self.log.adebug('Start', args)
        while True:
            try:
                rv = dict()
                for item in await self.col(True).find({field: True}, {'_id': 1, field: 1}).to_list(None):
                    if (value := item.get(field, None)) is not None:
                        rv[item['_id']] = value
                        if filtering and filtering == value:
                            del rv[item['_id']]
    ...
    async def find_one(self, _id: Any, fields: Iterable = None):
        args = Frame.args()
        await self.log.adebug('Start', args)
        while True:
            try:
                rv = await self.col(True).find_one({'_id': _id},
                                                   {'_id': 0} | {field: True for field in iter_split(data=fields)})
                await self.log.adebug('End', args)
                return rv
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {args}, {repr(exception)}')
    ...
    def unique(self, field: str = '_id') -> list:
        args = Frame.args()
        self.log.debug('Start', args)
        while True:
            try:
                rv = self.col().distinct(field)
                self.log.debug('End', args)
                return rv
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')
    ...
    async def unique_async(self, field: str = '_id') -> list:
        await self.log.adebug('Start')
        while True:
            try:
                rv = await self.col(True).distinct(field)
                await self.log.adebug('End')
                return rv
            except self.mongo_exceptions as exception:
                await self.log.awarning(f'Waiting for connection: {repr(exception)}')
...
def paths(file: str, sem_factor: int = Sem._factor) -> Path:
...
class IpPort:
    ...
    @staticmethod
    async def is_global_ip(ip: str, semaphore: Sem = sem, p: Path = None) -> OptionalBool:
        """
    ...
    @staticmethod
    async def ping(ip: str, semaphore: Sem = sem, p=None) -> OptionalBool:
        """
        Pings host.
    ...
    @staticmethod
    async def which_open(ip: str = LOCALHOST, semaphore: Sem = sem, log: Logr = None, p: Path = None) -> UnionDictList:
        """
        Scan host or hosts.

...
MongoCursor = pymongo.cursor.Cursor
...
class Mongo(MongoBase):
    ...
    def col_find(self, find: dict = None, name: str = None) -> pymongo.cursor.Cursor:
        while True:
            try:
    ...
    def dump_dict(self, name: str = None) -> dict:
        self.log.debug('Start')
        while True:
            try:
                col = self.col_find(name=name)
                rv = {doc.get('_id'): del_key(data=doc, key='_id') for doc in col}
                self.log.debug('End')
                return rv
            except self.mongo_exceptions as exception:
                self.log.warning(f'Waiting for connection: {repr(exception)}')

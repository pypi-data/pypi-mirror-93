...
from pytket.circuit import BasisOrder, Bit, Qubit, UnitID  # type: ignore
...
class BackendResult:
    ...
    the form of statevector, unitary arrays or density matrices. For measured results, a map of
    ...
    def __init__(
        self,
        *,
        q_bits: Optional[Iterable[Qubit]] = None,
        c_bits: Optional[Iterable[Bit]] = None,
        counts: Optional[Counter[OutcomeArray]] = None,
        shots: Optional[OutcomeArray] = None,
        state: Any = None,
        unitary: Any = None,
        ...
        def _process_unitids(
            var: Dict[Bit, int], attr: str, lent: int, uid: Any
        ) -> None:
            if var is not None:
                setattr(self, attr, dict(reversed(pair) for pair in enumerate(var)))  # type: ignore
                if lent != len(var):
                    raise ValueError(
                        (
        ...
        if self.contains_measured_results:
            ...
            _process_unitids(c_bits, "c_bits", _bitlength, Bit)  # type: ignore
        ...
        if self.contains_state_results:
            ...
            _process_unitids(q_bits, "q_bits", _n_qubits, Qubit)  # type: ignore
    ...
    def __eq__(self, other: "BackendResult") -> bool:  # type: ignore
        return (
    ...
    def get_bitlist(self) -> List[Bit]:
        ...
        raise AttributeError("BackendResult does not include a Bits list.")
    ...
    def get_qbitlist(self) -> List[Qubit]:
        ...
        if self.q_bits is not None:
            return _sort_keys_by_val(self.q_bits)
        raise AttributeError("BackendResult does not include a Qubits list.")
    ...
    def _get_state_res(self, qubits: Sequence[Qubit]) -> StoredResult:
        ...
        if not _check_permuted_sequence(qubits, self.q_bits):
            raise ValueError(
                "For state/unitary/density_matrix results only a permutation of all qubits can be requested."
            )
    ...
    def get_result(
        ...
        :param request_ids: Ordered set of either Qubits or Bits for which to
            ...
            For statevector/unitary/density_matrix results some permutation of all qubits must be requested.
        ...
        :raises ValueError: For state/unitary/density_matrix results only a permutation of all

...
class DeviceDcElwa(ModbusRtuDevice):
    ...
    def __supervise__(self):
        ...
        modbusErrorRate = self.getcommunicationerrorsrate()
        if modbusError != True and modbusErrorRate != None and modbusErrorRate > MODBUSWARNLEVEL and modbusErrorRate < 1:
        ...
        if value == 0 and modbusError == False:
        ...
        if self.__checkRegiterTimeStamp__() != True and modbusError == False:
            logging.warning("[DeviceDcElwa] " + self.getidentifier() + " - Register values too old. Communication errors expected.")
            registerError = True
        else:
            registerError = False

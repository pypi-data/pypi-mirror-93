...
class DcsConnection:
        ...
        while self.__running__ == True:
            ...
            if self.__receiveBuffer__ != None and len(self.__receiveBuffer__)>=32:			
                ...
                if(rec_serialno != self.__serial__ ):
                    ...
                    if self.__receivedMessageQueue__.qsize() >= 2:      #todo LIMIT als konstante
        ...
        while self.__running__ == True:        
            ...
            if(message):
                ...
                try:
                    ...
                    if(difference.total_seconds() < 5):     #Todo Konstante definieren
                        self.__handlemessage__(message["type"], message["payload"], message["tid"])
                        processingtime = datetime.now() - message["receivetime"]
                        if(processingtime.total_seconds() > 5):     #Todo Konstante definieren
                            logging.warning(str(id(self)) + " processing time too log: " + str(difference.total_seconds()))
                    else:
                        logging.warning(str(id(self)) + " message to old. Rejecting message. Age: " + str(difference.total_seconds()))
                except Exception as e:
    ...
    def __handlemessage__(self, msgtype, payload, tid):
        ...
        if(msgtype == 6):
            ...
            # if payload != None:
            #     decryptedpayload = self.__decryptPayload__(payload)
            nack=True
            try:
                self.loadlogdata()
                nack=False
    ...
    def loadlogdata(self):
        if(self.__device__ == None):
            ...
            t = datetime.now()
            self.__logData__ = {"i_power": t.second * 100, "time": str(t)}
        else:

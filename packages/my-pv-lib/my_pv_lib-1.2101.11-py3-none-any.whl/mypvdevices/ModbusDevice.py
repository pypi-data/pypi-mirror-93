...
class ModbusDevice:
    ...
    def stop(self):
        ...
        for connection in self.__connections__:
            try:
                ...
                logging.info("[Device] ID " + str(self.__serial__) + " logdata sent.")
            except:
                logging.warning("[Device] ID " + str(self.__serial__) + " cannot send logdata while stopping.")
    ...
    def setlogdatasendinterval(self, interval):
        if interval != None and interval >= 0 and interval < 90000:
            ...
            logging.info("[Device] ID " + str(self.__serial__) + " logdata send interval set to " + str(self.__logdatasendinterval__))
            return True
        else:
    ...
    def setsetupvalue(self, key, value):
        ...
        try:
            ...
            logging.warning("[Device] ID " + str(self.__serial__) + " Setup-Element does not exist: " + str(e))
        ...
        try:
            ...
            if key in self.__settingsMap__:
                ...
                if registervalue != value:
                    logging.warning("[Device] ID " + str(self.__serial__) + " Register verification failed. " + str(registervalue[register]))
                    raise Exception("register verification failed.")
            else:
                logging.debug("[Device] ID " + str(self.__serial__) + " no register mapping for key " + str(key))
        ...
        for connection in self.__connections__:
            try:
                if not connection.isconnected():
                    logging.info("[Device] " + self.getidentifier() + " not connected to " + str(connection.getServer()))
                    errors += 1
            except Exception as e:
                logging.warning("[Device] " + self.getidentifier() + " cannot check connection to " + str(connection.getServer()) + ". " + str(e))
        if errors == 0:
            logging.debug("[Device] " + self.getidentifier() + ": DCS-Connections ok.")
            ...
            if self.__registers__ != None:
                if len(self.__registers__) > 0:
                    try:
                        ...
                        logging.warning("[Device] " + self.getidentifier() + ". Settingssync not successfull. " + str(e))
                    try:
                        self.__processregisters__()
                    except Exception as e:
                        logging.warning("[Device] " + self.getidentifier() + ". processing registers not successfull. " + str(e))
                else:
                    logging.warning("[Device] " + self.getidentifier() + ". No bus communication to device.")
            else:
                        ...
                        if self.__registers__[self.__settingsMap__[name]["register"]] != self.__setup__[name]:
                            ...
                            if self.__settingsMap__[name]["forced"]:
                                try:
                                    ...
                                    logging.warning("[Device] ID " + str(self.__serial__) + " cannot write register while settings sync. Set " + str(name) + " to "+str(self.__setup__[name])+". " + str(e))
    ...
    def getregistervalue(self, registerId):
        with self.__lock__:
            try:
                ...
                logging.warning("[Device] ID " + str(self.__serial__) + " register not found. Register Id: " + str(registerId))
                value = None
        return value
    ...
    def showcommunicationerrorsrate(self):
        ...
        if rate != None:
            rate = round(rate*100)
        logging.info("[Device] " + str(self.getidentifier()) + " - Communication error-rate: " +  str(rate) + "%, Errors: " +  str(self.getcommunicationerrorscounter()))

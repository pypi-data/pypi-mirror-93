...
class Basic(Process):
    ...
    def groove(self):
        ...
        for ctr in range(iterations):
            ...
            for aud in range(nvoices):
                audios[aud] = self.padtolength(audios[aud], maxlen)
            ...
            print("Audios : {0}".format(len(audios)))
            for ctr2 in range(2, nvoices):
                print(str(ctr2))
                compound = AudioSegment.silent(duration=cycle*len(rhythm),
                                               frame_rate=groove_layer.frame_rate)
                print(str(len(compound)))
                offset = ((ctr2 - 2) % cycle) * rlen
                print("    {0}".format(str(offset)))
                compound.overlay(audios[ctr2], position=offset)
                groove_layer.overlay(compound, times=1000)

            #summation = groove_base.overlay(groove_layer)
            summation = groove_layer

...
from th2_check2_recon.reconcommon import ReconMessage, MessageGroupType, SharedGroupPermission
...
class Rule:
    ...
    def __init__(self, event_store: EventStore, message_comparator: MessageComparator, cache_size: int,
                 match_timeout: int, shared_group_permission: SharedGroupPermission, shared_group: Cache.MessageGroup,
                 configuration) -> None:
        ...
        self.__shared_group_permission = shared_group_permission
        self.__shared_group = shared_group
        ...
        if shared_group_permission == SharedGroupPermission.read:
            self.__cache.message_groups.append(shared_group)

        logger.info(f"Rule '{self.get_name()}' initialized")

    def _add_to_shared_group(self, message):
        if self.__shared_group_permission == SharedGroupPermission.write:
            self.__shared_group.put(message)
    ...
    def process(self, proto_message: Message, attributes: tuple):
        ...
        if len(group_indices) != len(self.__cache.message_groups) - 1 \
                and self.__cache.message_groups[index_of_main_group].type != MessageGroupType.shared:
        ...
        if self.__cache.message_groups[index_of_main_group].type != MessageGroupType.shared:
            self.__cache.message_groups[index_of_main_group].put(message)

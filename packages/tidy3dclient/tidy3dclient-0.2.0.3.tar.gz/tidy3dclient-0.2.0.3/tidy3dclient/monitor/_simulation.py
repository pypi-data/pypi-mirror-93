...
def poynting(self, monitor):
    ...
    if mnt_data.E.size==0:
        raise ValueError("No electric field stored in the monitor.")
    if mnt_data.H.size==0:
        raise ValueError("No magnetic field stored in the monitor.")
...
def flux(self, monitor, normal=None):
    ...
    -------
    np.ndarray
        (Watt) The Poynting flux, an array of shape ``(Np, Nsample)``, where 
    ...
    return np.prod(mnt_data.mesh_step[cross_inds]) * \
...
def decompose(self, mode_monitor, Nmodes=None):
    ...
    if not isinstance(mode_monitor, ModeMonitor):
        raise TypeError("'ModeMonitor' instance expected.")
    ...
    if len(mnt_data.mode_plane.modes[0])==0:
        print("Warning: no modes found in monitor, use "
                "Simulation.compute_modes() first.")

    if mnt_data.data is False:
        raise RuntimeError("Solver results not loaded into monitor yet.")

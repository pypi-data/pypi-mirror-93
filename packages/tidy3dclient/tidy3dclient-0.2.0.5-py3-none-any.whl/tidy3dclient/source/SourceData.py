...
class SourceData(object):
    ...
    def _compute_modes_plane_wave(self, Nmodes):
        ...
        eps2D = mplane.eps
        if np.amax(eps2D) - np.amin(eps2D) > 1e-5:
            sim.print("Warning: 'PlaneWave' is intended to be placed "
                "entirealy in a homogeneous material.")

        if Nmodes > 1:
            raise RuntimeError("compute_modes(PlaneWave) can only be used "
                "for a single mode - the plane wave with a specified "
                "polarization.")
    ...
    def _compute_modes_plane_source(self, Nmodes):
        ...
        eps2D = mplane.eps
        if np.amax(eps2D) - np.amin(eps2D) > 1e-5:
            sim.print("Warning: 'PlaneSource' is intended to be placed "
                "entirealy in a homogeneous material.")

        if Nmodes > 1:
            raise RuntimeError("compute_modes(PlaneWave) can only be used for "
                "a single mode, the plane wave with a specified polarization.")

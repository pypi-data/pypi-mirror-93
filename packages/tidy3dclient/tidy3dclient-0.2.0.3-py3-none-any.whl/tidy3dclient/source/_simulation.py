...
def set_mode(self, source, mode_ind):
    ...
    if isinstance(source, PlaneWave) or isinstance(source, PlaneSource):
        if mode_ind > 0:
            raise RuntimeError("Plane wave mode index can only be set to 0, "
        ...
        if mode_ind >= len(mplane.modes[0]):
            print("Warning: selected mode index larger than number of "
        ...
        if Eedge > 1e-3:
            self.print("Warning: mode field does not decay at boundaries, "
                "which may produce unexpected results.")

        for key in ['E', 'H']:
            max_im = np.amax(np.abs(np.imag(mode[key])))
            if max_im > 1e-5:
                self.print("Warning: ignoring imaginary part of field %s"%key +
                    " with maximum magnitude %1.4f."%max_im)

...
from ..utils import (inside_box, inside_box_inds, cs2span, check_3D_lists,
                        listify)
from ..constants import int_, float_, complex_, fp_eps, xyz_list, xyz_dict

class Source(object):

    def __init__(self, source_time, span, amplitude=1., name=None):
        """Base class sources. Available subclasses:
...
class VolumeSource(Source):
    ...
    def __init__(self, source_time, center, size, component, 
        ...
        try:
            ...
            raise ValueError("Unrecognized component %s, must be one of %s."%(
                            str(component), str(cinds)))
...
class ModeSource(Source):
    ...
    def __init__(self, source_time, center, size,
        ...
        if self.norm_ind.size !=1:
            raise ValueError("Exactly one element of 'size' must be zero.")
    ...
    def __repr__(self):
        ...
        if self.mode_ind is None:
            rep += "mode index   = not set"
        else:
            rep += "mode index   = %d"%self.mode_ind
        rep += "}\n"
...
class PlaneWave(ModeSource):
    ...
    def __init__(self, source_time, injection_axis, position, polarization,
        ...
        if injection_axis not in inj_axes:
            raise ValueError("'injection_axis' must be one of %s."%(
                                ", ".join(inj_axes)))
        ...
        try:
            ...
            raise ValueError("'position' is a single float giving the plane "
                            "position along the 'normal' axis.")
        try:
            self.epol_ind = xyz_dict[polarization]
        except:
            raise ValueError("'polarization' must be one of 'x', 'y', or 'z'.")

        if norm_ind==self.epol_ind:
            raise ValueError("'polarization' orientation cannot be the same "
                                "as 'injection_axis'.")
...
class PlaneSource(ModeSource):
    ...
    def __init__(self, source_time, position, normal, direction='forward',
        ...
        # mode_ind can only be 0 for a PlaneSource. A single mode - the 
        # required plane wave - is returned by PlaneSource.compute_modes().
        self.mode_ind = 0
    ...
    def __repr__(self):
        ...
        rep += "normal       = %s\n"%self.normal
        rep += "direction    = %s\n"%self.direction
        rep += "diff_order   = [" + ', '.join(["%d"%d 
                                    for d in self.diff_order]) + "] \n"
        if np.linalg.norm(self.polarization) < fp_eps:
            rep += "pol_angle    = %1.4f\n"%self.pol_angle
        else:
            rep += "polarization = [" + ', '.join(["%1.2f"%p 

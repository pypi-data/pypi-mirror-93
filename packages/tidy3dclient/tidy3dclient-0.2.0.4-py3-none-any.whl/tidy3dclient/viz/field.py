...
def _check_monitor(mnt_data, field):
    if field.lower()=='e':
        if mnt_data.E.size==0:
            raise ValueError("Monitor has no stored E-field values.")
    elif field.lower()=='h':
        if mnt_data.H.size==0:
            raise ValueError("Monitor has no stored H-field values.")
    elif field.lower()=='s':
        if mnt_data.S.size==0:
            raise ValueError("Monitor has no stored S-field values. "
                             "Run the simulation and use "
                             "Simulation.poynting(Monitor) to compute S.")     
    else:

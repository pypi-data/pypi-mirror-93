...
class Medium(object):
    ...
    def __init__(self, epsilon=1, sigma=0, name=None):
        ...
        if isinstance(epsilon, DispersionModel):
            ...
            if 0 < epsilon < 1:
                print("Warning: permittivity smaller than one could result "
                    "in numerical instability. Use Courant stability factor "
                    "value lower than the smallest refractive index value.")

            elif epsilon <= 0:
                print("Warning: permittivity smaller than zero can result in "
                    "numerical instability and should be included as a "
                    "dispersive model.")

                if epsilon < -100:
                    print("For large negative values consider using PEC "
                          "instead.")
            self.dispmod = None
            self.eps = epsilon
            self.sigma = sigma

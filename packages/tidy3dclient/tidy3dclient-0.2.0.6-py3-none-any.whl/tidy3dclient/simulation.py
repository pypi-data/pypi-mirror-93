...
from .utils import cs2span, intersect_box, check_3D_lists, object_name
...
class Simulation(object):
    ...
    def _check_size(self):
        ...
        if self.Np > 4e9:
            raise RuntimeError("Total number of grid points %1.2e exceeds "
                "current limit 4e9, increase the mesh step or decrease the "
                "size of the simulation domain."%self.Np)

        if self.Np*self.Nt > 2e14:
            raise RuntimeError("Product of grid points and time steps "
                "%1.2e exceeds current limit 2e14. Increase the mesh step or "
                "decrease the 'run_time' of the simulation."%self.Np*self.Nt)
    ...
    def _set_run_time(self, run_time):
        ...
        self.Nt = np.array(self.grid.tmesh.size).astype(int_)

...
def __make_requests(baseUrl, inputs, source, target, authentication, outputPath, serverOptions):
    requests = []
    for sql in inputs:
...
def runner(runnerArgs):
    ...
    serverOptions = {
        'format': 'true',
        'createTableSettings': {
            'orsfCreateTableVColType': 'view'
        },
        'commonSettings': {
            'orsfDatatypeForDate': 'DATE'
        }
    }
    try:
        sys.stderr.write('\n')
        ...
        if args.server_config:
            properties = __read_property_file(args.server_config)
            # In SQL-1878 we'll make the backend end names for these properties more user
            # friendly to match what we are exposing to users here and eliminate this
            # extra layer of mapping
            if 'format' in properties:
                serverOptions['format'] = properties['format']
            # create-table is a group on the backend and virtual-column-representation is a child of
            # that group we use dashes within a keys and then underscores between a group and a child
            if 'create-table_virtual-column-representation' in properties:
                serverOptions['createTableSettings']['orsfCreateTableVColType'] = properties['create-table_virtual-column-representation']
            # common is a group, date-data-type is a child
            if 'common_date-data-type' in properties:
                serverOptions['commonSettings']['orsfDatatypeForDate'] = properties['common_date-data-type']
        ...
        responses = __make_requests(baseUrl, inputs, args.source, args.target, args.auth, outputPath, serverOptions)

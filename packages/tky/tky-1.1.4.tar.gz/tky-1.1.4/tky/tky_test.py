...
def test_mg():
    ...
    vls = '相思叶图样图森破旱田杳加盟楼主'
    vll = ('方言', '数学', '物理', '化学')
    from tky.t_tsg import test_msg
...
# 验证文件夹方法
def excel_test():
    file_url = r'G:\web\test\test.xlsx'
    # a = t_file.Excel.read_data(file_url)
    # print(a)
    b = t_file.Excel.write_data(file_url,stn='Sheet1',row_n=1,dow_n=1,value='name')
    c = t_file.Excel.write_data(file_url,stn='Sheet1',row_n=3,dow_n=1,value='test')
    d = t_file.Excel.write_data(file_url,stn='Sheet1',row_n=1,dow_n=2,value='values')
    e = t_file.Excel.write_data(file_url,stn='Sheet1',row_n=3,dow_n=2,value='test1')

...
class File_val:
    ...
    def xml_msg(self: str = 'null', path: str = 'null'):
        ...
        try:
            ...
            if (null(self) or null(path)) == 'N':
                return '入参异常：请传入不为空的 self,path 值...'
            else:
                dom = minidom.parse(self)
                xml_list = []
                for e in path:
    ...
    def path_val(self: str = 'null', val: int = 'null'):
        ...
        :param  val : int  哪个文件,如最后一个：-1；
        :return  文件的名称；
        """
        try:
            if (null(self) or null(val)) == 'N':
                return '入参异常：请传入不为空的 self,val 值...'
            else:
                listdir = os.listdir(self)
                val_name = listdir[int(val)]
                return str(val_name)
        except EnvironmentError as err:
            return '发生异常：%s' % err

...
class BaseKeyCloak(object):
    ...
    def get_backend(self):
        ...
        for backend in backends:
            if issubclass(backend, OIDCAuthenticationBackend):
                return backend
...
class CreateUser(BaseKeyCloak):
    def __call__(self, email):
        ...
        token_info = self.keycloak_openid.token(email, password)
        user_info = self.keycloak_openid.introspect(token_info['access_token'])
        ...
        if len(users) == 1:
            ...
            user = backend.create_user(user_info)

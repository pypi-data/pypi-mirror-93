...
def print_assignment_prob(a: Assignment, prob: float, precision=DEFAULT_PRECISION, printer=print):
...
def get_node_full_assignments(node: Node, all_nodes: List[Node]) -> List[Assignment]:
    """
    Return the list of all full assignments under which the given node holds.
    """
    return sum_out(((node, True),), all_nodes)

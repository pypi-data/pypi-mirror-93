...
class MinAdm(Semantics):
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        for node in self.af.get_nodes():
            node_parents = node.get_parents()
            if node_parents:
                node_full_assignments = asg.get_node_full_assignments(node, self.af.get_nodes())
                for attacker in node_parents:
                    attacker_parents = attacker.get_parents()
                    if attacker_parents:
                        assignment: List[Tuple[Node, bool]] = [(defender, False) for defender in attacker_parents]
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        for node in self.af.get_nodes():
            ...
            if [] in cnf:
                ...
                self.constraints_linear.extend(ls.constraints_node_equals_value(node, 0))
                continue
            node_assignments = asg.get_node_full_assignments(node, self.af.get_nodes())
            cnf_assignments = asg.positive_cnf_assignments(cnf, self.af.get_nodes())
...
def prCmp_linear_constraints(nodes: List[Node], ls: LinearSolver):
    ...
    constraints = []
    for node in nodes:
        node_assignments = asg.get_node_full_assignments(node, ls.af.get_nodes())
        if node.is_initial():

...
class Semantics:
    ...
    def get_z3_constraints(self, z3i: Z3Instance):
        if self.constraints_z3 is None:
            self.constraints_z3 = []
            self.generate_z3_constraints(z3i)
        return self.constraints_z3

    def get_linear_constraints(self, ls: LinearSolver):
...
class Min(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            ...
            self.constraints_z3.append(node_prob == 0)
...
class Neu(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            node_prob = z3i.get_node_prob_var(node)
            self.constraints_z3.append(node_prob == 0.5)
...
class Max(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            node_prob = z3i.get_node_prob_var(node)
            self.constraints_z3.append(node_prob == 1)
...
class Dirac(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            node_prob = z3i.get_node_prob_var(node)
            self.constraints_z3.append(z3.Or(node_prob == 0, node_prob == 1))
...
class Ter(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            node_prob = z3i.get_node_prob_var(node)
            self.constraints_z3.append(z3.Or(node_prob == 0, node_prob == 0.5, node_prob == 1))
...
class Inv(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for edge in self.af.get_edges():
            ...
            self.constraints_z3.append(prob_node_from == 1 - prob_node_to)

    def generate_linear_constraints(self, ls: LinearSolver):
        for edge in self.af.get_edges():
            node_from_assignments = asg.get_node_assignments(edge.node_from, self.af.get_nodes())
            node_to_false_assignments = asg.get_node_assignments(edge.node_to, self.af.get_nodes(),
...
class Rat(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for edge in self.af.get_edges():
            prob_node_from = z3i.get_node_prob_var(edge.node_from)
            prob_node_to = z3i.get_node_prob_var(edge.node_to)
            self.constraints_z3.append(z3.Implies(prob_node_from > 0.5, prob_node_to <= 0.5, ctx=z3i.context))
...
class Opt(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            ...
            self.constraints_z3.append(node_prob >= 1 - summation)

    def generate_linear_constraints(self, ls: LinearSolver):
        all_nodes = self.af.get_nodes()
...
class CF(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for edge in self.af.get_edges():
            ...
            self.constraints_z3.append(prob_var == 0)

    def generate_linear_constraints(self, ls: LinearSolver):
        for edge in self.af.get_edges():
            assignment: List[Tuple[Node, bool]] = [(edge.node_from, True), (edge.node_to, True)]
            full_assignments = asg.sum_out(tuple(assignment), self.af.get_nodes())
            self.constraints_linear.extend(ls.constraints_assignments_equal_value(full_assignments, 0))


# Admissibility

def almost_sure_defense_constraint(z3i: Z3Instance, node: Node):
    """
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            if node.is_initial():
                ...
                continue
            node_prob = z3i.get_node_prob_var(node)
            as_defense_constraint = almost_sure_defense_constraint(z3i, node)
            self.constraints_z3.append(z3.Implies(node_prob == 1, as_defense_constraint, ctx=z3i.context))
...
class PrAdm(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            if node_parents:
                attacker_assignment: List[Tuple[Node, bool]] = [(attacker, False) for attacker in node_parents]
                attacker_prob_var = z3i.get_prob_var(tuple(attacker_assignment))
            else:
            ...
            if node_children:
                attackee_assignment: List[Tuple[Node, bool]] = [(attackee, False) for attackee in node_children]
                attackee_prob_var = z3i.get_prob_var(tuple(attackee_assignment))
            else:
                # there are no attackees, so the prob of any attackee holding is 0
                # thus, in the negated view, the prob is 1
                attackee_prob_var = 1
            if node_parents or node_children:
                self.constraints_z3.append(attackee_prob_var >= attacker_prob_var)
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        for node in self.af.get_nodes():
            ...
            if node_parents:
                attacker_assignment: List[Tuple[Node, bool]] = [(attacker, False) for attacker in node_parents]
                attacker_full_assignments = asg.sum_out(tuple(attacker_assignment), self.af.get_nodes())
            ...
            if node_children:
                attackee_assignment: List[Tuple[Node, bool]] = [(attackee, False) for attackee in node_children]
                attackee_full_assignments = asg.sum_out(tuple(attackee_assignment), self.af.get_nodes())
        ...
        for node in self.af.get_nodes():
            ...
            if node_parents:
                ...
                for attacker in node_parents:
                    ...
                    if attacker_parents:
                        assignment: List[Tuple[Node, bool]] = [(defender, False) for defender in attacker_parents]
                        prob_var = z3i.get_prob_var(tuple(assignment))
                        self.constraints_z3.append(node_prob <= 1 - prob_var)
                    else:
                        self.constraints_z3.append(node_prob == 0)
            else:
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            if [] in cnf:
                ...
                self.constraints_z3.append(node_prob == 0)
                continue
            cnf_assignments = asg.positive_cnf_assignments(cnf, self.af.get_nodes())
            prob_vars = [z3i.get_prob_var(a) for a in cnf_assignments]
            # self.constraints_z3.append(node_prob <= z3.Sum(prob_vars))
            self.constraints_z3.append(node_prob.__le__(z3.Sum(prob_vars)))
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        for node in self.af.get_nodes():
            ...
            if [] in cnf:
                ...
                self.constraints_linear.extend(ls.constraints_node_equals_value(node, 0))
                continue
            node_assignments = asg.get_node_assignments(node, self.af.get_nodes())
            cnf_assignments = asg.positive_cnf_assignments(cnf, self.af.get_nodes())
...
class WCmp(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            as_defense_constraint = almost_sure_defense_constraint(z3i, node)
            self.constraints_z3.append(z3.Implies(node_prob == 1, as_defense_constraint, ctx=z3i.context))
            self.constraints_z3.append(z3.Implies(as_defense_constraint, node_prob == 1, ctx=z3i.context))


def prCmp_z3_constraints(nodes: List[Node], z3i: Z3Instance) -> List[z3.BoolRef]:
    """
...
class PrCmp(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        self.constraints_z3.extend(prCmp_z3_constraints(self.af.get_nodes(), z3i))

    def generate_linear_constraints(self, ls: LinearSolver):
        prAdm = PrAdm(self.af)
        self.constraints_linear.extend(prAdm.get_linear_constraints(ls))
        self.constraints_linear.extend(prCmp_linear_constraints(self.af.get_nodes(), ls))
...
class JntCmp(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            prob_vars = [z3i.get_prob_var(a) for a in cnf_assignments]
            # self.constraints_z3.append(node_prob == z3.Sum(prob_vars))
            self.constraints_z3.append(node_prob.__eq__(z3.Sum(prob_vars)))
...
class ElmClassicalSemantics(Semantics):
    ...
    """
    def __init__(self, classical_semantics: Type[classical.ClassicalSemantics], af: DPGM):
    ...
    def generate_linear_constraints(self, ls: LinearSolver):
        ...
        sem_extensions = sem.get_extensions()
        non_adm_assignments = []
        for assignment in asg.generate(self.af.get_nodes()):
            if asg.to_extension(assignment) not in sem_extensions:
...
class ElmCF(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.CF, af)
...
class ElmAdm(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.Admissible, af)
...
class ElmCmp(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.Complete, af)
...
class ElmGrn(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.Grounded, af)
...
class ElmPrf(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.Preferred, af)
...
class ElmSStl(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.SemiStable, af)
...
class ElmStl(ElmClassicalSemantics):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(classical.Stable, af)
...
class LabAdm(Semantics):
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            self.constraints_z3.append(z3.Implies(node_prob == 0, z3.Or(or_list, z3i.context), z3i.context))
...
class LabCmp(Semantics):
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            for parent_node in node.get_parents():
                parent_node_prob = z3i.get_node_prob_var(parent_node)
                self.constraints_z3.append(z3.Implies(node_undec, parent_node_prob < 1, z3i.context))
                parent_undec = open_interval_constraint(parent_node_prob, z3i.context)
                or_list.append(parent_undec)
            self.constraints_z3.append(z3.Implies(node_undec, z3.Or(or_list, z3i.context), z3i.context))
...
class WNorS(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            ...
            # self.constraints_z3.append(node_prob <= attackers_prob)
            self.constraints_z3.append(node_prob.__le__(attackers_prob))
...
class NorS(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            ...
            attackers_prob = z3i.get_prob_var(tuple(assignment))
            # self.constraints_z3.append(node_prob == attackers_prob)
            self.constraints_z3.append(node_prob.__eq__(attackers_prob))
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        for node in self.af.get_nodes():
            ...
            if node.value is not None:
                ...
                # self.constraints_z3.append(node_prob == node.value)
                self.constraints_z3.append(node_prob.__eq__(node.value))
            elif node.interval is not None:
                low, high = node.interval
                # self.constraints_z3.append(node_prob >= low)
                # self.constraints_z3.append(node_prob <= high)
                self.constraints_z3.append(node_prob.__ge__(low))
                self.constraints_z3.append(node_prob.__le__(high))
...
class NNor(Semantics):
    ...
    """
    def __init__(self, af: DPGM, use_prior_probs=False):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for node in self.af.get_nodes():
            ...
            if self.use_prior_probs:
                if node.value is not None:
                    # self.constraints_z3.append(node_prior_prob_var == node.value)
                    self.constraints_z3.append(node_prior_prob_var.__eq__(node.value))
                elif node.interval is not None:
                    i_min, i_max = node.interval
                    self.constraints_z3.append(z3.And(node_prior_prob_var >= i_min, node_prior_prob_var <= i_max))
                else:
                    self.constraints_z3.append(z3.And(node_prior_prob_var >= 0, node_prior_prob_var <= 1))
            else:
                self.constraints_z3.append(node_prior_prob_var == 1)

            if node.is_initial():
                node_prob = z3i.get_node_prob_var(node)
                # self.constraints_z3.append(node_prob == node_prior_prob_var)
                self.constraints_z3.append(node_prob.__eq__(node_prior_prob_var))
                continue
            ...
            for parent_assignment in asg.generate(node.get_parents()):
                ...
                for (parent, status) in parent_assignment:
                    if status:
                        ...
                        product *= (1 - edge_var)
                # self.constraints_z3.append(prob_var == product)
                self.constraints_z3.append(prob_var.__eq__(product))
...
class NNorAF(NNor):
    ...
    """
    def __init__(self, af: DPGM):
        super().__init__(af, use_prior_probs=True)
...
class CFs(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for edge in self.af.get_edges():
            ...
            assignment: List[Tuple[Node, bool]] = [(edge.node_from, True), (edge.node_to, True)]
            prob_var = z3i.get_prob_var(tuple(assignment))
            self.constraints_z3.append(prob_var <= 1 - edge_var)
...
class StrengthCF(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for edge in self.af.get_edges():
            ...
            self.constraints_z3.append(prob_var <= (1 - edge_var) * node_to_prob)
...
class StrengthSupportCF(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for edge in self.af.get_edges():
            ...
            prob_var = z3i.get_prob_var(tuple(assignment))
            node_to_prob = z3i.get_node_prob_var(edge.node_to)
            # self.constraints_z3.append(prob_var >= edge_var * node_to_prob)
            self.constraints_z3.append(prob_var.__ge__(edge_var * node_to_prob))
...
class DiracClassicalSemantics(Semantics):
    ...
    def generate_z3_constraints(self, z3i: Z3Instance):
        ...
        for extension in cs.get_extensions():
            assignment = asg.from_extension(extension, self.af.get_nodes())
            prob_var = z3i.get_prob_var(assignment)
            or_list.append(prob_var == 1)
        self.constraints_z3.append(z3.Or(or_list))

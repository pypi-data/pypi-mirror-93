...
class DPGM:
    ...
    def add_edge(self, edge: Edge):
        ...
        # if acyclicity was previously computed and holds, we need to reset it
        if self.acyclic is not None and self.acyclic:
            self.acyclic = None
    ...
    def print(self):
        ...
        print(str(len(self.all_edges)) + " edges: "
    ...
    def is_acyclic(self):
        ...
        if len(self.get_initial_nodes()) == 0:
            self.acyclic = False
            return False

        # remove leaves until either the graph is empty (-> acyclic) or until no leaves remain (-> cyclic)
        nodes = list(self.get_nodes())
        while len(nodes) > 0:
            leaves = [n for n in nodes if all([c not in nodes for c in n.all_children])]
            if len(leaves) == 0:
                self.acyclic = False
                return False
            for leaf in leaves:

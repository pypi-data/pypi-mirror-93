...
class Z3Instance:
    ...
    def _generate_full_prob_vars(self):
        ...
        if not self.full_joint_assignments:
            self.full_joint_assignments = asg.generate(self.af.get_nodes())
    ...
    def distribution_from_model(self, model) -> Distribution:
        distribution = dict()
        for assignment in self.full_joint_assignments:
            prob_var = self.get_prob_var(assignment)
            distribution[tuple(assignment)] = get_prob_var_value(prob_var, model)

...
class PersistedRmq(object):
    ...
    def __init__(self, channel, client_id=None, client_types=None, on_message=None):
    ...
    async def stop(self):
        ...
        logging.error(f'redis unsubscribe channel:{self.subscribe_chn}')

    async def close(self):
        logging.error(f'will close redis connection')
        self.conn.close()
        await self.conn.wait_closed()
        logging.error(f'redis connection closed')
    ...
    async def subscribe(self, timeout=None):
        ...
        if old_messages:
            try:
                ...
                await self.close()
                return
        logging.error(f'进入订阅:{self.subscribe_chn}2')
        ...
        while self.running:
            if await chs[0].wait_message():
                ...
                try:
                    ...
                    logging.error(e)
                    await self.conn.unsubscribe(self.subscribe_chn)
                    logging.error(f'收到Redis消息下发websocket失败 导致 redis unsubscribe channel:{self.subscribe_chn}')
                    await self.close()
                    return
            else:
                break
        await self.close()
        logging.error(f"已经退出循环")

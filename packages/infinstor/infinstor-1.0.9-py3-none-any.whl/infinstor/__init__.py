...
def run_transform(run_options, **kwargs):
    ...
    if (run_location == "inline"):
        experiment_id = None
        if ('experiment_id' in run_options):
            experiment_id = str(run_options['experiment_id'])
        if (parent_run_id != None):
            nes = True
        else:
            nes = False
        print("run_transform: experiment_id is " + str(experiment_id))
        with start_run(nested=nes, experiment_id=experiment_id) as run:
...
def run_transforms(transforms):
    ...
    if ('experiment_id' in transforms[0]['run_options']):
        experiment_id = str(transforms[0]['run_options']['experiment_id'])
        print("run_transforms: experiment_id is specified as " + experiment_id)
        run = start_run(experiment_id = experiment_id)
    else:
        print("run_transforms: experiment_id is not specified")
        run = start_run()

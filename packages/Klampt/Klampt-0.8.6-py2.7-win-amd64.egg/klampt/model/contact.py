...
class Hold:
    ...
    def setFixed(self,link,contacts):
        """Creates this hold such that it fixes a robot link to match a list
        of contacts (in world space) at its current transform.

        Args:
            link: a robot link or rigid object, currently contacting the
                environment / object at `contacts`
            contacts (list of ContactPoint): the fixed contact points, given
...
def equilibriumTorques(robot,holdList,fext=(0,0,-9.8),internalTorques=None,norm=0):
    ...
    Args:
        ...
        internalTorques (list, optional): if given, a list of length
            `robot.numDofs` giving internal torques. For example, using this
            can incorporate dynamics into the solver.

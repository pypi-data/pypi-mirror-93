...
class PixelAchievements:
    ...
    async def get_all_achievements(self):
        ...
        achievements_file = cachedir / "achievements.json"

        try:
            async with aiofiles.open(achievements_file, "r") as fin:
                data = json.loads(await fin.read())
        except FileNotFoundError:
            # Don't have this yet, so we need to cache it
            data = await self._download_achievements_resource()
            self.achievements = data["achievements"]
            async with aiofiles.open(achievements_file, "w") as fout:
                await fout.write(json.dumps(data))
        else:
            if data["last_check_time"] + timedelta(days=7) > dt.utcnow().timestamp():
                self.achievements = data["achievements"]
            else:
                try:
                    achievements_file.unlink()
                except FileNotFoundError:
                    # If this ever happens, it means the file was 
                    # removed between opening it and now
                    raise
                else:
                    await self.get_all_achievements()
    
    async def _download_achievements_resource(self, backoff_cnt: int=0) -> dict:
        backoff = 2
            ...
            async with session.get(RAW_ACHIEVEMENTS_URL) as r:
                if r.status == 200:
                    try:
                        ...
                        if backoff_cnt == 8:
                            # At this point, we've waited over 8 minutes in total and 
                            # are not getting a parseable response, so just stop trying 
                            # and raise the error
                            raise
                        await asyncio.sleep(backoff**backoff_cnt)
                        backoff_cnt += 1
                        return await self._download_achievements_resource()
                    else:
                        self.achievements = data["achievements"]
                        data["last_check_time"] = dt.utcnow().timestamp()
                        return data
    ...
    def from_player_data(cls, data: dict):
        ...
        if "rank" in data:
            try:
                ...
                # if there's some value for data["rank"] that 
                # isn't here, skip and find the next valid rank
                pass
...
class Player:
    ...
    def online(self) -> bool:
        ...
        """
        if not self.last_logout and not self.last_login:
            return False

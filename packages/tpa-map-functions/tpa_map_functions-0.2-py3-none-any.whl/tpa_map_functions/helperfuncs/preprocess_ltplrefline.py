...
Adopted to tpa module: Leonhard Hermansdorfer
...
if __name__ == '__main__':
    ...
    mod_performance_assessment_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                                                   .split('mod_performance_assessment')[0], 'mod_performance_assessment')

    sys.path.append(mod_performance_assessment_path)

    import tire_perf_assessment.helper_funcs_tpa.src.manage_paths as manage_paths

    bool_plot = True

    path_dict = manage_paths.manage_paths()

    output_data = preprocess_ltplrefline(filepath2ltpl_refline=path_dict['filepath2ltpl_refline'],

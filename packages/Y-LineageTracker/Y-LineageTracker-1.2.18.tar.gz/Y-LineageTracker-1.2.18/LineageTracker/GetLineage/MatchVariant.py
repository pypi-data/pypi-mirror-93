...
class MatchVariant(object):
    ...
    def match_variant_snp(self, ref_info, data_info, file_format):
        ...
        if file_format == 'bam':
            ...
            hap_function = (lambda x: matched_marker+matched_info[i, -9] if x == match_marker else x)
            for i in range(len(matched_info)):
                match_marker = matched_info[i, -5]
            ...
            hap_function = (lambda x: matched_marker+matched_info[i, -9] if x[0] == match_marker else x)
            ins_function = (lambda x: matched_marker+matched_info[i, -9] if x[0] in ins_loc_list else x)
            del_function = (lambda x: matched_marker+matched_info[i, -9] if x[0] in del_loc_list else x)
            for i in range(len(matched_info)):
                if ('ins' or 'del') not in matched_info[i, -5]:
                    if matched_info[i, -5] == matched_info[i, 3]:
                        match_marker = '0'
                        matched_info[i, header_cut: header_end] = list(map(hap_function, matched_info[i, header_cut: header_end]))
                    else:
                        variant_list = matched_info[i, 4].split(',')
                        if len(variant_list) > 1:
                            if matched_info[i, -5] in variant_list:
                                alt_num = str(variant_list.index(matched_info[i, -5]) + 1)
                                match_marker = alt_num
                                matched_info[i, header_cut: header_end] = list(map(hap_function, matched_info[i, header_cut: header_end]))
                        elif len(variant_list) == 1:
                            if matched_info[i, -5] == matched_info[i, 4]:
                                match_marker = '1'
                                matched_info[i, header_cut: header_end] = list(map(hap_function, matched_info[i, header_cut: header_end]))

                elif matched_info[i, -5].startswith('ins'):
                    if matched_info[i, -5] == 'ins':
                        variant_list = matched_info[i, 4].split(',')
                        if len(variant_list) > 1:
                            ...
                            if len(matched_info[i, 4]) > len(matched_info[i, 3]):
                                match_marker = '1'
                                matched_info[i, header_cut: header_end] = list(map(hap_function, matched_info[i, header_cut: header_end]))
                    elif re.search(r'ins \d+ bp', matched_info[i, -5]):
                        ins_bp = matched_info[i, -5][4]
                        variant_list = matched_info[i, 4].split(',')
                        if len(variant_list) > 1:
                ...
                elif matched_info[i, -5].startswith('del'):
                    if matched_info[i, -5] == 'del':
                        variant_list = matched_info[i, 4].split(',')
                        if len(variant_list) > 1:
                    ...
                    elif re.search(r'del -\d+ bp', matched_info[i, -5]):
                            del_bp = matched_info[i, -5][5]
                            variant_list = matched_info[i, 4].split(',')
                            if len(variant_list) > 1:

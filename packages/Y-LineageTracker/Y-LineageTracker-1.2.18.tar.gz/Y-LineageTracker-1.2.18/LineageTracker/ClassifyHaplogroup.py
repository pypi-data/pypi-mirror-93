...
def classify_parser():
    ...
    parser.add_argument('-b', '--build',
                        ...
                        default=37,
                        choices=[37, 38],
                        help='build: The build version of the reference genome used for NRY haplogroup classification. The default is 37.')
...
def main():
    ...
    from FilesIO import CommonData, check_cutoff, check_BAM_VCF_input, check_samples, time_count
    # check input files
    input, format = check_BAM_VCF_input(arguments.vcf, arguments.bam)
    ...
    ref_info, ref_ind_info = common_data.read_ref_info(arguments.snp, arguments.approximate)

    # filter samples
    from ProcessData import ReadFilterData
    read_filter = ReadFilterData.ReadFilterData(input, sample)
    if format == 'bam':
        data_info, inds = read_filter.read_bam(ref_info, arguments.build, 'snp')
    else:
    ...
    tracker = TrackLineage.TrackLineage(matched_info, ref_ind_info, arguments.build, format, arguments.chip)

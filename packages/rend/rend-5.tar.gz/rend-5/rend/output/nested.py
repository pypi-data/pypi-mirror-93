...
def ustring(hub, indent, color, msg, prefix="", suffix="", endc=attr(0)):
    ...
    try:
        ...
        try:
            return fmt.format(
                indent, color, prefix, hub.output.nested.to_unicode(msg), endc, suffix,
            )
        except UnicodeDecodeError:
    ...
    # as until about python32 it was limited to 12 digits only by default
        ...
        for line in ret.splitlines():
            ...
            if isinstance(line, bytes):
                ...
                break
            out.append(hub.output.nested.ustring(indent, fg(2), line, prefix=line_prefix))
            first_line = False
        ...
        for key in keys:
            val = ret[key]
            out.append(hub.output.nested.ustring(indent, color, str(key), suffix=":", prefix=prefix))

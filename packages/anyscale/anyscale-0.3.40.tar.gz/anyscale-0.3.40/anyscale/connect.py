...
ANYSCALE_BASE_DOCKER_IMAGE_CPU = "anyscale/ray-ml:nightly-cpu"
ANYSCALE_BASE_DOCKER_IMAGE_GPU = "anyscale/ray-ml:nightly-gpu"
...
class SessionBuilder:
    ...
    def connect(self) -> None:
        ...
        self._populate_cluster_config(cluster_yaml)
        ...
        if self._session_name is None or not self._get_session(
            project_id, self._session_name
        ):
            # Unconditionally create the session if it doesn't exist.
            needs_up = True
        else:
    ...
    def _up_session(
        self, project_id: str, session_name: str, cwd: Optional[str]
    ) -> None:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as output_log_file:
            with self._subprocess.Popen(
                ...
                if retcode:
                    cmd = " ".join(command)
                    msg = "Executing '{}' in {} failed, see {}".format(
                        cmd, self._project_dir, output_log_file.name
                    )
                    self._log.error(msg)
    ...
    def _get_or_create_updated_session(  # noqa: C901
        ...
        if session_name is None:
            ...
            for session in results:
                ...
                try:
                    ...
                    self._log.debug(
                        "ConnectionError to session, will revisit", session.name
                    )
        ...
        if session_name is None:
            ...
            for i in range(MAX_SESSIONS):
                ...
                if name not in used_names:
                    session_name = name
                    self._log.info("Starting session", session_name)
                    break
        ...
        if session_hash != files_hash:
            self._log.info("Syncing latest project files to", session_name)
            # TODO(ekl): race condition here since "up" breaks the lock.
            if self._tmp_conn:
                self._tmp_conn.disconnect()
                self._tmp_conn = None
            # Update session.
            self._up_session(project_id, session_name, self._project_dir)
    ...
    def _connect_to_session(self, project_id: str, session_name: str) -> None:
        ...
        if "python_version" in conn_info:
            local_py = "{}.{}".format(sys.version_info[0], sys.version_info[1])
            remote_py = conn_info["python_version"].rsplit(".", 1)[0]

            # TODO(ekl) avoid containers with version mismatch.
            if local_py != remote_py:
                self._log.info(
                    "Warning: server Python version {} does not match "
                    "local version {}, this is likely to cause serialization "
                    "errors.".format(remote_py, local_py)
                )
        ...
        self._log.info(
            "Connected to {}, view at: {}/o/anyscale-internal/"
            "projects/{}/sessions/{}".format(
    ...
    def _populate_cluster_config(self, cluster_yaml: Dict[str, Any]) -> None:
        """Populate cluster config with serverless compute options."""

        base_docker_image_cpu = ANYSCALE_BASE_DOCKER_IMAGE_CPU
        base_docker_image_gpu = ANYSCALE_BASE_DOCKER_IMAGE_GPU
        ...
        # TODO(ekl) ray client server should be started by default
        cluster_yaml["head_start_ray_commands"] = [
            "ray stop",
            "ulimit -n 65536; cd ~/"
            + os.path.basename(str(self._project_dir))

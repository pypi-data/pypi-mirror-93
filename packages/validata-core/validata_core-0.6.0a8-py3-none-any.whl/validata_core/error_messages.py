...
def constraint_name_and_message(cell_value, field_def: Dict, constraint_name: str):
    ...
    if constraint_name == "required":
        ...
        add_on = f" (au lieu de {len(cell_value)} actuellement)"
        msg = f"Le texte attendu doit comporter au moins {constraint_val} caractère(s)"
        return "Valeur trop courte", msg + add_on

    elif constraint_name == "maxLength":
        add_on = f" (au lieu de {len(cell_value)} actuellement)"
        msg = f"Le texte attendu ne doit pas comporter plus de {constraint_val} caractère(s)"
        return "Valeur trop longue", msg + add_on
...
def error_translate(err: ferr.Error, schema: frictionless.Schema):
    ...
    if isinstance(err, ferr.TypeError):
        ...
        # Number
            ...
            if "," in field_value:
                ...
                err[
                    "message"
                ] = f"Utilisez le point comme séparateur décimal ({value_str})."
            else:
                err[
                    "message"
        ...
        # Boolean
            ...
            err[
                "message"
            ] = f"Les valeurs acceptées sont {true_values_str} (vrai) et {false_values_str} (faux)"
    ...
    # missing header
        ...
        err["message"] = f"La colonne obligatoire `{field_name}` est absente du fichier"

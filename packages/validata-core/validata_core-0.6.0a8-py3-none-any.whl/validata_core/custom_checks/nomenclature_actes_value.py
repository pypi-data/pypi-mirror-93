...
class NomenclatureActesValueError(errors.CellError):
    ...
    tags = ["#body"]
    template = "La valeur '{cell}' ne respecte pas le format des nomenclatures d'actes ({note})"
    description = "Les nomenclatures d'actes doivent commencer par un préfixe particulier puis d'une oblique (/)"
...
class NomenclatureActesValue(Check):
    ...
    def validate_task(self):
        if self.__column not in self.table.schema.field_names:
            note = 'Colonne "%s" non trouvée' % self.__column
    ...
    def validate_row(self, row):
        ...
        if '/' not in cell_value:
            note = "caractère oblique (/) non trouvé"
            yield NomenclatureActesValueError.from_row(row, note=note, field_name=self.__column)
            return

        nomenc = cell_value[:cell_value.find('/')]
        if norm_str(nomenc) not in self.__nomenclatures:
            note = f"le préfixe de nomenclature Actes \"{nomenc}\" n'est pas reconnu"
            yield NomenclatureActesValueError.from_row(row, note=note, field_name=self.__column)
            return

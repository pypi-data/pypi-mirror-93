...
def validate_workflows(workflow_package: ModuleType, export_path: Optional[Path] = None) -> None:
    ...
    dags = _extract_airflow_dags_from_package(workflow_package)
    _validate_dags(dags)
...
def _extract_airflow_dags_from_package(workflow_package: ModuleType) -> List[DAG]:
    return _extract_objects_from_package_by_type(workflow_package, DAG)

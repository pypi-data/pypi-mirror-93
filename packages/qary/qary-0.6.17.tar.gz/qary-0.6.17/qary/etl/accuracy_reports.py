...
from qary.skills import corrected_qa, qa


filepath = pathlib.Path(constants.DATA_DIR, 'testsets', 'dialog', 'qa-2020-04-25.json')
...
def accuracy_report(*skills, scored_qa_pairs=scored_qa_pairs, upload=True):
    """  runs get_bot_accuracies() on a list of skills and adds results to labeled files and directories and uploads copy to digitalocean
...
def print_accuracy_report(verbose=True):
...
def question_of_longest_shortest_runtime(results):
    ...
    (333.0865876674652, 'When was Barack Obama President?', 8.409801483154297, 'Who is the Famous motivational speaker from Atlanta?')
    """
...
def print_report_summary(verbose=True):
    """ prints reports to terminal

import sys
import socket
from deepleaps.ipc.RunningCommand import *
from deepleaps.ipc.RunningModuleVisualizer import *
...
class RunnableModuleFactory(object):
    ...
    def reload_module(self, scripts):
    ...
    def default_setting(self, scripts):
        if 'default_setting_finish' not in dir(self):
            self.default_setting_finish = True
            self.add_from_script(scripts)
    ...
    def create_runnable_module(self, name, config):
        ...
        for module_name in self.command_modules:
            ...
            if config.command in dir(current_module):
...
class RunnableModuleController(object):
    def __init__(self, config, controller):
        ...
        self._runnable_define = Config.get_empty()
        if self.config.get('command_path', None) is not None:
            self.factory.job_queue.put(('LOAD', self.config.command_path))

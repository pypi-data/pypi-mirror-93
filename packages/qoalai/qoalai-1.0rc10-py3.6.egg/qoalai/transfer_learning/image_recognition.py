    ...
    def optimize(self, 
        ...
        for i in range(iteration):
            ...
            for j in range(subdivition):
                print ("---> befor batch generator")
                x_train, y_train = next(train_generator)
                print ("---> after batch generator")
                print (x_train.shape)
                print (y_train.shape)
                feed_dict = {}
            ...
            if report_mode: 
                if report_endpoint != None: 
                    ...
                    report_payload['name'] = training_name
                    report_payload['train_loss'] = round(t_loss, 3)
                    report_payload['train_acc'] = '-'
                    report_payload['train_recall'] = '-'
                    report_payload['train_precision'] =  '-'
                    report_payload['train_f1'] = '-'
                    report_payload['val_loss'] = round(t_loss, 3)
                    report_payload['val_acc'] = round(val_acc, 3)
                    report_payload['val_recall'] = recal
                    report_payload['val_precision'] = precision
                    report_payload['val_f1'] = f1

                    # post report to the endpoint
                    r = requests.post(report_endpoint, json=report_payload)
                    print ("Report status code: ", r.status_code)
                else: 
                    print ("Report endpoint is None...")

    ...
    def run(self, algo, metric='logloss', n_folds=10, n_iter=10,
        ...
        for i in range(n_iter):
            ...
            for train_index, test_index in kf.split(self.X, self.y):
                ...
                self.logger.info(f"Test fold mean accuracy: {clf.score(X_test, self.y[test_index])}")
            print_progress_bar(i + 1, n_iter, prefix='Progress', suffix=f'complete (CV {i + 1}/{n_iter})', length=30)

        end = time.time()
        self.logger.debug(f"Total elapsed time: {(end - start):.2f}")

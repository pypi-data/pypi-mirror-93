...
dark_color = '#292929'
light_color = '#ffffff'
# footprint_colors = {'good': '#3F81BC', 'bad': 'gray', 'best': 'green'}
footprint_colors = {'good': 'green', 'best': 'orange', 'bad': 'yellow'}
...
class App:
    def __init__(self, df_dataset: pd.DataFrame, df_metadata: pd.DataFrame,
        ...
        self.w_footprint_algo = pn.widgets.Select(options=df_footprint.index.unique(level='algo').to_list(), value='')
    ...
    def footprint_area(self, algo):
        ...
        footprint_good = hv.Polygons([border_good.tolist()], label='Good').opts(line_width=1, line_alpha=0.2,
                                                                                line_color='black',
        ...
        #                                                                      fill_alpha=.2, show_legend=True)
        footprint_best = hv.Polygons([border_best.tolist()], label='Best').opts(line_width=1, line_alpha=0.2,
                                                                                line_color='black',
    ...
    def get_pane(self):
        ...
        def selection_callback2(bbox, region_element, selection_expr, footprint, fp_on):
            ...
            if fp_on:
                ...
                return hv.Polygons([[[0, 0]]], label='Good').opts(fill_color=footprint_colors['good']) * \
                       hv.Polygons([[[0, 0]]], label='Best').opts(fill_color=footprint_colors['best'])

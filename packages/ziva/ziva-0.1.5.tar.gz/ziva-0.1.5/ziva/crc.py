...
def calculate_crc16(data:bytes) -> str:
    """Return calculated crc16 code from string input."""

    crc = CRC16().calculate(data)
    crc_hex = '{:4x}'.format(crc).replace(' ', '0').upper()
    return crc_hex

if __name__ == '__main__':
    input_crc = input('Enter crc input [bytes]:')
    input_crc_bytes = bytes(input_crc, encoding='utf-8')
    crc = calculate_crc16(data=input_crc_bytes)
    print (crc)
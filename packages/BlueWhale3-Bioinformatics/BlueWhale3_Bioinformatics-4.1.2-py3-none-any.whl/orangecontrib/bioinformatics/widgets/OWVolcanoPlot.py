...
def __(key):
    return i18n.t('bioinformatics.owvolcanoPlot.' + key)
...
class OWVolcanoPlot(OWDataProjectionWidget):
    ...
    class Warning(OWDataProjectionWidget.Warning):
        insufficient_data = Msg(
            __("msg.insufficient_data_to_compute")
        )

        gene_enrichment = Msg('{}, {}.')
        no_selected_gene_sets = Msg(__("msg.no_select_gene"))

    class Error(OWDataProjectionWidget.Error):
        exclude_error = Msg(__("msg.targe_label_most_include_one_value"))
        negative_values = Msg(__("msg.input_negative_value"))
        data_not_annotated = Msg(__("msg.input_data_not_annotated"))
        gene_column_id_missing = Msg(__("msg.cannot_identify_gene"))
    ...
    def _add_controls(self):
        box = gui.vBox(self.controlArea, __("box_targe_label"))

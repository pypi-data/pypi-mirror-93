...
def variable_labels_tooltip(var):
    ...
    if var.attributes:
        ...
        text += __('label.variable_labels')
        text += "<br/>".join(labels)
    return text


def discrete_variable_tooltip(var):
    text = __("label.discrete") % (escape(var.name), len(var.values))
...
def continuous_variable_toltip(var):
    text = __("label.continuous") % escape(var.name)
    text += variable_labels_tooltip(var)
    return text


def string_variable_tooltip(var):
    text = __("label.string") % escape(var.name)
    text += variable_labels_tooltip(var)
    return text
...
def standarditem_from_columngroup(colgroup):
    ...
    item.setToolTip(__("label.column_group_tip").format(escape(colgroup.name)))
    item.setFlags(item.flags() & ~Qt.ItemIsEditable)
...
class LabelSelectionWidget(QWidget):
    ...
    * The children of the current selected top level item are
    ...
    def __init__(self):
        ...
        l_box = group_box(self.tr(__("label.label")))
        v_box = group_box(self.tr(__("label.value")))

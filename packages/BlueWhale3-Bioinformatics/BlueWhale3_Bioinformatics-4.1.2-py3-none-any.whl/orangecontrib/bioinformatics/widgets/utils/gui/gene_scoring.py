...
class GeneScoringWidget(QWidget):
    ...
    scores = [
        gene_scoring_method(__("label.T_test"), score_t_test, TwoTail, TwoSampleTest),
        gene_scoring_method(__("label.mann_whitney"), score_mann_whitney, LowTail, TwoSampleTest),
        gene_scoring_method(__("label.hypergeometric_test"), score_hypergeometric_test, TwoTail, TwoSampleTest),
    ]
    ...
    def set_method_design_area(self, settings_var):
        ...
        radioButtons(
            ...
            [__("label.rest"), __("label.cluster")],
            callback=self.on_design_selection_changed,
            label=__("label.design"),
        )
    ...
    def set_test_type(self, settings_var):
        ...
        radioButtons(
            ...
            self.parent,
            self.test_type,
            [__('label.greater'), __('label.two_sided'), __('label.less')],
            list(map(lambda s: s.title(), ALTERNATIVES)),
            callback=self.on_test_type_changed,
            label=__("label.test_type"),
        )

...
def __(key):
    return i18n.t('bioinformatics.owGOBrowser.' + key)
...
class OWGOBrowser(widget.OWWidget):
    ...
    inputs = [
        (
        i18n.t("bioinformatics.common.cluster_data"), Orange.data.Table, "set_dataset", widget.Single + widget.Default),
        (i18n.t("bioinformatics.common.reference_data"), Orange.data.Table, "set_reference_dataset")
    ]

    outputs = [(i18n.t("bioinformatics.common.select_gene_data"), Orange.data.Table),
               (i18n.t("bioinformatics.common.enrichment_report"), Orange.data.Table)]
    ...
    def __init__(self, parent=None):
        ...
        # Input tab
        self.inputTab = gui.createTabPage(self.tabs, __("tab_input"))
        box = gui.widgetBox(self.inputTab, __("box.info"))
        self.infoLabel = gui.widgetLabel(box, __("row_no_data_input"))

        gui.button(
            box,
            self,
            __("btn.annotation_info"),
            callback=self.show_info,
            tooltip=__("tooltip.show_information"),
        )
        ...
        self.referenceRadioBox = gui.radioButtonsInBox(
            ...
            [__("btn.entire_genome"), __("btn.reference_set")],
            tooltips=[__("tooltip.reference_entire_genome"), __("tooltip.reference_input_signal_gene")],
            box=__("box.reference"),
        ...
        gui.radioButtonsInBox(
            ...
            self,
            "aspect_index",
            [__("btn.biological_process"), __("btn.cellular_component"), __("btn.molecular_function")],
            box=__("box.aspect"),
            callback=self.__invalidate,
        )

        # Filter tab
        self.filterTab = gui.createTabPage(self.tabs, __("tab_filter"))
        box = gui.widgetBox(self.filterTab, __("box.filter_go_node"))
        gui.checkBox(
            box,
            self,
            ...
            tooltip=__("tooltip.filter_by_num_of_gene"),
        )
        ...
        gui.spin(
            ...
            tooltip=__("tooltip.min_num_of_instances"),
        )
        ...
        gui.checkBox(
            ...
            __("checkbox_p_value"),
            callback=self.filter_and_display_graph,
            tooltip=__("tooltip.filter_by_term_p_value"),
        )
        ...
        gui.doubleSpin(
            ...
            callback=self.filter_and_display_graph,
            callbackOnReturn=True,
            tooltip=__("tooltip.max_term_p_value"),
        )

        # use filter_by_p_value for FDR, as it was the default in prior versions
        gui.checkBox(
            box, self, "filter_by_p_value", __("checkbox_fdr"), callback=self.filter_and_display_graph,
            tooltip=__("checkbox_fdr_tip")
        )
        gui.doubleSpin(
            gui.indentedBox(box),
            self,
            ...
            callback=self.filter_and_display_graph,
            callbackOnReturn=True,
            tooltip=__("tooltip.max_p_value"),
        )

        box = gui.widgetBox(box, __("box.significance_test"))

        gui.radioButtonsInBox(
            box,
            self,
            "prob_func",
            [__("btn.binomial"), __("btn.hypergeometric")],
            tooltips=[__("btn.binomial_tip"), __("btn.hypergeometric_tip")],
            callback=self.__invalidate,
        )  # TODO: only update the p values
        box = gui.widgetBox(self.filterTab, __("box.annotation_evidence_code"), addSpace=True)
        ...
        # Select tab
        self.selectTab = gui.createTabPage(self.tabs, __("tab_select"))
        box = gui.radioButtonsInBox(
            self.selectTab,
            self,
            "selection_direct_annotation",
            [__("btn.directly_or_indirectly"), __("btn.directly")],
            box=__("box.annotated_gene"),
            callback=self.example_selection,
        )

        box = gui.widgetBox(self.selectTab, __("box.output"), addSpace=True)
        gui.radioButtonsInBox(
            box,
            self,
            "selection_disjoint",
            btnLabels=[__("btn.all_select_gene"), __("btn.term_specific_gene"), __("btn.common_term_gene")],
            tooltips=[
                __("tooltip.output_gene_select_go_term"),
                __("tooltip.output_gene_appear_only_go_term"),
                __("output_gene_select_common_go_term"),
            ],
            callback=self.example_selection,
        )
        ...
        self.listView.setHeaderLabels([__("label.go_term"), __("label.cluster"), __("label.reference"), __("label.p_value"),
                           __("label.fdr"), __("label.gene"), __("label.enrichment")])
        ...
        self.sigTerms.setHeaderLabels([__("label.go_term"), __("label.cluster"), __("label.reference"), __("label.p_value"),
                           __("label.fdr"), __("label.gene"), __("label.enrichment")])
    ...
    def clear(self):
        self.infoLabel.setText(__("text.input_no_data"))
        self.warning(0)
        self.warning(1)
        self.clear_graph()

        self.send(__("text.genes_data"), None)
        self.send(__("text.enrichment_report"), None)
    ...
    def set_reference_dataset(self, data=None):
        ...
        self.referenceRadioBox.buttons[1].setText(__("text.reference_set"))
    ...
    def __start_download(self, files_list):
        ...
        self.setStatusMessage(__("text.downloading"))
    ...
    def enrichment(self):
        ...
        if not self.use_reference_dataset or self.ref_data is None:
            ...
            if ref_count == 0:
                ...
                self.referenceRadioBox.buttons[1].setText(__("text.ref_count").format(ref_count))
    ...
    def __on_enrichment_done(self, results):
        ...
        if not self.terms:
            self.warning(0, __("text.no_enrich_terms"))
        else:
            self.warning(0)
    ...
    def _update_enrichment_report_output(self):
        ...
        if terms:
            ...
            terms_table = None
        self.send(__("text.enrichment_report"), terms_table)
    ...
    def filter_and_display_graph(self):
        if self.input_data and self.originalGraph is not None:
            ...
            if self.originalGraph and not self.graph:
                self.warning(1, __("text.filter_terms"))
            else:
                self.warning(1)
            self.clear_graph()
            self.display_graph()
    ...
    def commit(self):
        ...
        if self.use_attr_names:
            ...
            self.send(__("text.genes_data"), new_data)
            ...
            for row_index, row in enumerate(self.input_data):
                ...
                self.send(__("text.genes_data"), selected)
    ...
    def show_info(self):
        ...
        label = QLabel(dialog)
        label.setText(__("text.ontology") + self.ontology.header if self.ontology else __("text.ontology_not_loaded"))
        dialog.layout().addWidget(label)

        label = QLabel(dialog)
        label.setText(
            __("text.annotations") + self.annotations.header.replace("!", "")
            if self.annotations
            else __("text.annotations_not_loaded")
        )
        dialog.layout().addWidget(label)
        dialog.show()

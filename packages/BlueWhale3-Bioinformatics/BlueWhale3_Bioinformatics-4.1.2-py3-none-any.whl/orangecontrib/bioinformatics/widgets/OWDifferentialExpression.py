...
def __(key):
    return i18n.t('bioinformatics.owdifferentialExpression.' + key)
...
def score_fold_change(a, b, **kwargs):
    ...
    if np.any(res < 0):
        res[res < 0] = float("nan")
        warning = __("warn.scoring_method")
    return res, warning
...
class Histogram(pg.PlotWidget):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.getAxis("bottom").setLabel(__("label.score"))
        self.getAxis("left").setLabel(__("label.counts"))
...
class OWDifferentialExpression(widget.OWWidget):
    ...
    inputs = [(i18n.t("bioinformatics.common.data"), Orange.data.Table, "set_data")]
    outputs = [
        (i18n.t("bioinformatics.common.data_subset"), Orange.data.Table, widget.Default),
        (i18n.t("bioinformatics.common.remain_data_subset"), Orange.data.Table),
        (i18n.t("bioinformatics.common.select_gene"), Orange.data.Table)
    ]
    ...
    Scores = [
        (__("gbox.fold_change"), TwoTail, TwoSampleTest, score_fold_change),
        (__("gbox.log2"), TwoTail, TwoSampleTest, score_log_fold_change),
        (__("gbox.t_test"), TwoTail, TwoSampleTest, score_ttest_t),
        (__("gbox.t_test_p_value"), LowTail, TwoSampleTest, score_ttest_p),
        (__("gbox.anova"), HighTail, VarSampleTest, score_anova_f),
        (__("gbox.anova_p_value"), LowTail, VarSampleTest, score_anova_p),
        (__("gbox.signal_noise_ratio"), TwoTail, TwoSampleTest, score_signal_to_noise),
        (__("gbox.mann_whitney"), LowTail, TwoSampleTest, score_mann_whitney_u),
        (__("gbox.hypergeometric_test"), TwoTail, TwoSampleTest, hypergeometric_test_score),
    ]
    ...
    def __init__(self, parent=None):
        ...
        self.dataInfoLabel = gui.widgetLabel(box, __("row.no_data_input"))
        self.dataInfoLabel.setWordWrap(True)
        self.selectedInfoLabel = gui.widgetLabel(box, "\n")

        box1 = gui.widgetBox(self.controlArea, __("box.score_method"))
        gui.comboBox(
            box1,
            self,
            "score_index",
        ...
        box = gui.widgetBox(self.controlArea, __("box.target_label"))
        ...
        box = gui.widgetBox(self.controlArea, __("box.select"))
        ...
        self.low_value_spin = gui.doubleSpin(
            ...
            maxv=1e6,
            step=1e-6,
            label=__("row.lower_threshold"),
            callback=self.update_boundary,
            callbackOnReturn=True,
        )

        check = gui.checkBox(box, self, "compute_null", __("checkbox_compute_null_distribution"),
                             callback=self.update_scores)
        ...
        pval_spin = gui.doubleSpin(box1, self, "alpha_value", minv=2e-7, maxv=1.0, step=1e-7,
                                   label=__("row.alpha_value"))
        pval_select = gui.button(box1, self, __("btn_select"), callback=self.select_p_best, autoDefault=False)
        ...
        box1 = gui.widgetBox(box, orientation='horizontal')
        gui.spin(box1, self, "n_best", 0, 10000, step=1, label=__("row.best_rank"))
        gui.button(box1, self, __("btn_select"), callback=self.select_n_best, autoDefault=False)

        box = gui.widgetBox(self.controlArea, __("box.output"))

        acbox = gui.auto_commit(box, self, "auto_commit", __("btn_commit"), box=None)
    ...
    def clear(self):
        ...
        self.clear_plot()
        self.dataInfoLabel.setText(__("text.no_input"))
    ...
    def update_data_info_label(self):
        if self.data is not None:
            ...
            text = __("text.sample_gene") % (samples, genes)
            text += __("text.sample_target") % (",".join(target_labels))
        else:
            text = __("text.no_input")
    ...
    def update_selected_info_label(self):
        if self.data is not None and self.scores is not None:
            ...
            defined_txt = __("text.undefined_score").format(count_undef, count_scores)

        elif self.data is not None:
            nselected = 0
            defined_txt = __("text.no_defined")
        else:
            nselected = 0
            defined_txt = ""

        self.selectedInfoLabel.setText(defined_txt + "\n" + __("text.selected_gene").format(nselected))
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert(
            (
                (__("report.score_method"), self.Scores[self.score_index][0]),
                (__("report.upper_threshold"), self.max_value),
                (__("report.lower_threshold"), self.min_value),
                (__("report.compute_null_distribution"), self.compute_null),
                (__("report.permutations"), self.permutations_count),
                (__("report.α-value"), self.alpha_value),
                (__("report.best_rank"), self.n_best),
            )
        )

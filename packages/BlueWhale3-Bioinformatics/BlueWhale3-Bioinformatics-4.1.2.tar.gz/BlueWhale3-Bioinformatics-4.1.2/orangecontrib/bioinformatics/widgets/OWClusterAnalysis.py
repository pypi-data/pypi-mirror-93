...
def __(key):
    return i18n.t('bioinformatics.owclusterAnalysis.' + key)
...
class OWClusterAnalysis(OWWidget):
    ...
    class Inputs:
        data_table = Input("Data", Table, label=i18n.t("bioinformatics.common.data"))
        custom_sets = Input('Custom Gene Sets', Table, label=i18n.t("bioinformatics.common.custom_gene_set"))

    class Outputs:
        selected_data = Output("Selected Data", Table, label=i18n.t("bioinformatics.common.select_data"))
        gene_scores = Output("Gene Scores", Table, label=i18n.t("bioinformatics.common.gene_score"))
        gene_set_scores = Output("Gene Set Scores", Table, label=i18n.t("bioinformatics.common.gene_set_score"))
    ...
    class Warning(OWWidget.Warning):
        gene_enrichment = Msg('{}, {}.')
        no_selected_gene_sets = Msg(__("msg.no_selected_gene_sets"))

    class Error(OWWidget.Error):
        no_cluster_indicator = Msg(__("msg.no_cluster_indicator"))
        gene_as_attributes = Msg(__("msg.gene_as_attributes"))
        organism_mismatch = Msg(__("msg.organism_mismatch"))
        cluster_batch_conflict = Msg(__("msg.cluster_batch_conflict"))
    ...
    def __init__(self):
        ...
        # Info
        info_box = vBox(self.controlArea, __("box.info"))
        ...
        self.cluster_indicator_box = widgetBox(self.controlArea, __("box.cluster_indicator"))
        ...
        box = widgetBox(self.controlArea, __("box.batch_indicator"))
        ...
        # Gene scoring
        box = widgetBox(self.controlArea, __("box.gene_score"))
        ...
        # Gene Sets widget
        gene_sets_box = widgetBox(self.controlArea, __("box.gene_set"))
        ...
        box = vBox(self.controlArea, __("box.custom_gene_set"))
        ...
        genes_filter = widgetBox(splitter, __("box.filter_gene"), orientation=QHBoxLayout())
        spin(
            genes_filter,
            self,
            'max_gene_count',
            0,
            10000,
            label=__("label_count"),
            tooltip=__("tooltip_min_gen_count"),
        ...
        doubleSpin(
            ...
            1.0,
            0.0001,
            label=__("label_p_vale"),
            tooltip=__("tooltip_enrichment_max_p_value"),
        ...
        doubleSpin(
            ...
            0.0001,
            label=__("label_fdr"),
            tooltip=__("tooltip_max_error_discover_rate"),
        ...
        gene_sets_filter = widgetBox(splitter, __("box.filter_gene_set"), orientation=QHBoxLayout())
        spin(
            gene_sets_filter,
            self,
            'min_gs_count',
    ...
    def __update_info_box(self):
        ...
        if self.input_genes_ids:
            info_string += __('info.samples_clusters').format(
                self.input_data.X.shape[0], len(self.clusters) if self.clusters else '?'
            )
            info_string += __("info.unique_genes").format(len(self.input_genes_ids))
        else:
            info_string += __("info.no_data")

        if self.custom_data:
            info_string += __("info.set_genes").format(self.custom_data.X.shape[0], self.num_of_custom_sets)
    ...
    def __create_temp_class_var(self):
        """ See no evil !"""
        cluster_indicator_name = __("label_name")
        row_profile = None
        new_cluster_values = []

...
class OWdictyExpress(OWWidget):
    ...
    def __init__(self):
        ...
        # Login Section
        box = gui.widgetBox(self.controlArea, __("box_login"))

        self.namefield = gui.lineEdit(
            box, self, "username", __("row.username"), labelWidth=100, orientation='horizontal',
            callback=self.auth_changed
        )

        self.password = ''
        self.passfield = gui.lineEdit(
            box, self, "password", __("row.password"), labelWidth=100, orientation='horizontal',
            callback=self.auth_changed
        )
        ...
        box = gui.widgetBox(self.controlArea, __("box_output"), addSpace=True)
        ...
        self.refresh_button = gui.button(self.controlArea, self, __("btn_refresh"), callback=self.refresh)
    ...
    def load_experiments(self):
        if self.res:
            ...
            # status message
            self.setStatusMessage(__("state.download_exp"))
    ...
    def commit(self):
        ...
        if selected_item:
            ...
            # status message
            self.setStatusMessage(__("state.download_exp_data"))

...
def __(key):
    return i18n.t('bioinformatics.owmarkerGenes.' + key)
...
class OWMarkerGenes(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        using_local_files = widget.Msg(__("msg_using_local_files"))
    ...
    def __init__(self) -> None:
        ...
        self.filter_line_edit.setPlaceholderText(__("placeholder_filter_marker_gene"))
        layout.addWidget(self.filter_line_edit, 0, 0, 1, 3)

        # define available markers view
        self.available_markers_view = TreeView()
        box = gui.vBox(self.mainArea, __("box.available_marker"), addToLayout=False)
        ...
        box = gui.vBox(self.mainArea, __("box.select_marker"), addToLayout=False)
    ...
    def _init_description_area(self, layout: QLayout) -> None:
        ...
        """
        box = gui.widgetBox(self.mainArea, __("box.desc"), addToLayout=False)
        ...
        # description filed
        self.descriptionlabel.setText(__("label_desc"))
    ...
    def _init_control_area(self) -> None:
        ...
        """
        box = gui.widgetBox(self.controlArea, __("box.database"), margin=0)
        self.source_index = -1
        self.db_source_cb = gui.comboBox(box, self, 'source_index')
        self.db_source_cb.activated[int].connect(self._set_db_source_index)

        box = gui.widgetBox(self.controlArea, __("box.organism"), margin=0)
        ...
        box = gui.widgetBox(self.controlArea, __("box.group_by"), margin=0)
        self.group_by_cb = gui.comboBox(
            box, self, 'selected_root_attribute', items=[__("row.cell_type"), __("row.function")], callback=self._setup
        )
    ...
    def _update_description(self, view: TreeView) -> None:
        ...
        if len(selection) > 1 or len(selection) == 0 or qmodel.node_from_index(selection[0]).data_row is None:
            self.descriptionlabel.setText(__("desc.gene_info"))
        else:
            data_row = qmodel.node_from_index(selection[0]).data_row
            self.descriptionlabel.setHtml(
                __("desc.gene_name") + f" {data_row['Name']}<br/>" +
                __("desc.entrez_id") + f" {data_row['Entrez ID']}<br/>" +
                __("desc.cell_type") + f" {data_row['Cell Type']}<br/>" +
                __("desc.function") + f" {data_row['Function']}<br/>" +
                __("desc.reference") + f" <a href='{data_row['URL']}'>{data_row['Reference']}</a>"
            )
    ...
    def _update_data_info(self) -> None:
        ...
        self.info.set_output_summary(__("output_selected").format(str(len(sel_model))))

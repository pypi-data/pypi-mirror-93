...
def __(key):
    return i18n.t('bioinformatics.owgenialisexpressions.' + key)
...
class FilterByDateModified(IntEnum):
    ...
    @staticmethod
    def labels():
        return [__('row.any_time'), __('row.past_hour'), __('row.past_24_hours'), __('row.past_week'), __('row.past_month')]
...
class SortBy(IntEnum):
    ...
    def labels():
        return [
            ...
            __('label.name_a_z'),
            __('label.name_z_a'),
            __('label.contributor_a_z'),
            __('label.contributor_z_a'),
            __('label.date_modified_new'),
            __('label.date_modified_old'),
        ]
...
class QuantileTransformDist(IntEnum):
    ...
    @staticmethod
    def values():
        return [__('btn.normal'), __('btn.uniform')]
...
class CollapsibleFilterComponent(OWComponent, QObject):
    ...
    FILTER_FULL_TEXT_LABEL = __('search')
    TOGGLE_BTN_LABEL = __('narrow_search')
    FILTER_NAME_LABEL = __('filter_name')
    FILTER_CONTRIB_LABEL = __('contributor')
    FILTER_OWNER_LABEL = __('owner')
    FILTER_MODIFIED_LABEL = __('date_modified')
    SORTING_LABEL = __('sorting')
...
class NormalizationComponent(OWComponent, QObject):
    ...
    BOX_TITLE = __('normalization')
    QUANTILE_NORM_LABEL = __('quantile_normalization')
    LOG_NORM_LABEL = __('log_norm')
    Z_SCORE_LABEL = __('z_score')
    QUANTILE_TRANSFORM_LABEL = __('quantile_transform')
    ...
    def __init__(self, parent_widget, parent_component):
        ...
        self.z_score_axis_btn = gui.radioButtons(
            ...
            self,
            'z_score_axis',
            btnLabels=[__('btn.columns'), __('btn.rows')],
...
class SignInForm(QDialog):
    def __init__(self, flags, *args, **kwargs):
        ...
        self.setWindowTitle(__('title'))
        self.setFixedSize(400, 250)

        self.server_cb_label = QLabel(__('server'))
        self.server_cb = QComboBox(self)
        self.server_cb.addItems(RESOLWE_URLS)
        self.server_cb.setEditable(True)

        self.username_label = QLabel(__('username'))
        self.username_line_edit = QLineEdit(self)
        self.username_line_edit.setPlaceholderText(__('enter_username'))
        ...
        self.password_label = QLabel(__("password"))
        self.password_line_edit = QLineEdit(self)
        self.password_line_edit.setPlaceholderText(__("enter_password"))
...
class OWGenialisExpressions(widget.OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        table = Output('Expressions', Table, label=i18n.t("bioinformatics.common.expressions"))

    class Warning(widget.OWWidget.Warning):
        no_expressions = Msg(__("msg.no_expressions"))
        no_data_objects = Msg(__("msg.no_data_objects"))
        unexpected_feature_type = Msg(__("msg.unexpected_feature_type"))
        multiple_feature_type = Msg(__("msg.multiple_feature_type"))
    ...
    def __init__(self):
        ...
        # Control area
        self.info_box = gui.widgetLabel(gui.widgetBox(self.controlArea, __("box.info"), margin=3), __("msg.no_data"))

        self.exp_type_combo = gui.comboBox(
            self.controlArea, self, 'exp_type', label=__("box.exp_type"), callback=self.on_output_option_changed
        )
        self.proc_type_combo = gui.comboBox(
            self.controlArea, self, 'proc_type', label=__("box.process_name"), callback=self.on_output_option_changed
        )
        self.input_anno_combo = gui.comboBox(
            self.controlArea,
            self,
            'input_annotation',
            label=__("box.expression_source"),
        ...
        box = gui.widgetBox(self.controlArea, __('box.sign_in'))
        self.user_info = gui.label(box, self, '')
        self.server_info = gui.label(box, self, '')

        box = gui.widgetBox(box, orientation=Qt.Horizontal)
        self.sign_in_btn = gui.button(box, self, __('box.sign_in'), callback=self.sign_in, autoDefault=False)
        self.sign_out_btn = gui.button(box, self, __('btn.sign_out'), callback=self.sign_out, autoDefault=False)

        self.commit_button = gui.auto_commit(self.controlArea, self, 'auto_commit', __("btn.commit"), box=False)

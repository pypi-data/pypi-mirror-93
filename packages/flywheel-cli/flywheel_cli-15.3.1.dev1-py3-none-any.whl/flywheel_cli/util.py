...
class SDKClient:
    ...
    def get_deid_profile(self, group, project):
        ...
        if resolved_prj:
            try:
                response = self.call_api(
                    f"/projects/{resolved_prj.id}/settings",
                    "GET",
                    response_type=object,
                )

                if response.get("deid_profile", None):
                    return response["deid_profile"]
        ...
        if resolved_grp:
            try:
                response = self.call_api(
                    f"/groups/{resolved_grp.id}/settings",
                    "GET",
                    response_type=object,
                )
        ...
        if response.get("deid_profile", {}).get("dicom"):

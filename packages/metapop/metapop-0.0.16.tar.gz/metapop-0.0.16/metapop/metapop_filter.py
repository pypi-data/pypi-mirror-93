...
def filt(files, command_base, b_d_base, threads, mag_dict, length_dict, ref_file):
	...
	for sample in cleaned_bams:
		...
		#sort = ["samtools", "sort", "-@", str(threads), "-o", command_base[1] + "/MetaPop/02.Filtered_Samples/"+base_name+"_sorted.bam", files+"/"+sample]
		#subprocess.call(sort, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
		print("file passed.")
		md_files.append([files+"/"+sample, base_name, ref_file, command_base[1]])
...
def fill_mdz(args):
	...
	calmd = ["samtools", "calmd", "-u", "-b", "-", ref]
	
	md_to_bam = ["samtools", "view", "-h", "-b", "-", "-o", output]
	
	view = subprocess.Popen(sam_to_bam, stdout=subprocess.PIPE)
	md = subprocess.Popen(calmd, stdin=view.stdout, stdout = subprocess.PIPE)

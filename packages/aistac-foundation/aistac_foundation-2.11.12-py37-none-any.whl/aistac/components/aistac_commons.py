...
class AistacCommons(object):
    ...
    def list_formatter(value: Any) -> list:
        ...
        if isinstance(value, (list, tuple, set)):
            return list(value)
        if isinstance(value, (collections.KeysView, collections.ValuesView, collections.ItemsView)):
            return list(value)
...
class AnalyticsSection(object):
    ...
    def is_attribute(self, attribute: str):
        """Checks if an attribute exists in the section"""
    ...
    def __delattr__(self, item):
        raise AttributeError(
            "{} is an immutable class and attributes can't be removed".format(self.__class__.__name__))
...
class DataAnalytics(object):
    ...
    def get(self, section: str, attribute: str, default: Any=None):
        """returns a specific attribute from a section"""
        if self.is_section(section):
            inst = eval(f"self.{section}")
            if inst.is_attribute(attribute):
                return eval(f"self.{section}.{attribute}")
        return default
    ...
    @staticmethod
    def get_tree_branches(analytics_blob: dict) -> list:
        ...
        def get_level(_analysis: dict, tree: list):
            for name, values in _analysis.items():
                tree.append(values.get('address'))
    ...
    @staticmethod
    def from_branch(analytics_blob: dict, branch: str) -> DataAnalytics:
        """ given a branch name, returns the Data Analytics on that tree branch

        :param analytics_blob: an analytics blob created through associative analytics
        :param branch: the analytics blob branch to retrieve
        ...
        for k in keys:
            if index:
                ...
                if result.get(k, {}).get('sub_category'):
                    result = result.get(k, {}).get('sub_category', {})
            index = not index

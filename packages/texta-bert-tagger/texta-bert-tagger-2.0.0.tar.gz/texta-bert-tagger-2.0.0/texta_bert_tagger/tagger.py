...
class BertTagger:
    ...
    def load_pretrained(self, state_dict=None):
        ...
        if not BertTagger.is_roberta_model(model_to_load):
            model = BertForSequenceClassification.from_pretrained(model_to_load, state_dict = state_dict, num_labels = self.config.n_classes)
        else:
            model = RobertaForSequenceClassification.from_pretrained(model_to_load, state_dict = state_dict, num_labels = self.config.n_classes)
    ...
    @staticmethod
    def save_pretrained_model(bert_model: str, save_dir: str, state_dict: json = None, logger = None):
        ...
        if not os.path.exists(model_dir) or len(os.listdir(model_dir)) < 1:
            ...
            if not BertTagger.is_roberta_model(bert_model):
                model = BertForSequenceClassification.from_pretrained(bert_model, state_dict = state_dict)
            else:
                model = RobertaForSequenceClassification.from_pretrained(bert_model, state_dict = state_dict)
            model.save_pretrained(model_dir)
    ...
    @staticmethod
    def save_pretrained_tokenizer(bert_model: str, save_dir: str, logger = None):
    ...
    @staticmethod
    def download_pretrained_models(bert_models: List[str], save_dir: str, logger = None):
        ...
        for bert_model in bert_models:
            try:
                BertTagger.save_pretrained_tokenizer(bert_model, save_dir, logger=logger)
                BertTagger.save_pretrained_model(bert_model, save_dir, logger=logger)
            except:
    ...
    def _get_pos_index(self):
        ...
        elif "true" in self.config.classes:
    ...
    def evaluate_model(self, iterator: DataLoader) -> TaggingReport:
        ...
        for i, batch in enumerate(iterator):
            ...
            # probabilities_all_classes = logits.softmax(1).numpy()

            # Select probabilities only for the true class
            # probabilities = [probabilities_all_classes[i][label_id] for i, label_id in enumerate(labels_flat)]
    ...
    def load_tokenizer(self):
        ...
        if not BertTagger.is_roberta_model(model_to_load):
            ...
            tokenizer = RobertaTokenizer.from_pretrained(model_to_load)

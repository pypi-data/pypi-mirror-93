"""
Taeg Assign command-line utility
"""
...
from .utils import run_tests, write_taeg_config_file, run_generate_autograder
from .output import generate_student_zipfile
...
def main(args):
    ...
    try:
        ...
        # generate the .taeg file if needed
        if not assignment.is_rmd and assignment.service or assignment.save_environment:
            if assignment.is_r:
                warnings.warn(
                    "Taeg Service and serialized environments are unsupported with R, "
                    "configurations ignored"
                )
            else:
                write_taeg_config_file(master, result, assignment)

        generate_student_zipfile(result, assignment)

...
from .cell_generators import (
    gen_load_cell, gen_init_cell, gen_markdown_response_cell, gen_export_cells, gen_submit_cells, gen_check_all_cell, 
...
def transform_notebook(nb, assignment, args):
    ...
    if assignment.load_cell and not args.no_load_cell and assignment.is_python:
        transformed_cells = [gen_load_cell(assignment)] + transformed_cells
    ...
    if assignment.submit_cell and not args.no_submit_cell and assignment.is_python:
        submit_cell = assignment.submit_cell
        if submit_cell is True:
            submit_cell = {}

        transformed_cells += gen_submit_cells(
            submit_cell.get('instructions', ''), 
            assignment
        )

...
def generate_student_zipfile(result_dir, assignment):

    student_dir = result_dir / 'student'
    assignment_id = assignment.service["assignment_id"]
    cur_dir = os.getcwd()
    os.chdir(student_dir)
    zip_cmd = ["zip", "-r", f"{assignment_id}.zip", "."]
    zipped = subprocess.run(zip_cmd, stderr=PIPE)

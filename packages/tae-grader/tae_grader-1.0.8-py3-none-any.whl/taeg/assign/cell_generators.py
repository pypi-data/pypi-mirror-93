...
def gen_load_cell(assignment):
    """
    Generates a cell to load Taeg in the notebook. The code cell has the following contents:

    .. code-block:: python

        # loadialize Taeg
        %%capture 
        files = "https://github.com/taecoding/data/raw/master/assignment/{class_id}/{assignment_id}.zip"
        !pip install tae-grader && wget $files && unzip -o {assignment_id}.zip
    
    Returns:
        ``nbformat.NotebookNode``: the load cell
    """
    service = assignment.service

    cell = nbformat.v4.new_code_cell()
    source_lines = ['%%capture']
    source_lines.append('files = "{assignment_url}/{class_id}/{assignment_id}.zip"'.format(**service))
    source_lines.append('!pip install tae-grader && wget $files && unzip -o {}.zip'.format(service['assignment_id']))
    cell.source = '\n'.join(source_lines)
    lock(cell)
    return cell
...
def gen_submit_cells(instruction_text, assignment, pdf=True, filtering=True):
    """
    Generates submit cells that instruct the student the run a code cell calling 
    ``taeg.Notebook.submit`` to perform their submission. The Markdown cell contains:
    ...
    .. code-block:: markdown
        ...
        Make sure you have run all cells in your notebook in order before running the cell below, so 
        that all images/graphs appear in the output. The cell below will generate a zipfile for you 
        to submit. **Please save before submiting!**
    ...
    .. code-block:: python
        ...
        # Save your notebook first, then run this cell to submit your submission.
        grader.submit()
        
    Args:
        instruction_text (``str``): extra instructions for students when submiting
        assignment (``taeg.assign.assignment.Assignment``): the assignment configurations
        pdf (``bool``, optional): whether a PDF is needed
        filtering (``bool``, optional): whether PDF filtering is needed
    
    Returns:
        ``list`` of ``nbformat.NotebookNode``: generated submit cells
    """
    instructions = nbformat.v4.new_markdown_cell()
    instructions.source = "## Submission\n\nMake sure you have run all cells in your notebook in order before " \
    "running the cell below. **Please save before submiting!**"
    
    if instruction_text:
        instructions.source += '\n\n' + instruction_text

    submit = nbformat.v4.new_code_cell()
    source_lines = ["# Save your notebook first, then run this cell to submit your submission."]
    source_lines.append(f"%notebook {assignment.master}")
    source_lines.append("grader.submit()")
    submit.source = "\n".join(source_lines)

    lock(instructions)
    lock(submit)

    return [instructions, submit, nbformat.v4.new_markdown_cell(" ")]     # last cell is buffer

############################################
##### Tests for taeg service start.py #####
############################################

import os
import testing.postgresql
import taeg.service.create
import queries
import json
...
from taeg.service import start
from taeg.service.create import main as create
...
class TestServiceTrackHandler(AsyncHTTPTestCase, TestCase):
    ...
    def get_app(self):
        ...
        class TrackApplication(Application):
            def __init__(self):
                settings = dict(notebook_dir="test_submissions")
                handlers = [(r"/track", start.TrackHandler)]
                Application.__init__(self, handlers, **settings)
                self.db = queries.TornadoSession(queries.uri(**dsn))
    ...
    def test_track_fail(self):
        ...
        request = {'api_key': 'key1', 'track_info': track_info_invalid} 
        resp1 = self.fetch('/track', method='POST', body=json.dumps(request))

        request = {'api_key': 'invalid_key', 'track_info': track_info}
        resp2 = self.fetch('/track', method='POST', body=json.dumps(request))

        self.assertEqual(resp1.code, 200)
        self.assertEqual(resp2.code, 200)
        
        self.assertIn('score', resp1.body.decode("utf-8"))
        self.assertIn('invalid API key', resp2.body.decode("utf-8"))
    ...
    def test_one_track(self):
        ...
        request = {'api_key': 'key1', 'track_info': track_info} 
        resp1 = self.fetch('/track', method='POST', body=json.dumps(request))
        ...
        self.cursor.execute(
            """
            SELECT * FROM track_questions WHERE user_id = 1 ORDER BY user_id ASC
            """
        )
        results = self.cursor.fetchall()
        self.assertEqual(len(results), 1)
    ...
    def reset_timestamps(self):
        self.cursor.execute(
            """
            UPDATE track_questions SET timestamp = (%s)
            """,
            [datetime.utcnow() - timedelta(seconds=121)]
        )

...
from taeg.generate.autograder import main as autograder
...
class TestAutograder(TestCase):
    ...
    def test_gradescope(self):
        ...
        subprocess.run(["docker", "build", TEST_FILES_PATH, "-t", "taeg-gradescope-test"], check=True)
        # self.assertEqual(len(build.stderr), 0, build.stderr.decode("utf-8"))

        # launch the container and return its container ID
        launch = subprocess.run(["docker", "run", "-dt", "taeg-gradescope-test", "/autograder/run_autograder"], stdout=PIPE, stderr=PIPE)

...
from taeg.check.logs import Log
from taeg.check.notebook import Notebook, _TAEG_LOG_FILENAME
...
class TestLogs(TestCase):
    ...
    def test_Notebook_check(self):
        ...
        for test_file in os.listdir(self.test_directory):
            ...
            self.grading_results[test_name] = grader.check_local(test_name)

        log = Log.from_file(_TAEG_LOG_FILENAME)
    ...
    def test_question_entry(self):
        ...
        for test_file in os.listdir(self.test_directory):
            if os.path.splitext(test_file)[1] != ".py":
                continue
            test_name = os.path.splitext(test_file)[0]
            self.entry_results[test_name] = grader.check_local(test_name)
    ...
    def test_shelve(self):
        ...
        entry.shelve(env, delete=True, filename = _TAEG_LOG_FILENAME, ignore_modules=['calendar'])
    ...
    def test_Log_getItem(self):
        ...
        entry1.flush_to_file(_TAEG_LOG_FILENAME)
    ...
    def test_Log_iter(self):
        #log = Log.from_file(_TAEG_LOG_FILENAME, ascending = True)
        #list of entries
        #logIter = log.iter()
        ...
        entry1.flush_to_file(_TAEG_LOG_FILENAME)
        entry2.flush_to_file(_TAEG_LOG_FILENAME)
        entry3.flush_to_file(_TAEG_LOG_FILENAME)

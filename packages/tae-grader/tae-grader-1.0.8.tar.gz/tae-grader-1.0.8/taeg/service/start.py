...
from taeg.utils import pac_now
...
class HealthHander(BaseHandler):
    async def get(self):
        now = datetime.now()
        self.write(f'<h1>Hello {now}</h1>')
        self.finish()
...
class SummaryHandler(RequestHandler):
    async def get(self):
        global CONN
        df = pd.read_sql("SELECT track_id, assignment_id, class_id, question_id, user_id, username, score, timestamp FROM track_questions", con=CONN)
        # results = await self.db.query("SELECT track_id, assignment_id, class_id, question_id, user_id, score FROM track_questions")
...
class TrackHandler(RequestHandler):
    """
    Processes and validates student question submission
    ...
    async def post(self):
        ...
        try:
            ...
            assert 'track_info' in request.keys(), 'track contains no track_info'
            ...
            await self.track(track_info, api_key)
        except AssertionError as e:
            print("Track failed due to client error: {}".format(e))
    ...
    async def validate(self, track_info, api_key):
        """
        Ensures a submision is valid by checking user credentials, submission frequency, and
        validity of question file.

        Arguments:
            track_info (``dict``): track_info in JSON form
        ...
        username = results.as_dict()['username'] or results.as_dict()['email'].split('@')[0] if len(results) > 0 else None
        results.free()
        assert user_id, 'invalid API key: {}'.format(api_key)

        # get assignment and class id from notebook metadata 
        assert 'class_id' in track_info, 'missing required metadata attribute: class_id'
        assert 'assignment_id' in track_info, 'missing required metadata attribute: assignment_id'
        assert 'question_id' in track_info, 'missing required metadata attribute: question_id'
        assert 'score' in track_info, 'missing required metadata attribute: score'

        class_id = track_info['class_id']
        assignment_id = track_info['assignment_id']
        question_id = track_info['question_id']
        ...
        results = await self.db.query(
            ...
            WHERE user_id = %s AND assignment_id = %s AND class_id = %s AND question_id = %s
            ORDER BY timestamp DESC 
            LIMIT 1
            """, 
            (user_id, assignment_id, class_id, question_id)
        )
        last_tracted = results.as_dict()['timestamp'] if len(results) > 0 else None
        results.free()

        if last_tracted:
            delta = pac_now() - last_tracted
            # rate_limit = 120
        ...
        return (user_id, username, assignment['class_id'], assignment_id, assignment['assignment_name'], question_id, score)

    async def track(self, track_info, api_key):
        """
        ...
        """
        try:
            user_id, username, class_id, assignment_id, assignment_name, question_id, score = await self.validate(track_info, api_key)
        except TypeError as e:
            print("Track Type failed for user with API key {} due to error: {}: ".format(api_key, e))
            return
        except AssertionError as e:
            print("Track Assert failed for user with API key {} due to error: {}".format(api_key, e))
            self.write_error(400, message=e)
            return

        # fetch next submission id
        results = await self.db.query("SELECT nextval(pg_get_serial_sequence('track_questions', 'track_id')) as id")
        track_id = results.as_dict()['id']
        results.free()

        print("Successfully received track {} from user {}".format(track_id, username))

        # store submission to database
        results = await self.db.query("INSERT INTO track_questions (track_id, assignment_id, class_id, question_id, score, user_id, username, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                                            [track_id, assignment_id, class_id, question_id, score, user_id, username, pac_now()])
        assert results, 'track failed'
        results.free()

        self.track_id = track_id

        self.write('Track {} received.'.format(track_id))
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if 'message' in kwargs:
            self.write('Track failed: {}'.format(kwargs['message']))
        else:
            self.write('Track failed.')
...
class Application(tornado.web.Application):
    ...
    def __init__(self):
        ...
        handlers = [
            ...
            (r"/auth", LoginHandler),
            (r"/courses/(.*)", StaticFileHandler, {"path": "/home/azureuser/courses/cs21-assignments/public"}),
            (r"/health", HealthHander)
        ]

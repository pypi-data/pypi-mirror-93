...
def replace_notebook_instances(nb_path):
    """
    Replaces the creation of ``taeg.Notebook`` instances in a notebook at ``nb_path`` that have custom 
    ...
    instance_regex = r"taeg.Notebook\([\"'].+[\"']\)"
    for cell in nb['cells']:
        source = get_source(cell)
        for i, line in enumerate(source):
            line = re.sub(instance_regex, "taeg.Notebook()", line)
            source[i] = line
        cell['source'] = "\n".join(source)

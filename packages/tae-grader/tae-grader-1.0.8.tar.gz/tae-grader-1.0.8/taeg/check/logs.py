...
class EventType(Enum):
    ...
    Attributes:
        ...
        INIT: initialization of an ``taeg.Notebook`` object
...
class LogEntry:
    """
    An entry in Taeg's log. Tracks event type, grading results, success of operation, and errors
    thrown.

    Args:
        event_type (``EventType``): the type of event for this entry
        results (``list`` of ``taeg.test_files.abstract_test.TestCollectionResults``, optional): the 
    ...
    Attributes:
        ...
        unshelved (``list`` of ``str``): a list of variable names that were unable to be pickled during
            shelving
        results (``list`` of ``taeg.test_files.abstract_test.TestCollectionResults``): grading results 
    ...
    def __repr__(self):
        if self.question:
            return "taeg.logs.LogEntry(event_type={}, question={}, success={}, timestamp={})".format(
                self.event_type, self.question, self.success, self.timestamp.isoformat()
            )

        return "taeg.logs.LogEntry(event_type={}, success={}, timestamp={})".format(
    ...
    def get_results(self):
        ...
        Returns:
            ``list`` of ``taeg.test_files.abstract_test.TestCollectionResults``: the results at this 
    ...
    def shelve_environment(env, variables=None, ignore_modules=[]):
        ...
        for k, v in env.items():
            ...
            # don't store modules or taeg.Notebook instances
...
class Log:
    ...
    def get_results(self, question):
        ...
        Returns:
            ``taeg.test_files.abstract_test.TestCollectionResults``: the most recent result for the question

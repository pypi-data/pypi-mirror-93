...
class GradingResults:
    ...
    Initialize with a list of ``taeg.test_files.abstract_test.TestCollectionResults`` objects and 
    ...
    Args:
        results (``list`` of ``taeg.test_files.abstract_test.TestCollectionResults``): the list of 
            grading results
    
    Attributes:
        raw_results (``list`` of ``taeg.test_files.abstract_test.TestCollectionResults``): the 
    ...
    def verify_against_log(self, log, ignore_hidden=True):
        ...
        Args:
            log (``taeg.logs.Log``): the log to verify against
    ...
    def to_gradescope_dict(self, config={}):
        ...
        dictionary of configurations for the Gradescope assignment generated using Taeg Generate.

...
def from_yaml(raw_data, client=None, **kwargs):
    ...
    '''
    for obj in yaml.load_all(raw_data):
        create_update_replace(obj, client, **kwargs)


def create_update_replace(obj, client=None, **kwargs):
    ''' invoke the K8s API to create or replace a kubernetes object.
        ...
        @return: response object from Kubernetes API call
    '''
    k8s_Api = find_k8s_api(obj, client)
    try:
        res = invoke_api(k8s_Api, 'create', obj, **kwargs)
        logging.debug(
            'K8s: %s created -> uid=%s',
            describe(obj),
            res.metadata.uid
        ...
        try:
            ...
            res = invoke_api(k8s_Api, 'patch', obj, **kwargs)
            logging.debug(
                'K8s: %s PATCHED -> uid=%s',
                describe(obj),
                res.metadata.uid
            ...
            try:
                ...
                res = invoke_api(k8s_Api, 'delete', obj, **kwargs)
...
def find_k8s_api(obj, client=None):
    ...
    grp = ''.join(
        part.capitalize() for part in grp.rsplit('.k8s.io',
                                                 1)[0].split('.')
    )
    ver = ver.capitalize()
...
def invoke_api(k8sApi, action, obj, **args):
    ...
    func_name = '%s_%s' % (action, kind)
    if hasattr(k8sApi, func_name):
        # namespace agnostic API
        function = getattr(k8sApi, func_name)
    else:
        func_name = '%s_namespaced_%s' % (action, kind)
        function = getattr(k8sApi, func_name)
        args['namespace'] = namespace
    if not 'create' in func_name:

...
class MaskCompositor(ImageViewer):
  ...
  def addLabelImg(self, labelImg: np.ndarray, names='Label', ignoreBackground=True):
    ...
    if not isinstance(names, t.Sequence):
  ...
  def save(self, saveFile: FilePath='', cropToViewbox=False, toClipboard=False) -> bool:
    ...
    saveImg = pg.fn.makeQImage(bg, alpha=True, copy=False, transpose=False)
    painter = QtGui.QPainter(saveImg)

    # Only capture the region with images
    imRect = self.imgItem.mapRectToScene(self.imgItem.boundingRect())
    visibleMasks = [p.name() for p in params_flattened(self.masksParam) if p.value()]
    oldVis = self.legend.isVisible()
    if not visibleMasks:
      self.legend.hide()
    oldScale = 0.5/np.array(self.imgItem.pixelSize())
    try:
      # Legend does not scale itself, handle this
      self.legend.scale(*oldScale)
      self._updateLegendPos()
      scene.render(painter, QtCore.QRectF(saveImg.rect()), imRect)
    finally:
      self.legend.scale(*(1/oldScale))
      self._updateLegendPos()
    if oldVis:
      self.legend.show()
    if cropToViewbox:

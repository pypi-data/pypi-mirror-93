...
class ImageViewer(CompositionMixin, pg.PlotWidget):
  ...
  def __init__(self, imgSrc: np.ndarray=None, **kwargs):
    ...
    under the mouse (if self.mouseEnabled = True)    
    """
    ...
    self.sigSceneMouseMoved.connect(self._onMouseMove)
    self.mouseEnabled = True

  def _onMouseMove(self, pos: QtCore.QPointF):
    relpos = self.imgItem.mapFromScene(pos)
    xyCoord = np.array([relpos.x(), relpos.y()], dtype=int)
    if (self.imgItem.image is None
        or np.any(xyCoord < 0)
        or np.any(xyCoord > self.imgItem.image.shape[:2][::-1])):
      return
    cursorClr = self.imgItem.image[xyCoord[1], xyCoord[0],...].astype(int)
    self.updateCursorInfo(xyCoord, cursorClr)

  def updateCursorInfo(self, xyPos: np.ndarray, pxColor: np.ndarray):
    ...
    if self.pxColorLbl is None:
      return
    self.pxColorLbl.setText(f'Pixel Color: {pxColor}')
    ...
    if np.mean(pxColor) > 127:
      ...
      fontColor = 'white'
    self.pxColorLbl.setStyleSheet(f'background:rgba{tuple(pxColor)}; color:{fontColor}')
  ...
  def widgetContainer(self, asMainWin=False, **kwargs):
    """
...
class MaskCompositor(ImageViewer):
  def __init__(self, img: np.ndarray=None):
    ...
    # Create properties
    # -----
    self.legend = CompositorLegend(offset=(5,5), horSpacing=5, brush='cccd', pen='k')
    ...
    self.masksParam = getParamChild(self.toolsEditor.params, 'Masks')
    ...
    with nameFormatter.set(newFmt):
      ...
      self.toolsEditor.registerFunc(self.toggleAllVisible, namePath=('Masks',))
    if img is not None:
      self.setImage(img)
  ...
  def _getUniqueName(self, baseName:str):
    ii = 2
    name = baseName
  ...
  def addLabelImg(self, labelImg: np.ndarray, name='Label', ignoreBackground=True):
    """
    ...
    :param name: Base name of each label. E.g. if 3 labels are present in the image, the
      resulting masks will be named <name> 1, <name> 2, <name 3>
    ...
    newNames = [name]*len(newMasks)
    self.addMasks(newMasks, newNames)
  ...
  def clearOverlays(self):
    ...
    self.legend.clear()
    self.toolsEditor.params.child('Masks').clearChildren()
  ...
  def save(self, saveFile: FilePath='', cropToViewbox=False, toClipboard=False) -> bool:
    """
    :param saveFile:
    ...
    """
    pm = self.imgItem.getPixmap()
    painter = QtGui.QPainter(pm)
    # TODO: Get legend exporting too
    visibleMasks = [p.name() for p in params_flattened(self.masksParam) if p.value()]
    for ii, (name, entry) in enumerate(self.recordDf.iterrows()):
      if name in visibleMasks:
        entry['item'].paint(painter)
        legItems = self.legend.items[ii]
        # legItems[0].paint(painter)
        # legItems[1].paint(painter, None)
    painter.end()
    saveImg = pm.toImage()
    if cropToViewbox:
      ...
      saveImg = saveImg.copy(QtCore.QRect(*pts))
    if toClipboard:
      QtWidgets.QApplication.clipboard().setImage(saveImg)

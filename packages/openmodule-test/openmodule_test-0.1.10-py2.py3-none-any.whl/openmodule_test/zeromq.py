...
class ZMQTestMixin:
    ...
    def setUp(self):
        ...
        if self.protocol == "inproc://":
            ...
            random_port = random.randrange(30000, 60000)
            sub = f"tcp://127.0.0.1:{random_port}"
            pub = f"tcp://127.0.0.1:{random_port + 1}"
    ...
    def rpc(self, channel: str, type: str, request, timeout=5, receive_response=True) -> Optional[dict]:
    ...
    def assertSubscription(self, *topics):
        assert all(x in self.topics for x in topics), (

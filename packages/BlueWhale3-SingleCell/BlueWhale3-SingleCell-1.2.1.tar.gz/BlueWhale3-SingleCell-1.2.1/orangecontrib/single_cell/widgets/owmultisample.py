...
def __(key):
    return i18n.t('single_cell.owmultisample.' + key)
...
class RecentLineEdit(QLineEdit):
    def __init__(self, parent):
        ...
        self.setText(__("text.recent_files"))
...
class OWMultiSample(owloaddata.OWLoadData):
    ...
    class Information(owloaddata.OWLoadData.Information):
        file_in_list = Msg(__('msg.file_in_list'))

    HEADER_SCHEMA = (
        ("selected", ""),
        ("name", __("label.file")),
        ("source", __("label.source_name")),
        ("cells", __("label.cells")),
        ("genes", __("label.genes")),
        ("size", __("label.size")),
        ("sparsity", __("label.sparsity")),
        ("remove", "")
    )
    OUTPUT_TYPES = [__("btn.intersection"), __("btn.union")]
    ...
    def __init__(self):
        ...
        file_label.setText(__('text.add_files'))

        self.recent_combo.setLineEdit(RecentLineEdit(self))
        self.recent_combo.setCurrentText(__('text.recent_files'))
        ...
        open_button.setText(__("text.browse"))
        ...
        gui.radioButtons(
            ...
            label=__('btn.output_genes'), orientation=Qt.Horizontal,
            callback=self._output_type_changed
        )
        gui.rubber(output_box)
        cb_box = gui.auto_commit(output_box, self, "auto_commit", __("btn.load_data"))
    ...
    def __file_item(self, exists):
        ...
        item.setEditable(False)
        if not exists:
            item.setToolTip(__('tooltip_missing_file_system'))
    ...
    def send_report(self):
        ...
        self.report_items(
            __('report.concatenated_sample'),
            [(__("report.file"), ", ".join(paths) if paths else __("report.no_file")),
             (__("report.output_genes"), self.OUTPUT_TYPES[self.output_type])])

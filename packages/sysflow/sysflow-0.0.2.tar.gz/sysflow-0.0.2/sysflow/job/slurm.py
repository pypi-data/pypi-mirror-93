...
class Slurm:
    ...
    def merge_config(slurm, res, res_):
    ...
    def config(self, **args_dict):
        if is_empty(user_config):
            ...
            user_args_dict = args_dict
        else:
            user_args_dict = load(user_config)
            self.merge_config(user_args_dict, args_dict)
            dump(user_args_dict, user_config)

        slurm_bash = self.sub_script_head(user_args_dict)

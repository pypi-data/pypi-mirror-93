...
class IntervalTask(TaskAbstract):
    ...
    def add_job(self, interval, func, *args, **kwargs):
        ...
        if args or kwargs:
            _func = Utils.func_partial(func, *args, **kwargs)
        else:
            _func = func

        return self._scheduler.add_job(_func, r'interval', seconds=interval)
...
class CronTask(TaskAbstract):
    ...
    def add_job(self, crontab, func, *args, **kwargs):
        ...
        return self._scheduler.add_job(_func, CronTrigger.from_crontab(crontab, TIMEZONE))

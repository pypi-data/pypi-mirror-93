...
from keras.layers import (Input, LSTM, Embedding, GlobalMaxPooling1D, Lambda, Dense, Layer, Multiply, Bidirectional)
from keras.models import Model
from keras.optimizers import Adam
from logging import getLogger
import numpy as np
from theano.tensor import TensorType as Tensor
...
class BiLSTMSiameseNetwork(KerasSiameseModel):
    ...
    def create_model(self) -> Model:
        ...
        if self.triplet_mode:
            dist = Lambda(self._pairwise_distances, output_shape=(None, None))([lstm_c, lstm_r])
        else:
            dist = Lambda(self._diff_mult_dist, output_shape=[8 * self.hidden_dim])([lstm_c, lstm_r])
    ...
    def create_score_model(self) -> Model:
        ...
        if self.triplet_mode:
            ...
            dist_score = Lambda(lambda x: self._euclidian_dist(x), name="score_model", output_shape=(None,))
            score = dist_score([emb_c, emb_r])
        else:
            score = self.model.get_layer("score_model").output
            score = Lambda(lambda x: 1. - K.squeeze(x, -1), output_shape=(None,))(score)
        score = Lambda(lambda x: 1. - x, output_shape=(None,))(score)

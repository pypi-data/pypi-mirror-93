...
class ESNFeedbackRegressor(BaseEstimator, MultiOutputMixin, RegressorMixin):
    ...
    def predict(self, X):
        ...
        self._node_to_node.transformer_list[0][1].set_regressor(regressor=self._regressor)
    ...
    def _validate_hyperparameters(self):
        ...
        if not self.nodes_to_nodes or self.nodes_to_nodes is None:
            self.nodes_to_nodes = [('default', FeedbackNodeToNode())]
        else:
...
class ESNFeedbackClassifier(ESNFeedbackRegressor, ClassifierMixin):
    """Echo State Network feedback classifier.

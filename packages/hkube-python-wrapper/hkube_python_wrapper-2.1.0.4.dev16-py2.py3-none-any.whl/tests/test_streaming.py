...
def test_streaming_manager_load():
    ...
    try:
        ...
        print('1 ****************************' +str( process.memory_info().rss))
        streamingManagaerB.startMessageListening()
        time.sleep(1)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('2 ****************************' +str( process.memory_info().rss))
        streamingManagaerA.sendMessage(bytearray(1000000))
        time.sleep(4)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('3 ****************************' +str( process.memory_info().rss))
        # assert resultsAtC['flowLength'] == 1
        # assert resultsAtC['flowFirstSource'] == 'C'
        # assert resultsAtC['msg'] == 'stam_klum'
        # send to none default flow
        # streamingManagaerB.sendMessage(bytearray(1000000), 'master')
        time.sleep(5)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('4 ****************************' +str( process.memory_info().rss))
        # assert resultsAtC['flowLength'] == 2
        # assert resultsAtC['flowFirstSource'] == 'C'
        streamingManagaerB.setupStreamingListeners(listen_config, parentsDel, 'B')
        process = psutil.Process(os.getpid())
        print('4.5 ****************************' +str( process.memory_info().rss))
        streamingManagaerB.setupStreamingListeners(listen_config, parents, 'B')

        process = psutil.Process(os.getpid())
        print('4.55 ****************************' +str( process.memory_info().rss))
        streamingManagaerA.stopStreaming()
        # streamingManagaerB.stopStreaming()

        gc.collect()
        process = psutil.Process(os.getpid())
        print('5 ****************************' +str( process.memory_info().rss))
        streamingManagaerA.setupStreamingProducer(statsInvoked, producer_configA, ['B'], 'A')
        streamingManagaerB.setupStreamingListeners(listen_config, parents, 'B')
        streamingManagaerB.registerInputListener(onMessageAtB)
        time.sleep(1)
        ...
        print('6 ****************************' +str( process.memory_info().rss))

        streamingManagaerB.startMessageListening()
        time.sleep(1)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('7 ****************************' +str( process.memory_info().rss))
        streamingManagaerA.sendMessage(bytearray(1000000))

        time.sleep(4)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('8 ****************************' +str( process.memory_info().rss))
        # assert resultsAtC['flowLength'] == 1
        # assert resultsAtC['flowFirstSource'] == 'C'
        # assert resultsAtC['msg'] == 'stam_klum'
        # send to none default flow
        ...
        gc.collect()
        process = psutil.Process(os.getpid())
        print('9 ****************************' +str( process.memory_info().rss))
        # assert resultsAtC['flowLength'] == 2
        # assert resultsAtC['flowFirstSource'] == 'C'
        streamingManagaerA.stopStreaming()
        streamingManagaerB.setupStreamingListeners(listen_config, parentsDel, 'B')
        ...
        for _ in range(1,10):
            ...
            gc.collect()
            process = psutil.Process(os.getpid())
            print('10 ****************************' + str(process.memory_info().rss))
            streamingManagaerA.setupStreamingProducer(statsInvoked, producer_configA, ['B'], 'A')
            streamingManagaerB.setupStreamingListeners(listen_config, parents, 'B')
            streamingManagaerB.registerInputListener(onMessageAtB)
            time.sleep(1)
            ...
            print('11 ****************************' + str(process.memory_info().rss))

            streamingManagaerB.startMessageListening()
            time.sleep(1)
            gc.collect()
            process = psutil.Process(os.getpid())
            print('12 ****************************' + str(process.memory_info().rss))
            streamingManagaerA.sendMessage(bytearray(1))

            time.sleep(4)
            gc.collect()
            process = psutil.Process(os.getpid())
            print('13 ****************************' + str(process.memory_info().rss))
            # assert resultsAtC['flowLength'] == 1
            # assert resultsAtC['flowFirstSource'] == 'C'
            # assert resultsAtC['msg'] == 'stam_klum'
            # send to none default flow
            ...
            gc.collect()
            process = psutil.Process(os.getpid())
            print('14 ****************************' + str(process.memory_info().rss))
            streamingManagaerA.stopStreaming()
            streamingManagaerB.setupStreamingListeners(listen_config, parentsDel, 'B')

        gc.collect()
        process = psutil.Process(os.getpid())
        print('15 ****************************' + str(process.memory_info().rss))
        streamingManagaerA.setupStreamingProducer(statsInvoked, producer_configA, ['B'], 'A')
        streamingManagaerB.setupStreamingListeners(listen_config, parents, 'B')
        streamingManagaerB.registerInputListener(onMessageAtB)
        time.sleep(1)
        ...
        print('16 ****************************' + str(process.memory_info().rss))

        streamingManagaerB.startMessageListening()
        time.sleep(1)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('17 ****************************' + str(process.memory_info().rss))
        streamingManagaerA.sendMessage(bytearray(1000000))

        time.sleep(4)
        gc.collect()
        process = psutil.Process(os.getpid())
        print('18 ****************************' + str(process.memory_info().rss))
        # assert resultsAtC['flowLength'] == 1
        # assert resultsAtC['flowFirstSource'] == 'C'
        # assert resultsAtC['msg'] == 'stam_klum'
        # send to none default flow
        ...
        gc.collect()
        process = psutil.Process(os.getpid())
        print('19 ****************************' + str(process.memory_info().rss))
    finally:
        streamingManagaerA.stopStreaming()
        streamingManagaerB.stopStreaming()
        gc.collect()
        process = psutil.Process(os.getpid())
        print('20 ****************************' +str( process.memory_info().rss))

...
class connect_api_ConnectHelper:
    ...
    def connectSyncRequest(method,path,headers,params = None,rqlParams = None,data = None,fileArg = None,fileName = None,fileContent = None,currentRequest = None,logLevel = None):
        ...
        return connect_Env.getApiClient().syncRequestWithLogger(method,url,headers,data,fileArg,fileName,fileContent,None,logger)
...
class connect_api_impl_ApiClientImpl(connect_Base):
    ...
    def syncRequest(self,method,url,headers,body,fileArg,fileName,fileContent,certificate,logLevel = None):
        return self.syncRequestWithLogger(method,url,headers,body,fileArg,fileName,fileContent,certificate,connect_Env.getLogger(),logLevel)

    def syncRequestWithLogger(self,method,url,headers,body,fileArg,fileName,fileContent,certificate,logger,logLevel = None):
...
class connect_models_AssetRequest(connect_models_IdModel):
    ...
    def getConversation(self):
        convs = connect_models_Conversation.list(connect_api_Query().equal("instance_id",self.id),self)
        conv = (convs.get(0) if ((convs.length() > 0)) else None)
        if (((conv is not None) and ((conv.id is not None))) and ((conv.id != ""))):
            return connect_models_Conversation.get(conv.id,self)
        else:
            return None
    ...
    def _updateConversation(self,message):
        ...
        if (conversation is not None):
            try:
                conversation.createMessage(message,self)
            except BaseException as _g:
                None
...
class connect_models_Conversation(connect_models_IdModel):
    ...
    def createMessage(self,text,request = None):
    ...
    @staticmethod
    def create(instanceId,topic,request = None):

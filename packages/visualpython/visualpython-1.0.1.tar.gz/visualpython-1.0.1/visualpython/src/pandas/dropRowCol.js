define([
    ...
    PandasPackage.prototype.bindOptions = function() {
        ...
        $(document).on('change', this.wrapSelector('#i0'), function() {
            var axis = $(that.wrapSelector('#axis')).val();
            // axis = 열(1) 이면 컬럼목록 표시
            if (axis == '1') {
                pdGen.vp_bindColumnSource(that, this, ['i1']);
            } else {
                $(that.wrapSelector('#i1')).replaceWith(function() {
                    return $(`<input id="i1" type="text" class="vp-input" />`);
                });
            }
        });
        ...
        // column bind
        // on dataframe change
        $(document).on('change', this.wrapSelector('#axis'), function() {
            var axis = $(this).val();
            // axis = 열(1) 이면 컬럼목록 표시
            if (axis == '1') {
                pdGen.vp_bindColumnSource(that, $(that.wrapSelector('#i0')), ['i1']);
            } else {
                $(that.wrapSelector('#i1')).replaceWith(function() {
                    return $(`<input id="i1" type="text" class="vp-input" />`);
                });
        ...
        // move 'axis' option to required box
        $(this.wrapSelector('#axis')).closest('tr').insertBefore(
            $(this.wrapSelector('#i1')).closest('tr')
        );

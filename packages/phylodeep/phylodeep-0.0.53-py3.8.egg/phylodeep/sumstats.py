...
def coordinates_comp(events):
    ...
    y_axis = []
    x_axis = []
    for j in range(len(binscor) - 1):
        y_axis.append(np.average(events[floor(binscor[j]):floor(binscor[j + 1]), 0]))
        x_axis.append(np.average(events[floor(binscor[j]):floor(binscor[j + 1]), 2]))

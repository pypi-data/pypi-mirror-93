...
def encode_into_summary_statistics(tree, sampling_proba):
    ...
    :param tree: ete3.Tree, on which the summary statistics will be computed
    ...
    summaries.extend(sumstats.colless(tree))
    summaries.extend(sumstats.sackin(tree))
    ...
    return summaries, rescale_factor


def encode_into_most_recent(tree, sampling_proba):
    """Rescales all trees from tree_file so that mean branch length is 1,
    then encodes them into full tree representation (most recent version)

    :param tree: ete3.Tree, that we will represent in the form of a vector

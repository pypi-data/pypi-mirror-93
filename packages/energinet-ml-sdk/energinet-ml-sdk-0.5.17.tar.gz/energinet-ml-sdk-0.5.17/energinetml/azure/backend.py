...
class AzureBackend(AbstractBackend):
    ...
    def create_compute_cluster(self, workspace, name, vm_size, min_nodes, max_nodes,
        ...
        except azureml.exceptions._azureml_exception.ComputeTargetException as e:
            # It seems impossible to get the error message without
            # getting the rest of the API's raw HTTP response...
            message = re.findall('"message":"([^"]+)"', str(e))[0]
            raise self.BackendException(message)
    ...
    def get_local_training_context(self, force_download):
        ...
        """
        return AzureLocalTrainingContext(force_download)
    ...
    def submit_model(self, model, params):
        ...
        with model.temporary_folder() as path:
            run = experiment.submit(ScriptRunConfig(

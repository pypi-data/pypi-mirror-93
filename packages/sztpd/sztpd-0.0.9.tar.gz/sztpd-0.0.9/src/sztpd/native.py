...
class NativeViewHandler(RouteHandler):
		...
		try:D=A.loop.run_until_complete(B)
			...
			if _V in D[A.dal.app_ns+K]:
				for C in D[A.dal.app_ns+K][_V]:
					H=C[_Q];B=_handle_plugin_created('',{_V:C},'',A);A.loop.run_until_complete(B)
					if _R in C:
						for E in C[_R][_e]:X=E[_Q];I='FOO/plugins/plugin='+H+'/BAR';B=_handle_function_created('',{_e:E},I,A);A.loop.run_until_complete(B)
	...
	async def handle_get_restconf_root(D,request):
		...
		B,H=utils.check_http_headers(E,D.supported_media_types,accept_required=_S)
	...
	async def handle_get_opstate_request(C,request):
		...
		if A.status==401:return A
		B,J=utils.check_http_headers(D,C.supported_media_types,accept_required=_S)
	...
	async def handle_put_config_request_lower_half(D,data_path,query_dict,request_body):
		...
		async with D.fifolock(Write):
			...
			if H==_S:return web.Response(status=201)
...
async def _handle_function_created(watched_node_path,jsob,jsob_data_path,opaque):
	...
	B.plugins[C][_R][A]=D
async def _handle_function_deleted(watched_node_path,opaque):A=watched_node_path;B=opaque;C=re.sub(_f,_g,A);D=A.rsplit('=',1)[1];del B.plugins[C][_R][D]
...
async def _handle_asymmetric_public_key_created_or_changed(watched_node_path,jsob,jsob_data_path,obj):
	...
	except PyAsn1Error as C:raise dal.CreateOrChangeCallbackFailed(_A0+B.rsplit(_A,1)[0]+_M+str(C)+_N)
	K=serialization.load_der_public_key(J);O=A[_C][_A1];H=base64.b64decode(O)
	if A[_C][_G]==_a:
		try:P,G=decode_der(H,asn1Spec=rfc5915.ECPrivateKey())
		except PyAsn1Error as C:raise dal.CreateOrChangeCallbackFailed(Q+B.rsplit(_A,1)[0]+_M+str(C)+_N)
	elif A[_C][_G]==_b:
...
async def _handle_asymmetric_private_key_created_or_changed(watched_node_path,jsob,jsob_data_path,obj):
	...
	else:raise dal.CreateOrChangeCallbackFailed(_A2+A[_C][_G]+_Z+B.rsplit(_A,1)[0]+_M+str(D)+_N)
	...
	except PyAsn1Error as D:raise dal.CreateOrChangeCallbackFailed(_A0+B.rsplit(_A,1)[0]+_M+str(D)+_N)
...
async def _handle_verify_asymmetric_key_and_certs_post_sweep(watched_node_path,conn,opaque):
	...
	if _O in D[_C]:
		if _P in D[_C][_O]:
			...
			for H in G:
				N=H['jsob'][_P][_i];O=base64.b64decode(N);P,V=decode_der(O,asn1Spec=rfc5652.ContentInfo());Q=utils.degenerate_cms_obj_to_ders(P)
				for R in Q:
...
async def _handle_trust_anchor_cert_created_or_changed(watched_node_path,jsob,jsob_data_path,obj):
	...
	except PyAsn1Error as J:raise dal.CreateOrChangeCallbackFailed('Parsing trust anchor certificate CMS structure failed for '+B.rsplit(_A,1)[0]+_M+str(J)+_N)

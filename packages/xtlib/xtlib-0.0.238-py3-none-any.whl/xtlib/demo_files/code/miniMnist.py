...
class Trainer():
    ...
    def print_env_vars(self):
        ...
        for key in keys:
            if key.startswith("E_"):
                # skip massive amounts of ITP-injected env vars
                skip_count += 1
                continue

            if skip_count:
                print("    ===> <{:,} E_XXX environment variables skipped>".format(skip_count))
                skip_count = 0
                
            value = os.environ[key]

            if len(value) > max_value_len:
                value = value[0:max_value_len] + "..."
    ...
    def run(self):
        ...
        if run and run.store.database:
            # ensure we close all logging
            print("miniMNST: mongo call stats")
            run.store.database.print_call_stats()
            run.close()

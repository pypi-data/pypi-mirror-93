...
class Run(object):
    def __init__(self, run_data, primary_metric):
        self.run_name = run_data["run_name"]   # for debugging
    ...
    def define_hyperparameters(self, records):
        ...
        for record in records:
            ...
            if not "," in value_string:
                console.print("skipping hparam: {}={}".format(name_string, value_string))
                continue

            console.print("adding hparam: {}={}".format(name_string, value_string))
        ...
        for record in run_reports:
            ...
            for hp_name, hp_value in run.hpname_hpvalue_list:
                ...
                if hp_value not in hparam.value_setting_dict:
                    ...
                    ignore_run = True
                    console.print("skipping run: {} (hp_value={}, dict={})".format(run.run_name, hp_value, hparam.value_setting_dict))
            ...
            #console.print("added run: {}".format(run.run_name))

...
class FreshServiceApi:
    ...
    def call_api(self, method, resource, params=None, payload=None):
        headers = {"Authorization": "Basic %s" % self.encode_apikey_for_fs(), "content-type": "application/json"}
        param_string = ''
        if params:
            param_string += '?'
        ...
        if method == 'get':
            ...
            request = requests.post(endpoint_url, headers=headers, json=payload)
        elif method == 'put':
            request = requests.put(endpoint_url, headers=headers, json=payload)
        elif method == 'delete':
            request = requests.delete(endpoint_url, headers=headers, json=payload)

...
class DspIngest:
    ...
    def call_for_dspingest(self):
        config_dir='config'
        abs_config_dir=os.path.join(dsp_ingest.__path__[0],config_dir)
        logs=get_logfiles(abs_config_dir)
        # iterate through the config.yamls meant for log-generator and get the logfiles.
        for log in logs:
            log_file_handle=open(log)
            process = Process(target=process_target_func(log_file_handle))

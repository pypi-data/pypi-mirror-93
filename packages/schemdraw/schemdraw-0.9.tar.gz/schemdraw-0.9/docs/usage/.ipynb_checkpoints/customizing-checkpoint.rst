...
However, because the segments are added to the figure when `add` is called, the drawing will not pick up manual changes automatically.
Pass the `refresh=True` parameter to draw to force a re-draw and pick up the customized segments.

...
class Prediction(BaseRecord):
    ...
    def validate_inputs(self):
        ...
        if not isinstance(
            ...
            raise TypeError(
                f"prediction_label {self.prediction_label} has type {type(self.prediction_label)}, but must be one "
                f"of: str, bool, float, int "
            )
...
class Actual(BaseRecord):
    ...
    def validate_inputs(self):
        ...
        if not isinstance(
            ...
            raise TypeError(
                f"actual_label {self.actual_label} has type {type(self._convert_element(self.actual_label))}, but "
                f"must be one of: str, bool, float, int "
            )
    ...
    def validate_inputs(self):
        ...
        if self.feature_importances is not None and bool(self.feature_importances):
            for k, v in self.feature_importances.items():
                if not isinstance(self._convert_element(v), float):
                    raise TypeError(
        ...
        for row, v in enumerate(self.prediction_ids):
            ...
            if self.features is not None:
                ...
                for column, name in enumerate(self.feature_names):
                    val = self._get_value(value=self.features[row][column], name=name)
                    if val is not None:

...
class TestNer(unittest.TestCase):
    ...
    def test_ner_webio(self):
        to_test = ['en.ner.atis','en.ner.weibo']
        for r in to_test:
            print(nlu.load(r).predict('Hello wrdl'))

    def test_ner_airline(self):
        data = [' what is the arrival time in san francisco for the 755 am flight leaving washington',
                '"i want to fly from baltimore to dallas round trip"]']
        ner_df = nlu.load('en.ner.aspect.airline').predict(data)
        print(ner_df)

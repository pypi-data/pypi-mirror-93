...
from froeling_lib import Froeling, SerialPortIOError, ResponseReadError
try:
    import socket
    import selectors
    import types
...
def format_temperature(value_bytes):
    """
    ...
    most significant byte first, where the integer is the actual temperature measurement, multiplied
    by 2.
    :param value_bytes: bytes with a signed integer that is twice the actual temperature in °C
    :return: a string with formatted temperature
    """
    return "{:.1f}°C".format(int.from_bytes(value_bytes, "big", signed=True) / 2.0)
...
class FroelingProxyServer:
    ...
    If any invalid characters are received (i.e. [^0-9A-Fa-f\n\r]), the connection is hung up
    immediately.
        ...
        if newline_index is not None:
            ...
            if request:
                try:
                    ...
                    data.outb += b"!" + str(e).encode("UTF-8") + b"\n"

...
class Game(Window):
    ...
    def on_draw(self):
        ...
        if self.game_over_status:
            ...
            if int(self.total_time) % 2 == 0:
                draw_text('按下 Tab 键重新开始游戏',
                          WIDTH // 2 - 150, HEIGHT // 2 - 200, color.WHITE,
                          font_name=('SimHei', 'PingFang'), font_size=20)
    ...
    def upgrade_till_next_level(self):
        if self.player.level >= self.score:
            draw_text(f'距离下一等级还差{self.player.level - self.score}分',
                      350, HEIGHT - 25, color.WHITE,
                      font_name=('SimHei', 'PingFang'), font_size=20)
        
        elif self.player.upgrade[2] < self.score < self.achievement:
            draw_text(f'距离达成目标还差{self.achievement - self.score}分',
                      350, HEIGHT - 25, color.WHITE,
                      font_name=('SimHei', 'PingFang'), font_size=20)

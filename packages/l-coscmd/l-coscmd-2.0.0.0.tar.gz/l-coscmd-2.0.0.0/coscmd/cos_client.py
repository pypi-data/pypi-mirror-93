    ...
    def __init__(self, appid, region, endpoint, bucket, secret_id, secret_key, token=None,
        ...
        if self._l5 != '':
            ...
            if len(splits) == 2:
                ...
                if ret == 0:
                    self._ip = qos['hostIp']
                    self._port = int(qos['hostPort'])
        logger.debug("l5 ip:{ip},port:{port}".format(ip=self._ip, port=self._port))
        self._oldurl = ''
...
class Interface(object):
        ...
        try:
            if conf._endpoint != "":
                ...
                sdk_config = qcloud_cos.CosConfig(Region=conf._region,
                                                  ...
                                                  UA=self._ua,
                                                  Timeout=self._timeout)
            if conf._ip != "" and conf._port != 0:
                sdk_config.set_ip_port(conf._ip, conf._port)
    ...
    def use_l5(self):
        return self._conf._ip != "" and self._conf._port != 0
    
    def get_host(self):
        host = '{bucket}.cos.{region}.myqcloud.com'.format(bucket=self._conf._bucket, region=self._conf._region)
        return host
    ...
    def list_part(self, cos_path):
        ...
        try:
            while IsTruncated == "true":
                ...
                http_headers = {}
                if self.use_l5():
                    self._conf._oldurl = url
                    host = self.get_host()
                    url = url.replace(host, self._conf._ip + ":" + str(self._conf._port))

...
from super_sweetest.globals import g, now
from super_sweetest.log import logger
from super_sweetest.utility import mkdir
...
def get_air_screenshot(file_path):
    sleep(1.5)
    snapshot(file_path)


def check_for_pil(image1, image2):
...
class Snapshot:
    ...
    def web_screen(self, step, element):
        # 截图
        self.web_appium_screen(step, element)

    def web_check(self, step, element):
        # 比对
        self.web_appium_check(step, element)
    ...
    # ################################################
    # appium
    def android_screen(self, step, element):
        # 截图
        self.web_appium_screen(step, element)

    def android_check(self, step, element):
        # 比对
        self.web_appium_check(step, element)

    def android_shot(self, step, element):
        self.android_screen(step, element)
        self.android_check(step, element)


    #########################################
    # air_android shot
    def air_android_screen(self, step):
        # 截图
        ...
        if g.snapshot or self.screen_flag or self.element_flag:
            ...
            get_air_screenshot(step['snapshot']['Real_Screen'])
            if screen_v:
                g.var[screen_v] = step['snapshot']['Real_Screen']


    def air_android_check(self, step):
        ...
        if Path(step['snapshot'].get('Expected_Screen', '')).is_file():
            ...
            diff, differ = check_for_pil(image1,image2)
            if differ == 0.0:
                # 图片间没有任何不同
            ...
            get_air_screenshot(step['snapshot']['Expected_Screen'])
            deep(step['snapshot']['Expected_Screen'])


    def air_android_shot(self, step):
        self.air_android_screen(step)
        self.air_android_check(step)
    ...
    # 公用 截图比对
    def web_appium_screen(self, step, element):
        # 截图 selenium/appium
        screen_v = self.output.get('#ScreenShot', '')
        element_v = self.output.get('#ElementShot', '')
    ...
    def web_appium_check(self, step, element):
        def deep(src):
            # 把不需要比较的部分贴白
            if self.expected.get('#blank'):
                blank(src, eval(self.expected.get('#blank')))

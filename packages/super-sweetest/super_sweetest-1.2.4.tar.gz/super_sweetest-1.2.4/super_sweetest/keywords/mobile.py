...
from super_sweetest.utility import compare
from appium.webdriver.common.touch_action import TouchAction
from selenium.common.exceptions import ElementClickInterceptedException
from super_sweetest.servers.elements_help import tap_or_input_elements_index
...
def input(step):
    ...
    if isinstance(data['text'], tuple):
        ...
        # class_name elements 通过index 输入,先判断locator传的是不是dict
        if isinstance(element_location,dict):
            tap_or_input_elements_index(g,element_location,data_=data)
        else:
...
def tap(step):
    ...
    if isinstance(element, str):
        ...
        if ',' in element:
            ...
            element_location = locating_element(element, 'CLICK')
            if isinstance(element_location,dict): # class_name elements 通过index 点击,先判断locator传的是不是dict

                tap_or_input_elements_index(g, element_location,data_=None)
            else:

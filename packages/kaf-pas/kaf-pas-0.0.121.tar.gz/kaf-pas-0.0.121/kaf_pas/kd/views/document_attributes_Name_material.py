...
def Document_attributes_Name_material_Fetch(request):
    return JsonResponse(
        DSResponse(
            request=request,
            data=Document_attributes.objects.
                select_related('attr_type').
                filter(attr_type__code='Наименование_материала').
                get_range_rows1(
                request=request,
...
@JsonResponseWithException()
def Document_attributes_Name_material_Add(request):
...
@JsonResponseWithException()
def Document_attributes_Name_material_Update(request):
...
@JsonResponseWithException()
def Document_attributes_Name_material_Remove(request):
...
@JsonResponseWithException()
def Document_attributes_Name_material_Lookup(request):

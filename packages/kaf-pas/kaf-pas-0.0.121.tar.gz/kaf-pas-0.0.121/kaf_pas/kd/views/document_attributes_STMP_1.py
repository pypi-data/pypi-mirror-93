...
def Document_attributes_STMP_1_Fetch(request):
    return JsonResponse(
        DSResponse(
            request=request,
            data=Document_attributes.objects.
                select_related('attr_type').
                filter(attr_type__code='STMP_1').
                get_range_rows1(
                request=request,
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Add(request):
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Update(request):
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Remove(request):
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Lookup(request):
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Info(request):
...
@JsonResponseWithException()
def Document_attributes_STMP_1_Copy(request):

...
from typing import Any, Generic, Hashable, Iterable, Mapping, TypeVar, Union, cast
...
class KeyedDeserializerFactory(Generic[K, S, T]):
    ...
    def __call__(
        ...
        return error_utils.unwrap(
            partial(DeserializerFactoryError, type_=_mk_key_union(mapping)),
    ...
    def mk_ds_internal(
        ...
        return error_utils.collect_all(
            [
                ...
                factory.mk_ds(cs, Tag(_mk_key_union(mapping))),
            ]
            + [
...
def _mk_key_union(types_it: Iterable[DeserializableType]) -> Any:
    types = tuple(map(type, types_it))
    if types:
        return Union[types]
    return str

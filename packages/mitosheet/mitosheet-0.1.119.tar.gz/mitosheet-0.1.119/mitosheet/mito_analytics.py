...
def is_local_deployment():
    ...
    # NOTE: on_tljh_mito is depricated! But we leave it for safety for now
    on_tljh_mito = user.startswith('jupyter-')

    return not on_kuberentes_mito and not on_tljh_mito
    

# Read in user info, and if it does not exist, then we create 
# a new static id for the user
user_info = get_user_info()
if user_info is None:
    new_static_user_id = get_random_id()
    # Then, we check if the user is on the server. If they are are on the server, then we
    # can get their email as their user name. If they are not on the server, then we set
...
# Then, we read in the user info, which now will always exist (due to the above creation)
user_info = get_user_info()

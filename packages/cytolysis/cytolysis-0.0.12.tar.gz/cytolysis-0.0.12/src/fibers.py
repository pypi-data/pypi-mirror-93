    ...
    def show(self,*args, sorter=None, plotter=None, **kwargs):
        if __IPV__:
            if sorter is not None:
                fibs = filter(sorter, self)
            else:
                fibs = self

            if plotter is not None:
                return plotter(fibs, *args, **kwargs)
            else:
                return self.plot_fibers(fibs, *args, **kwargs)

        else:
            return False

    def plot_fibers(self, fibers, *args, **kwargs):
        for fiber in fibers:
            self.plot_fiber(fiber, *args, **kwargs)
        return True

    def plot_fiber(self, fiber, *args, **kwargs):
        p=ipv.plot(fiber[:,0], fiber[:,1], fiber[:,2], **kwargs)


###### A class to contain a single filament : trying to make it leaner !

class Fiber(np.ndarray):
    """ Fiber
        A class that contains a filament. Yep, a whole class for that. Derived from np.ndarray
    ...
    def __init__(self, *args, name="fiber", id=1,

...
class Frame():
    ...
    def show(self,*args, options=None, **kwargs):
...
class Simulation(list):
    ...
    def show(self, number, *args, **kwargs):
        if not __IPV__:
            ...
            self[number].show(*args, **kwargs)
            if not self.shown:
                ipv.show()
                self.shown = True

    def show_single(self, number, *args, **kwargs):
        if self.figure is not None:
            ipv.clear()
        self.show(number,*args,**kwargs)
        ipv.show()